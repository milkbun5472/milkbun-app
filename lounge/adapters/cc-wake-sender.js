...
// 把会客厅自然正文投进言秋自己的耐久唤醒队列。
// Stack-chan 的 voice_inbox 只留设备事件；无需新建 CC 会话，也不多跑一层 relay 模型。
// 文件只保存本机、0600、append-only。dispatch/room 等机器 ID 不写进正文记录。
const fs = require('node:fs');
const path = require('node:path');

const DEFAULT_WAKE_INBOX = '/Users/lisa/Desktop/lisa-practice/yanqiu-den/stackchan-relay/wake/yanqiu/inbox.jsonl';
...
function getWakeQueueHealth({ inboxPath = process.env.YANQIU_WAKE_INBOX || DEFAULT_WAKE_INBOX } = {}) {
  try {
    ...
    return { online: true, running: false, transport: 'dedicated_wake_queue' };
  } catch (error) {
    return { online: false, running: false, transport: 'dedicated_wake_queue', error: error.code || 'wake_inbox_unavailable' };
  }
}
