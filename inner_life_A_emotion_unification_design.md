...
影子对账期允许两套**暂时同时算但互不喂入**，只为了比较曲线。最终运行时只有 A。

⚠️**旧九维 v62.07 已经整个删掉了**（Lisa 2026-09-04 定）：模块 `js/heart-drive-shadow.js`、心上页那块折叠灰条、评审包那一栏、以及独立库 `lisa_desire_drive_shadow_v1`，一并清空。这一段原来写的是「保留 30 天只读审计后封存；不硬删」——对账早就做完了，那套东西却还在每一轮回复上写 state、追审计、再把最多 500 行整个读出来修剪，只为喂一块她从没展开过的灰条。所以不再封存，直接删；旧读数从此查不到。
