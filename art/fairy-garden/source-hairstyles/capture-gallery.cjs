...
(async () => {
  ...
  try {
    const page = await browser.newPage({ viewport: { width: 1310, height: 1070 } });
    ...
    await page.addStyleTag({ content: '.angles,#current-style{visibility:hidden}' });
    ...
    for (const [angle, turn, label] of [['front', 0, '正面'], ['side', Math.PI/2, '侧面'], ['back', Math.PI, '背面']]) {
      for (const style of styles) {
        await page.evaluate(({ id, turn }) => { bodySliderReview.setStyle(id); bodySliderReview.setView(turn, 2.3, 1.31); }, { id: style.id, turn });
      ...
      const sheet = await browser.newPage({ viewport: { width: 1560, height: 700 } });
    ...
    console.log('Captured 36 real GLB views and three review sheets:', out);
