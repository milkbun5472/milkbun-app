...
禁止用增加发束数量来模拟参考图中的阴影、纹理或层次。参考图中的明暗不等于独立几何体。每款发型优先使用少量大型连续曲面构成整体发量，只在轮廓、分缝和明确的发束交界处建立几何。顶部禁止鱼鳞式、瓦片式、花瓣式层层叠加。

针对 M02：顶部应读作一个蓬松完整的中分发体，而不是大量独立发束。核心几何仅包括：整体后脑发体 + 左右中分刘海两大束 + 两侧少量辅助发束。正面视觉发束控制在约 6–10 个大块，不要 15–30 个小块。

**当前代码中的三层径向发束和小块叠加不符合这条约束，是需要替换的旧稿实现，不能作为继续加密发束的基础。** 6–10 指正面可辨识的视觉大块，不能通过把大量小发束合并为一个网格对象来满足数量要求。按轮廓、整体体积和真实分缝检查造型；材质、表面着色和照明负责表达不需要独立几何的明暗与纹理。
