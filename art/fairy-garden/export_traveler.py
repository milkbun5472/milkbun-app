...
def main():
    ...
    D.apply_dims(objs, {})
    # A seated hem folds over the thighs; front/back surfaces remain separated.
    # This shape lives beside the body morphs, shared by every runtime instance.
    for o in objs:
        if o.get('deformPart') != 'Tunic body':
            continue
        basis=o.data.shape_keys.key_blocks[0]
        pose=o.shape_key_add(name='seated')
        for src,dst in zip(basis.data,pose.data):
            p=src.co.copy()
            if p.z < .69:
                weight=min(1.,(.69-p.z)/.18)
                p.y -= (.69-p.z)*weight*.9
                p.z += (.69-p.z)*weight*.68
            dst.co=p
    ...
    bpy.ops.export_scene.gltf(filepath=OUT, export_format='GLB', use_selection=True, export_apply=True,
                              export_morph=True, export_morph_normal=False, export_extras=True)
    ...
    with open(os.path.abspath(os.path.join(HERE, '..', '..', 'apps', 'fairy-garden', 'doll.json')), 'w', encoding='utf-8') as fh:
        json.dump({'hair': hair, 'dims': dims, 'outfits': O.CATALOG}, fh, ensure_ascii=False, indent=1)
        fh.write('\n')
    with open(os.path.join(os.path.dirname(OUT), 'outfits.mjs'), 'w') as fh:
        fh.write('// Generated by art/fairy-garden/export_traveler.py.\nexport const OUTFITS = '+json.dumps(O.CATALOG, ensure_ascii=False)+';\n')
