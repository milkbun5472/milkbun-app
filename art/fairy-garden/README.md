...
- 发型以贴头壳和连续收尖的弧形发片组成，替换圆珠刘海、串珠长发。
...
```sh
DOLL_OUT=/tmp/fairy-doll-refined /Applications/Blender.app/Contents/MacOS/Blender --background --python art/fairy-garden/preview.py -- dims hair face
...
输出 `dims-refined`、`hair-refined`、`face-refined` 三套 PNG 与同名 `.blend`。
体型图的第 1 / 3 个使用同一组偏宽肩参数，第 2 / 4 个使用同一组偏收腰参数，
分别交换短发和长发，便于比较；不是四个独立的身体模型。
...
五种发型共用身体拓扑以及非法参数报错。预览另需目视检查；数学通过不代表美术满意。
