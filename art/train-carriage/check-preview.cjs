...
(async()=>{
 ...
 try{
  ...
  for(const name of ['table','berths','top','overview']){await page.locator(`[data-view=${name}]`).click();assert.equal(await page.evaluate(()=>carriageReview.currentView),name);}
  ...
  await page.locator('#shell').uncheck();await page.locator('#night').check();await page.screenshot({path:'/tmp/lisa-train-night.png'});await page.locator('#night').uncheck();
  ...
  fs.writeFileSync(__dirname+'/browser-validation.json',JSON.stringify({glb_sha256:createHash('sha256').update(fs.readFileSync(__dirname+'/carriage.glb')).digest('hex'),state,desktop:[1280,900],mobile:[390,844],views:4,shellToggle:true,nightToggle:true,drag:true,horizontalOverflow:false,errors,failures},null,2)+'\n');
