...
景片合成到一张 CanvasTexture，固定在后窗玻璃外侧，圆角透明裁切、单面显示、受车体深度遮挡；雨雪不会成为盖住整页的屏幕特效。此方案是 2D 分层视差窗景，尚非可进入的 3D 地图，也不随相机绕行生成侧向新景物。新增一个平面（2 三角形）、一张 GPU 纹理；移动设备初次加载 960×480，桌面 1440×720，基础 CPU 景片缓存共 5 张 1600×600，另有观景地缓存 2 张 2200×600。30fps 上限；页面隐藏不推进，返回时不追补离开时间。GLB 保持 e87085ce 开头的原件。
...
Lisa反馈窗景太慢、单调后，加入 `journey.mjs`：每条线路是按行进距离推进的 120 行程单位演示环（普通路段1倍速每秒推进1单位），7 个沿途段落，间隔保留开阔路段。林线为道口/村落/木桥/松林站/隧道/湖面/会车；海线为港口/海镇/高架桥/海风站/海岬隧道/海湾/会车；田野为道口/风车/村落/麦田站/铁桥/河岸牧场/会车。包含等候车辆、站台人影/行李/猫、晾衣与炊烟、风车转动、船、羊群、飞鸟、电线杆、迎面列车。均为窗内2D绘制，未新增3D模型，也不是可下车互动的小站。
...
新增公开预览接口：`scenery.journey`、`scenery.nextStop()`、`scenery.seekJourney(distance)`（确定性回放和截图）；既有`state.distance`仍为统一行程轴。`journey.test.mjs`验证段落顺序/回绕/跳转可见度/隧道渐变，`check-journey.cjs`验证三线21段实际图像、灯光进入与恢复、倍速/暂停跳段、对象数稳定及390px控制可达性。证据 `journey-validation.json`；截图 `/Users/lisa/.codex/visualizations/2026/09/24/train-journey/`。240组合和原模型预览也回归。此版仍为固定演示环，观景慢行会延长实际用时，未实现长途路线存档、音效、摄影或互动事件。


### 湖畔与海湾观景地

`destinations.mjs` 为林线湖面与海线村落绘制完整构图：雪峰、红屋、花园、栈桥及同源倒影；海镇包含坡地彩屋、钟楼、石阶、码头和帆船。春花、夏伞、秋果筐与落叶、冬季积雪及收船直接改变画面物件。夜间窗灯点亮，沿用天气与昼夜状态。

入口与出口各18%行程渐显/渐隐，树枝形成前景遮挡，远山与岸边独立视差。全景期间实际车速平滑降到用户所选速度的36%，离开恢复。两个目的地按钮和 `?destination=lake|town` 可直接预览；暂停时定位中央，运行时从入口开始，保留季节、天气、时间和暂停状态。`scenery.destination` / `effectiveSpeed` 可读取当前状态，`carriageReview.visitDestination(id)` 用于展示。

新增两张 CPU 景片缓存，仍是一张 GPU 窗景纹理、一个平面；GLB 不变。`destinations.test.mjs` 检查减速边界，`check-destinations.cjs` 检查16个四季昼夜画面、速度恢复、暂停、手机按钮及对象/纹理稳定。此轮为摄影与拼图准备景色，尚未接入拍照或拼图操作。
