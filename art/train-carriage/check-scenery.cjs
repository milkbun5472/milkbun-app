...
(async()=>{
 ...
 try{
  ...
  const result=await page.evaluate(()=>{
   ...
   for(const route of ['forest','coast','country'])for(const weather of ['clear','cloudy','rain','snow','fog'])for(const hour of [6,12,18.5,22]){
    r.setEnvironment({route,weather,hour,playing:false});samples.push({route,weather,hour,...signature()});
   }
  ...
  assert.equal(result.samples.length,60);assert.ok(new Set(result.samples.map(s=>s.hash)).size>=58);
  ...
  await mobile.locator('[data-view=window]').click();await mobile.selectOption('#route','coast');await mobile.selectOption('#weather','snow');
