import {travelContext,travelEnvironment,ROUTE_LENGTH} from './travel.mjs?v=fg-aa26ce08d191440c';
import {visibleJourney} from '../../art/train-carriage/journey.mjs?v=fg-aa26ce08d191440c';
import {cropRect} from './album.mjs?v=fg-aa26ce08d191440c';
const hash=s=>{let h=2166136261;for(const c of String(s))h=Math.imul(h^c.charCodeAt(0),16777619);return h>>>0;};
export const FILM_LIMIT=6;
export function photoPlan(s,companion,width,height){
 if(!companion?.id||(s.companionPhotos||[]).length>=36)return null;
 ...
 if(used>0&&hash(companion.id+':'+event.id)%4===0)return null;
 const taste=hash(companion.id),seed=hash(companion.id+':'+key),zoom=1+(taste%4)*.3;
 const pan={x:.2+(seed%61)/100,y:.3+((seed>>>8)%41)/100};
 return{key,trip,count:used+1,distance:s.distance,crop:cropRect(width,height,zoom,pan),subject:event.label,environment:travelContext(s)};
}
...
export function exchangePhotos(s){
 ...
 return {...s,photos:[...(s.photos||[]),...waiting.filter(p=>!known.has(p.id))],companionPhotos:[]};
}
