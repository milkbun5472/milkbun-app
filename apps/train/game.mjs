import {createPassengers} from './passengers.mjs?v=fg-aa26ce08d191440c';
import {photoPlan,photoLabel,exchangePhotos} from './photography.mjs?v=fg-aa26ce08d191440c';
import {createTravelCamera} from './camera-view.mjs?v=fg-aa26ce08d191440c';
import {removeAlbumItem} from './album.mjs?v=fg-aa26ce08d191440c';
import {createPuzzleDesk} from './puzzle-view.mjs?v=fg-aa26ce08d191440c';
import {createCarriageView} from '../../art/train-carriage/view.mjs?v=fg-aa26ce08d191440c';
import {restoreTrip,travelEnvironment,travelContext,advanceTrip} from './travel.mjs?v=fg-aa26ce08d191440c';
import {ROUTES,SEASONS,WEATHERS} from '../../art/train-carriage/scenery.mjs?v=fg-aa26ce08d191440c';
...
try{if(!host)throw Error('请从小世界的列车入口进入。');state=restoreTrip(host.load().worlds?.train);view=await createCarriageView(document.querySelector('#stage'),{immersive:true});passengers=await createPassengers(view,host,document.querySelector('#stage'));view.setView('table');view.setZoom(.72);sync();
 ...
 const before=state;state=plan?{...state,companionPhotos:[...(state.companionPhotos||[]),photo],cameraLog:{key:plan.key,trip:plan.trip,count:plan.count,distance:plan.distance}}:{...state,photos:[...(state.photos||[]),photo]};if(!flush()){state=before;return null;}return photo;}
