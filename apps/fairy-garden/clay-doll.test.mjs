...
test('v2 sliders, faces, skin and outfit colour slots are all real data in the asset',()=>{
 ...
 for(const [id,o] of Object.entries(catalog.outfits)){for(const k of ['cloth','trim'])assert.ok(k in o.colors,id+' '+k);assert.ok(Object.keys(o.colors).every(k=>['cloth','trim','bottom','accent','boots'].includes(k)),id);const {boots,...painted}=o.colors;assert.deepEqual(nodes.find(n=>n.extras?.slotBase&&n.extras.outfit===id).extras.slotBase,painted);
