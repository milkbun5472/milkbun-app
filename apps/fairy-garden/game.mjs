import {installSeasonBook} from './season-book.mjs?v=fg-cec05e71011712d3';
import {makeMagicView} from './magic-view.mjs?v=fg-cec05e71011712d3';
import {installViewControls} from './view-controls.mjs?v=fg-cec05e71011712d3';
import * as THREE from 'three';
import {createMapLoader} from './map-loader.mjs?v=fg-cec05e71011712d3';
import {makeSurroundings} from './surroundings.mjs?v=fg-cec05e71011712d3';
import {GLTFLoader} from './vendor/GLTFLoader.js?v=fg-cec05e71011712d3';
import {START,MAPS,NODES,weather,targetFor,actionError,walkable,findPath,perform,restoreState,freshState,COMPANION_DESTINATIONS,advanceTime,timeLabel,gardenIntent,seasonOf,hitInteraction,journalText,companionNearby,floorHeight,deepestAllowed,SEED_KINDS,SEED_DAYS,seedError,sowSeed,readySeeds,keepNotes,pinNote} from './world.mjs?v=fg-cec05e71011712d3';
import {makeForest} from './forest.mjs?v=fg-cec05e71011712d3';
import {makeDepths} from './depths.mjs?v=fg-cec05e71011712d3';
import {createTraveler} from './traveler.mjs?v=fg-cec05e71011712d3';
import {makeCompanionController,dailySchedule,plannedActivity} from './companion.mjs?v=fg-cec05e71011712d3';
...
async function load(){try{const loader=assetLoader;await mapLoader.ensure(data.map);const a=await loader.loadAsync('./doll.glb?v=fg-cec05e71011712d3');playerAvatar=createTraveler(a.scene);actor=playerAvatar.root;actor.name='Player';scene.add(actor);companionAvatar=createTraveler(a.scene,true);companionAvatar.root.name='Companion';scene.add(companionAvatar.root);
...
function refreshTime(){
 ...
 if(coloredSeason!==season.index){coloredSeason=season.index;for(const view of Object.values(mapViews).filter(v=>v.root.userData.seasonTint!==false))view.root.traverse(o=>{for(const m of (Array.isArray(o.material)?o.material:[o.material]))if(m?.color){m.userData.baseTint ??= m.color.getHex();m.color.setHex(m.userData.baseTint).lerp(new THREE.Color(season.tint),.12);}});} rain.material.color.set(weather(data.day)==='细雪'?'#ffffff':'#e4f3ef');rain.material.size=weather(data.day)==='细雪'?.06:.035;rain.visible=!underground&&['细雨','细雪'].includes(weather(data.day));
}
