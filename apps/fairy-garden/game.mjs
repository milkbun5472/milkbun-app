import {makeOutdoor} from './outdoor.mjs?v=fg-7b571cbd113377b9';
import {installSeasonBook} from './season-book.mjs?v=fg-7b571cbd113377b9';
import {makeMagicView} from './magic-view.mjs?v=fg-7b571cbd113377b9';
import {installViewControls} from './view-controls.mjs?v=fg-7b571cbd113377b9';
import * as THREE from 'three';
import {createMapLoader} from './map-loader.mjs?v=fg-7b571cbd113377b9';
import {makeSurroundings} from './surroundings.mjs?v=fg-7b571cbd113377b9';
import {GLTFLoader} from './vendor/GLTFLoader.js?v=fg-7b571cbd113377b9';
import {START,MAPS,NODES,weather,targetFor,actionError,walkable,findPath,perform,restoreState,freshState,COMPANION_DESTINATIONS,advanceTime,timeLabel,gardenIntent,seasonOf,hitInteraction,journalText,companionNearby,floorHeight,groundPoint,deepestAllowed,SEED_KINDS,SEED_DAYS,seedError,sowSeed,readySeeds,keepNotes,pinNote,SHARD_KINDS,VEIN_POOL,veinLow,fillVein,pinShard} from './world.mjs?v=fg-7b571cbd113377b9';
import {makeForest} from './forest.mjs?v=fg-7b571cbd113377b9';
import {makeDepths} from './depths.mjs?v=fg-7b571cbd113377b9';
import {createTraveler} from './traveler.mjs?v=fg-7b571cbd113377b9';
import {makeCompanionController,dailySchedule,plannedActivity} from './companion.mjs?v=fg-7b571cbd113377b9';const $=id=>document.getElementById(id),KEY='fairy-garden-prototype-v1';
...
async function load(){try{const loader=assetLoader;await mapLoader.ensure(data.map);const a=await loader.loadAsync('./doll.glb?v=fg-7b571cbd113377b9');playerAvatar=createTraveler(a.scene);actor=playerAvatar.root;actor.name='Player';scene.add(actor);companionAvatar=createTraveler(a.scene,true);companionAvatar.root.name='Companion';scene.add(companionAvatar.root);
...
const before=data;data=perform(data,kind,id,intent||undefined);acting=null;$('progress').hidden=true;stream.visible=false;if(data===before){ui();say('这次行动没有完成，材料和进度都保留了。');return;}
 ...
 say(kind==='enter'?'回到小屋了。可以在这里走走，或者睡到明天。':kind==='dive'?'井比看上去深。落到第一层时，石壁上有细碎的光。':
