import {makeOutdoor} from './outdoor.mjs?v=fg-8819c0c2e674d772';
import {installSeasonBook} from './season-book.mjs?v=fg-8819c0c2e674d772';
import {makeMagicView} from './magic-view.mjs?v=fg-8819c0c2e674d772';
import {installViewControls} from './view-controls.mjs?v=fg-8819c0c2e674d772';
import * as THREE from 'three';
import {createMapLoader} from './map-loader.mjs?v=fg-8819c0c2e674d772';
import {makeSurroundings} from './surroundings.mjs?v=fg-8819c0c2e674d772';
import {GLTFLoader} from './vendor/GLTFLoader.js?v=fg-8819c0c2e674d772';
import {START,MAPS,NODES,weather,targetFor,actionError,walkable,findPath,perform,restoreState,freshState,COMPANION_DESTINATIONS,advanceTime,timeLabel,gardenIntent,seasonOf,hitInteraction,journalText,companionNearby,floorHeight,groundPoint,sleepPose,wakeSleeper,deepestAllowed,SEED_KINDS,SEED_DAYS,seedError,sowSeed,readySeeds,keepNotes,pinNote,SHARD_KINDS,VEIN_POOL,veinLow,fillVein,pinShard,CRAFT_WAYS,SPOTS,craftError,craftThing,placeThing,placedAt,thingReady,bellRings,donate,donateError,collectedKinds,recipeIndex,RECIPE_TOTAL,QUEST_KINDS,QUEST_TAKEN_MAX,questBoard,questTaken,questPaid,takeError,takeQuest,turnIn,lampOn,lampShelter} from './world.mjs?v=fg-8819c0c2e674d772';
import {makeForest} from './forest.mjs?v=fg-8819c0c2e674d772';
import {makeDepths} from './depths.mjs?v=fg-8819c0c2e674d772';
import {createTraveler} from './traveler.mjs?v=fg-8819c0c2e674d772';
import {makeCompanionController,dailySchedule,plannedActivity} from './companion.mjs?v=fg-8819c0c2e674d772';const $=id=>document.getElementById(id),KEY='fairy-garden-prototype-v1';
...
function ui(){
 ...
 // 收藏馆：手上有做好的就说出来。捐进去的东西背包挤不掉，也是这条链唯一的出口
 {const n=(data.things||[]).filter(t=>thingReady(data,t)).length;
  $('museum').hidden=!MAPS[data.map].stations.museum;$('museum').disabled=!!acting;
  $('museum').textContent=n?'🏛 收藏馆 · 有 '+n+' 件能捐':'🏛 收藏馆';}
...
async function load(){try{const loader=assetLoader;await mapLoader.ensure(data.map);const a=await loader.loadAsync('./doll.glb?v=fg-8819c0c2e674d772');playerAvatar=createTraveler(a.scene);actor=playerAvatar.root;actor.name='Player';scene.add(actor);companionAvatar=createTraveler(a.scene,true);companionAvatar.root.name='Companion';scene.add(companionAvatar.root);
...
async function completeAction(){if(loadingMap)return;const {kind,id,intent}=acting;
 ...
 if(kind==='museum'){acting=null;$('progress').hidden=true;ui();openMuseum();return;}
