import {stepRoute} from './locomotion.mjs?v=fg-11f9b505f79c0a3c';
import {makeLakeView} from './lake-view.mjs?v=fg-11f9b505f79c0a3c';
import {makeMuseum} from './museum-view.mjs?v=fg-11f9b505f79c0a3c';
import {museumCaption} from './museum.mjs?v=fg-11f9b505f79c0a3c';
import {makeOutdoor} from './outdoor.mjs?v=fg-11f9b505f79c0a3c';
import {installSeasonBook} from './season-book.mjs?v=fg-11f9b505f79c0a3c';
import {makeMagicView} from './magic-view.mjs?v=fg-11f9b505f79c0a3c';
import {installViewControls,orthographicPanDelta} from './view-controls.mjs?v=fg-11f9b505f79c0a3c';
import * as THREE from 'three';
import {createMapLoader} from './map-loader.mjs?v=fg-11f9b505f79c0a3c';
import {makeSurroundings} from './surroundings.mjs?v=fg-11f9b505f79c0a3c';
import {GLTFLoader} from './vendor/GLTFLoader.js?v=fg-11f9b505f79c0a3c';
import {START,MAPS,NODES,lakeFrozen,onLakeIce,weather,targetFor,actionError,walkable,findPath,perform,restoreState,freshState,COMPANION_DESTINATIONS,advanceTime,timeLabel,gardenIntent,seasonOf,hitInteraction,journalText,companionNearby,floorHeight,groundPoint,exitFor,sleepPose,wakeSleeper,deepestAllowed,SEED_KINDS,SEED_DAYS,seedError,sowSeed,readySeeds,keepNotes,pinNote,SHARD_KINDS,VEIN_POOL,veinLow,fillVein,pinShard,CRAFT_WAYS,SPOTS,craftError,craftThing,placeThing,placedAt,thingReady,bellRings,donate,donateError,collectedKinds,recipeIndex,RECIPE_TOTAL,driftPick,driftError,drawBottle,BOTTLE_DAYS,sealBottle,sealError,floating,missArrived,missWaiting,missGaveUp,missLetGo,missTaken,missMaterial,talkedWith,QUEST_KINDS,QUEST_TAKEN_MAX,questBoard,questTaken,questPaid,takeError,takeQuest,turnIn,lampOn,lampShelter} from './world.mjs?v=fg-11f9b505f79c0a3c';
import {makeForest} from './forest.mjs?v=fg-11f9b505f79c0a3c';
import {makeDepths} from './depths.mjs?v=fg-11f9b505f79c0a3c';
import {createTraveler} from './traveler.mjs?v=fg-11f9b505f79c0a3c';
import {makeCompanionController,dailySchedule,plannedActivity} from './companion.mjs?v=fg-11f9b505f79c0a3c';const $=id=>document.getElementById(id),KEY='fairy-garden-prototype-v1';
...
async function load(){try{const loader=assetLoader;await mapLoader.ensure(data.map,data.position);const a=await loader.loadAsync('./doll.glb?v=fg-11f9b505f79c0a3c');playerAvatar=createTraveler(a.scene);actor=playerAvatar.root;actor.name='Player';scene.add(actor);companionAvatar=createTraveler(a.scene,true);companionAvatar.root.name='Companion';scene.add(companionAvatar.root);
...
function updateWorld(elapsed,dt){
 ...
 if(!paused){data.position={x:actor.position.x,z:actor.position.z};if(!chatting)timeAccumulator+=elapsed;if(timeAccumulator>=1){const oldDay=data.day,minutes=Math.floor(timeAccumulator);timeAccumulator-=minutes;data=advanceTime(data,minutes);if(data.day!==oldDay){syncThaw();ui();save();if(!acting)say(`第 ${data.day} 天，${weather(data.day,data.epoch)}。林地又长出了新的材料。`);}else refreshTime();}
