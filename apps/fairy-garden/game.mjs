import {makeOutdoor} from './outdoor.mjs?v=fg-a0a1e1b4b7e12560';
import {installSeasonBook} from './season-book.mjs?v=fg-a0a1e1b4b7e12560';
import {makeMagicView} from './magic-view.mjs?v=fg-a0a1e1b4b7e12560';
import {installViewControls} from './view-controls.mjs?v=fg-a0a1e1b4b7e12560';
import * as THREE from 'three';
import {createMapLoader} from './map-loader.mjs?v=fg-a0a1e1b4b7e12560';
import {makeSurroundings} from './surroundings.mjs?v=fg-a0a1e1b4b7e12560';
import {GLTFLoader} from './vendor/GLTFLoader.js?v=fg-a0a1e1b4b7e12560';
import {START,MAPS,NODES,weather,targetFor,actionError,walkable,findPath,perform,restoreState,freshState,COMPANION_DESTINATIONS,advanceTime,timeLabel,gardenIntent,seasonOf,hitInteraction,journalText,companionNearby,floorHeight,groundPoint,sleepPose,wakeSleeper,deepestAllowed,SEED_KINDS,SEED_DAYS,seedError,sowSeed,readySeeds,keepNotes,pinNote,SHARD_KINDS,VEIN_POOL,veinLow,fillVein,pinShard,CRAFT_WAYS,SPOTS,craftError,craftThing,placeThing,placedAt,thingReady,bellRings,QUEST_KINDS,QUEST_TAKEN_MAX,questBoard,questTaken,questPaid,takeError,takeQuest,turnIn,lampOn,lampShelter} from './world.mjs?v=fg-a0a1e1b4b7e12560';
import {makeForest} from './forest.mjs?v=fg-a0a1e1b4b7e12560';
import {makeDepths} from './depths.mjs?v=fg-a0a1e1b4b7e12560';
import {createTraveler} from './traveler.mjs?v=fg-a0a1e1b4b7e12560';
import {makeCompanionController,dailySchedule,plannedActivity} from './companion.mjs?v=fg-a0a1e1b4b7e12560';const $=id=>document.getElementById(id),KEY='fairy-garden-prototype-v1';
...
const mapLoader=createMapLoader({maps:MAPS,loadAsset:async url=>{const root=(await assetLoader.loadAsync(url)).scene;root.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;}});return root;},factories:{forest:makeForest,depths:makeDepths},attach:root=>{const depth=new THREE.MeshDepthMaterial({depthPacking:THREE.RGBADepthPacking});root.traverse(o=>{if(o.isMesh)o.customDepthMaterial=depth;});scene.add(root);},detach:root=>scene.remove(root)}),mapViews=mapLoader.views;
...
function ui(){
 ...
 $('objective').textContent=inside?'在自己的小屋歇一会儿':down?(data.stones?'再往深处找找月石':'在这几层找到第一颗月石'):woods?'带一些森林的微光回家':data.blooms===3?'月光花开了，可以采收':data.potions?'用月露唤醒整圃月光花':data.herbs>=2&&data.mushrooms?'材料齐了，试试炼制月露':'沿着庭院小路，去林间采集';
 ...
 $('sit-pond').hidden=!MAPS[data.map].seats;$('sit-pond').textContent=data.seat?'起身走走':'去池边坐下';$('sit-pond').disabled=!!acting;$('village-places').hidden=!MAPS[data.map].sites;fillPlaces();$('brew').hidden=data.map!=='garden';$('well').hidden=inside;$('garden').hidden=inside;$('enter-home').hidden=!MAPS[data.map].exits.enter;$('enter-home').disabled=!!acting;
 $('brew').textContent=data.sand>=3?'⚗ 用星砂炼月露':'⚗ 炼制月露';$('brew').title=data.sand>=3?'星砂 ×3':'铃叶草 ×2 + 荧光菇 ×1 + 清水 ×1；或星砂 ×3';
 $('travel').textContent=inside?'走出小屋':down?'↑ 顺着梯子上去':woods?'↙ 返回微光庭院':'↗ 前往萤光林地';
 $('dive').hidden=data.map!=='garden';$('dive').textContent='↓ 下到井里';
 // 锅里做东西：背包里有碎片才亮
...
async function load(){try{const loader=assetLoader;await mapLoader.ensure(data.map);const a=await loader.loadAsync('./doll.glb?v=fg-a0a1e1b4b7e12560');playerAvatar=createTraveler(a.scene);actor=playerAvatar.root;actor.name='Player';scene.add(actor);companionAvatar=createTraveler(a.scene,true);companionAvatar.root.name='Companion';scene.add(companionAvatar.root);
...
function go(target,job=null){if(!ready||acting)return false;const points=findPath({x:actor.position.x,z:actor.position.z},target,data.map);if(!points?.length){say('那边暂时走不过去，换一块空地试试。');return false;}data.seat=null;data=wakeSleeper(data);companionController.reset();ui();cameraFollow=true;path=points;task=job;targetRing.position.set(target.x,floorHeight(data.map,target)+.025,target.z);targetRing.visible=true;$('hint').style.opacity=0;if(!job)say(data.map==='depths'?'脚下是碎石，慢一点。':data.map==='forest'?'脚步轻一点，草叶里藏着小小的光。':'慢慢走，庭院里的路都属于这个下午。');return true;}
function request(kind,id){if(!ready||acting||loadingMap)return;if(kind==='rest'&&sleepPose(data)){beginAction({kind});return;}const err=actionError(data,kind,id);if(err){say(err);return;}const p=targetFor(data,kind,id);if(!p)return;if(go(p,{kind,id,sleepMode:kind==='bed'?$('sleep-mode').value:null}))say(kind==='visit'?`去${MAPS[data.map].sites[id].label}看看。`:({bed:'去选好的卧室，慢慢歇下来。',enter:'回到门口，走进自己的小屋。',sit:'去池塘边找个位置坐下。',well:'去井边装一壶清水。',garden:data.blooms===3?'去把开好的月光花收进花藏。':data.potions?'带着月露，去唤醒整圃花。':'沿着小路，去看看花圃。',brew:'带齐材料，去炼药锅旁边。',travel:data.map==='garden'?'沿着庭院小路，走向林间。':'穿过石门，带着收获回家。',dive:'走到井边，扶着绳梯往下。',deeper:'找到那个往下的洞口。',ladder:'走回梯子下面。',seed:'去林地的微光旁，等同行者一起唤醒种子。',star:'去看看星铃花。',lamp:'把花带到屋前，做一盏星铃灯。',gather:'走近一点，摘下这一丛森林的礼物。',rest:'回屋睡一觉，让日子慢慢往前走。'})[kind]);}
let digging=false;
...
function fillPlaces(){const select=$('place-select');if(select.dataset.map===data.map)return;select.replaceChildren();for(const [id,site]of Object.entries(MAPS[data.map].sites||{})){const o=document.createElement('option');o.value=id;o.textContent=site.label;select.append(o);}select.dataset.map=data.map;}
...
function beginAction(job){acting={...job,intent:job.kind==='bed'?job.sleepMode:job.kind==='garden'?gardenIntent(data):null,time:0};actor.rotation.y=job.kind==='brew'?-Math.PI/2:Math.PI;$('progress').hidden=false;$('progress').firstElementChild.style.width='0%';ui();say(job.kind==='visit'?'在这里停一会儿。':({bed:'把枕头放好，床铺暖暖的。',enter:'推开门，屋里的光暖暖的。',sit:'在水边慢慢坐下来。',well:'井水晃了一下，清凉地流进壶底。',garden:data.blooms===3?'把开好的花轻轻摘下来，留住花根。':data.potions?'月露落在叶尖，微光沿着叶脉散开。':'一点一点浇下去，叶子舒展开来。',brew:'草叶、荧光菇和清水，在锅里轻轻旋转……',seed:'把手放在微光两侧，等两个人的魔力慢慢汇合。',star:'留一点清水，看看花的变化。',lamp:'把花的微光留进灯罩里。',gather:'轻轻摘下，给它留一点明天生长的余地。',travel:'穿过小路，风里换了一种草木香。',rest:'灯熄了。窗外的风，替你翻过了一页日历。'})[job.kind]);}
...
function openCraft(){
 ...
 else $('craft-hint').textContent='挑一片碎片，再挑一种做法。做过的会记进「屋里」。';
 for(const sh of rows){const b=document.createElement('button');
  b.setAttribute('aria-pressed','false');
  b.innerHTML='<b>〔'+(SHARD_KINDS[sh.kind]||'碎片')+'〕</b>'+sh.text;
  b.onclick=()=>{craftPick=sh.id;[...list.children].forEach(x=>x.setAttribute('aria-pressed','false'));
   b.setAttribute('aria-pressed','true');[...ways.children].forEach(x=>x.disabled=false);};
  list.append(b);}
 for(const [key,way] of Object.entries(CRAFT_WAYS)){const b=document.createElement('button');
  b.textContent=way.label;b.title=way.note;b.disabled=true;
  b.onclick=()=>{if(!craftPick)return;const err=craftError(data,craftPick,key);
   if(err){say(err);return;}
   const before=data;data=craftThing(data,craftPick,key);
...
async function completeAction(){if(loadingMap)return;const {kind,id,intent}=acting;
 ...
 if(kind==='travel'||kind==='enter'){loadingMap=true;say('正在打开前方的小路…');try{await mapLoader.ensure(MAPS[data.map].exits[kind].to);}catch(e){acting=null;$('progress').hidden=true;ui();say('前方场景没有加载完成，进度留在原地，稍后再试。');return;}finally{loadingMap=false;}}
const before=data;data=perform(data,kind,id,intent||undefined);acting=null;$('progress').hidden=true;stream.visible=false;if(data===before){ui();say('这次行动没有完成，材料和进度都保留了。');return;}
 if(kind==='travel'||kind==='enter'||kind==='dive'||kind==='ladder'){actor.position.set(data.position.x,.08,data.position.z);flash();showMap(true);mapLoader.keep(data.map);}
 ...
 say(kind==='bed'?($('sleep-mode').value==='companion'?'已经给 TA 留好床了，你可以继续走走。':'躺下来休息了。可以睡到明天，也可以随时起床。'):kind==='enter'?'回到小屋了。可以在这里走走，或者睡到明天。':kind==='visit'&&id==='home'&&bellLine()?MAPS[data.map].sites[id].text+' '+bellLine():
...
function applyCompanion(mode=data.companion.mode){if(mode!==data.companion.mode)data=wakeSleeper(data,'companion');data={...data,companion:{...data.companion,name:$('companion-input').value.trim().slice(0,16)||'同行者',temperament:$('companion-type').value,mode}};companionController.reset();save();updateCompanionUI();}
...
function updateWorld(elapsed,dt){
 ...
 if(!paused){data.position={x:actor.position.x,z:actor.position.z};if(!chatting)timeAccumulator+=elapsed;if(timeAccumulator>=1){const oldDay=data.day,minutes=Math.floor(timeAccumulator);timeAccumulator-=minutes;data=advanceTime(data,minutes);if(data.day!==oldDay){ui();save();if(!acting)say(`第 ${data.day} 天，${weather(data.day,data.epoch)}。林地又长出了新的材料。`);}else refreshTime();}
...
let last=performance.now(),lastRender=0;function frame(now){requestAnimationFrame(frame);if(document.hidden){last=now;return;}const elapsed=Math.min((now-last)/1000,1),dt=Math.min(elapsed,.05);last=now;clock+=dt;if(loadingMap){renderer.render(scene,camera);return;}
 ...
 if(data.seat&&!moving)actor.rotation.y=MAPS[data.map].seats[data.seat].heading;playerAvatar.animate(clock,{moving,gesture:data.seat?'sit':acting?'water':'rest',height:floorHeight(data.map,{x:actor.position.x,z:actor.position.z}),sleepPose:sleepPose(data)});
 ...
 mapViews[data.map]?.update?.(data,clock);magicView.update(data,clock);outdoor.update(data,mapViews,clock,cameraPan);
...
window.FairyGardenGame={
 ...
 snapshot:()=>({epoch:data.epoch,season:seasonOf(data.day),magic:{...data.magic},journal:(data.journal||[]).slice(-14),day:data.day,time:timeLabel(data.minute),weather:weather(data.day,data.epoch),map:MAPS[data.map].name,position:{...data.position},companion:{name:data.companion.name,map:MAPS[data.companion.map].name,position:{...data.companion.position},activity:companionController.view().status},inventory:{water:data.water,herbs:data.herbs,mushrooms:data.mushrooms,potions:data.potions,flowers:data.blooms,harvest:data.harvest}}),
 ...
 craft:(shardId,way)=>{const err=craftError(data,shardId,way);if(err)return err;
  const before=data;data=craftThing(data,shardId,way);if(data===before)return '这一炉没成。';
  ui();save();return '';},
