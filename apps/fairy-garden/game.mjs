import {noteTravelBack,receiveTravelArt,festivalDay,festivalOpen,festivalError,festivalMissing,festivalDone,holdFestival,festivalBook,atFestival,seatsOf,SEAT_KINDS,FOODS,RECIPE_COST,BAG_LABELS,foodOf,recipeOf,nightMarketDay,nightMarketOpen,nextNightMarket,nightStock,vendorAt,vendorOfFood,vendorSpot,VENDOR_STALLS,foodError,foodPay,foodLabel,buyFood,eatError,eat,cookError,cook,foodBook,restorePantry,giftOptions,giftError,giveGift,giftBook,giftKey,giftCatalogue,rolledStance,bondBook,companionDestinations,himGiveError,himGive,inviteError,invite,inviteMet,keepInvite,refuse,neighborNear,NEIGHBOR_REACH,neighborTalkError,noteNeighborTalk,neighborView,neighborWaveError,waveAtNeighbor,neighborGiftError,giveToNeighbor,neighborPairs,NEIGHBOR_HAND,MARKET_GOODS,marketOpen,atMarket,marketError,buy,starNightReady,keepStarNight,starChartPieces,STAR_CHART_NEED,spendTime,ACTION_MINUTES,guideStep,guideTarget,guideAdvance,guideSaid,markGuideSaid,setGuide,restoreGuide,todayHints,calendarMarks,marketDay,nextMarketDay,marketStock,marketGood,isBirthday,nearInteraction,seatAt} from './world.mjs?v=fg-2b387d084fe00867';
import {bubbleHold,bubbleShow,bubbleSay,BUBBLE_MAX,BUBBLE_GAP,VOICE_GAP} from './speech.mjs?v=fg-2b387d084fe00867';
import {actionGesture,actionDuration,makeHeldFlower} from './doll-life.mjs?v=fg-2b387d084fe00867';
import {mergeLook,outfitColors,outfitId,hairId,dyesOf,HAIR_MODES,lookForTa,DEFAULT_LOOK,COMPANION_LOOK} from './wardrobe.mjs?v=fg-2b387d084fe00867';
import {repairError} from './world.mjs?v=fg-2b387d084fe00867';
import {makeCurio} from './curio-view.mjs?v=fg-2b387d084fe00867';
import {makePlacedKeepsakes} from './keepsake-view.mjs?v=fg-2b387d084fe00867';
import {growingDreams,dreamStage,dreamError,SEED_PLOTS} from './world.mjs?v=fg-2b387d084fe00867';
import {makeDreamGarden} from './dream-view.mjs?v=fg-2b387d084fe00867';
import {WELL_CURIOS,WELL_KITS,wellTide,wellContext,chooseWellKit,shardName} from './world.mjs?v=fg-2b387d084fe00867';
import {makeWellSigns} from './well-view.mjs?v=fg-2b387d084fe00867';
import {makeExcavatingView} from './excavating-view.mjs?v=fg-2b387d084fe00867';
import {makeStarChartView} from './starchart-view.mjs?v=fg-2b387d084fe00867';
import {makeMarketView} from './market-view.mjs?v=fg-2b387d084fe00867';
import {villagePlan,installMapZoom} from './village-map.mjs?v=fg-2b387d084fe00867';
import {makeRitualView} from './ritual-view.mjs?v=fg-2b387d084fe00867';
import {makeScoopingView} from './scooping-view.mjs?v=fg-2b387d084fe00867';
import {weatherLook} from './weather-look.mjs?v=fg-2b387d084fe00867';
import {BREWS,brewKey} from './brewing.mjs?v=fg-2b387d084fe00867';
import {makeBrewingView} from './brewing-view.mjs?v=fg-2b387d084fe00867';
import {MILL_RECIPES,materialLine,millRemaining,millError} from './workshop.mjs?v=fg-2b387d084fe00867';
import {makeWorkshopView} from './workshop-view.mjs?v=fg-2b387d084fe00867';
import {openingTag} from './opening-view.mjs?v=fg-2b387d084fe00867';
import {stepRoute} from './locomotion.mjs?v=fg-2b387d084fe00867';
import {makeLakeView} from './lake-view.mjs?v=fg-2b387d084fe00867';
import {makeMuseum} from './museum-view.mjs?v=fg-2b387d084fe00867';
import {museumCaption} from './museum.mjs?v=fg-2b387d084fe00867';
import {makeOutdoor} from './outdoor.mjs?v=fg-2b387d084fe00867';
import {installSeasonBook} from './season-book.mjs?v=fg-2b387d084fe00867';
import {makeMagicView} from './magic-view.mjs?v=fg-2b387d084fe00867';
import {installViewControls,orthographicPanDelta,orthographicCameraPose} from './view-controls.mjs?v=fg-2b387d084fe00867';
import * as THREE from 'three';
import {createMapLoader} from './map-loader.mjs?v=fg-2b387d084fe00867';
import {makeSurroundings} from './surroundings.mjs?v=fg-2b387d084fe00867';
import {GLTFLoader} from './vendor/GLTFLoader.js?v=fg-2b387d084fe00867';
...
const VOICE_KEY='x_fairyGardenVoice';
let voiceOn=false;try{voiceOn=localStorage.getItem(VOICE_KEY)==='1';}catch(e){}
let bubbleTurn=0;   // 一轮一个号：念到一半被新的一轮顶掉，旧的回调就不许再翻页
// 收起行动那条栏上写的「此刻该干嘛」：装好视角控件之后才挂上，ui() 先跑到也不会炸。
let foldNote=null;
// 这一局是哪个世界（她 2026-09-19：「每个庭院档连一个列车档连一个别的什么档」）。
// 一条存档记录＝一段旅程，底下挂着好几个世界各自的进度；这儿只认自己那一格。
const WORLD='garden';
...
// 模型走 Draco 压缩（2026-09-19）：33 个 .glb 原来一共 110.8 MiB，压完 13.9 MiB。
// 压的是几何——贴图本来就只有 64x64、一个场景 22 张才 75 KB，不是大头。
// 解码器放 vendor/draco/（wrapper + wasm 共 245 KB，一次性；不带那个 500 KB 的
// 纯 JS 兜底版——three 本身就要现代浏览器，wasm 是有的）。
// ⚠️preload：解码器是异步取的，第一个模型要用时才拉会多等一下；这儿先热上。
// ⚠️**village-ground.glb 和 village-market.glb 故意不压**：flowerbeds.test.mjs 与
//   market.test.mjs 直接读这两个文件的原始字节，拿美术几何去核对碰撞数据对不对得上。
//   Draco 把顶点搬进扩展里，accessor 就没有 bufferView，那两条不变量当场读不出来。
//   再压它俩只省 2.6 MiB，却要么废掉测试、要么让测试去 node 里解 Draco——不值。
...
const arrival=makeArrivingTrain(scene,MAPS.garden.station);
...
//   有的才显示；一样都没有时整行收起来。数还是同一处写，只是空的那几样不摆出来。
 ...
 {const st=MAPS.garden.station.target,near=data.map==='garden'&&!!host?.travel&&Math.hypot(data.position.x-st.x,data.position.z-st.z)<6;$('board-train').hidden=!near;$('board-train').disabled=!!acting||boarding;}
...
// 聊天面板盖住了底下多少（她 2026-09-19：「现在气泡只显示最后一句话了」）。
// ⚠️原来写死 CHAT_SHEET=.52，那是聊天【只有一个高度】时的数。v71.38 改成三档之后，
//   只掀起一条输入栏时游戏仍旧按半屏算，他站在下半屏说话就被判成「出画面」，
//   整串气泡一条都不冒；等她收起面板，只剩最后一句还在保持时间里——所以看着像只显示最后一句。
//   照顶栏那份 setHeadClear 的做法：量出来多少就报多少，这边只信这一个数
//   （施工规则/one-public-mechanism.md：上下两条让位走同一套）。
let chatClear=0;
function sceneBottom(){
 const panel=$('action-panel');
 const byPanel=panel&&!panel.hidden?panel.getBoundingClientRect().top:innerHeight;
 return Math.min(byPanel,innerHeight-chatClear);
}
...
async function load(){try{const loader=assetLoader;await mapLoader.ensure(data.map,data.position);const a=await loader.loadAsync('./doll.glb?v=fg-2b387d084fe00867');playerAvatar=createTraveler(a.scene);actor=playerAvatar.root;actor.name='Player';scene.add(actor);companionAvatar=createTraveler(a.scene,true);companionAvatar.root.name='Companion';scene.add(companionAvatar.root);dollSource=a.scene;marketView.setDoll(dollSource);syncNeighbors();
...
function openGift(){const list=$('gift-list');list.innerHTML='';const opts=giftOptions(data);
 ...
 $('gift-hint').textContent=opts.length?(who?'递给'+who.name+'。邻居不挑东西，收了就是交情。':'挑一样递过去。一天只递一样，所以挑一挑。递过的都记在花册的「相处」那一页。'):'手上还没有能递的东西：花圃、井里、锅里都长得出来。';
...
// ⚠️她 2026-09-19：「按了也有时候没反应要多按几次」。病根不是距离（那会弹话），
//   是这条路上有三处【悄悄什么都不做】：问 TA 喜不喜欢那一枪要跑好几秒，
//   这几秒里 giftAsking 把每一次点都吞掉，吞得一声不吭；这一枪回来之后
//   如果手上又起了别的事，还会再吞一次。她只看到「点了没反应」，于是多点几次。
//   现在两件事一起做：点下去【看得见在忙】（按钮变灰＋提示改口），
//   而且每一处拦下来都必须说一句话——这条路上不许再有一处静默 return。
function giftBusy(on,text){const list=$('gift-list');
 for(const b of list.children)b.disabled=on;
 if(text)$('gift-hint').textContent=text;}
async function pickGift(o){
 if(giftAsking){say('正在问 TA 心里怎么想的，等一下就好。');return;}
 if(!ready){say('庭院还没醒过来，等一下。');return;}
 if(acting){say('先等手上这件事做完，再把东西递过去。');return;}
 giftAsking=true;giftBusy(true,'问问 TA 心里怎么想的…');
 try{let taste=null;
  const nb=giftTarget();
  ...
  // 一样一样地喜欢（她 2026-09-18）：固定那些查开局问的那张单子；
  // 井里挖出来／做出来的那些，第一次递出去这一下现问一句——档位这边掷好了给它。
  const key=giftKey(o);
  if(host&&host.tastes){
   const fixed=giftCatalogue().some(x=>x.key===key);
   if(!host.hasTastes()&&fixed)say('第一次递东西给 TA。先问问 TA 心里怎么想的，再递过去…');
   try{
    if(fixed){const rows=await host.tastes();taste=(rows||[]).find(r=>r.key===key)||null;}
    else if(host.tasteOne){
     const stance=rolledStance(String(data.partnerId||'')+':'+key);
     const words=await host.tasteOne({key,item:{name:o.name,note:o.note||'',from:o.from||'',fromWell:fromWell(o)},stance});
     taste={key,family:o.family,stance,words};
    }
   }catch(e){say(e.message);return;}}
  if(!ready){say('庭院还没醒过来，这一样还在你手上。');return;}
  if(acting){say('刚才手上起了别的事，这一样还在你手上，再递一次。');return;}
  data.position={x:actor.position.x,z:actor.position.z};const err=giftError(data,o.ref);if(err){say(err);return;}
 ...
 }finally{giftAsking=false;giftBusy(false);}}
...
// 念出来那个开关：她点一下就换一种，记在这台设备上（跟视角那几项一个口径）。
function paintAloud(){const b=$('read-aloud');if(!b)return;
 b.textContent=voiceOn?'🔊 念出来 · 开':'🔇 念出来 · 关';
 b.title=voiceOn?'他说的每一句都念出来，念完这句才冒下一句':'只冒字，不出声';}
// 站台上一眼就有「登上列车」（她 2026-09-25：「庭院进车站现在上不去列车」）
$('board-train').onclick=()=>{const st=MAPS.garden.station.target;if(Math.hypot(data.position.x-st.x,data.position.z-st.z)<.6){beginAction({kind:'boardTrain'});return;}if(go(st,{kind:'boardTrain'}))say('走到站台，准备上车。');};
$('read-aloud').onclick=()=>{voiceOn=!voiceOn;try{localStorage.setItem(VOICE_KEY,voiceOn?'1':'0');}catch(e){}
 if(!voiceOn){try{host&&host.stopAloud&&host.stopAloud();}catch(e){}bubbleTurn++;if(bubbleText)bubbleUntil=Date.now()+bubbleHold(bubbleText);}
 paintAloud();say(voiceOn?'开了。他说的每一句都会念出来，念完这句才冒下一句。':'关了，只冒字不出声。');};
paintAloud();
$('give-flower').onclick=()=>{if(!ready){say('庭院还没醒过来，等一下。');return;}
 if(acting){say('先等手上这件事做完，再挑东西递过去。');return;}
 data.position={x:actor.position.x,z:actor.position.z};const gate=giftGate();if(gate&&!giftOptions(data).length){say(gate);return;}openGift();};
...
// 从座位上起来：人和存档一起回到坐下之前站的那一点（坐垫上不是空地，站在那儿寸步难行）
const leaveSeatState=d=>{const seat=d.seat&&seatsOf(d.map)[d.seat];return seat?{...d,seat:null,position:{...(seat.approach||d.position)}}:d;};
function leaveSeat(){
 if(!data.seat)return;
 const seat=seatsOf(data.map)[data.seat],back=seat&&seat.approach;
 data={...data,seat:null,position:back?{...back}:data.position};
 if(back&&actor){actor.position.x=back.x;actor.position.z=back.z;}
 companionController.reset();
}
function sitAt(id){seatActivity='sit';if(data.seat===id){leaveSeat();ui();save();return;}request('sit',id);}
...
// ── 换季那晚的灯会（她 2026-09-18：「一季一件要准备的大事」）：走到栈桥、两个人都在、三样齐了才放；结算在 world.holdFestival
let festivalWait=false;
function openFestival(){const err=festivalError(data);if(err){say(err);festivalWait=err==='等他到了一起放。';return;}festivalWait=false;$('festival-text').value='';$('festival-hint').textContent='带来的：'+festivalBook(data).needs.map(n=>n.label).join('、')+'。灯上可以留一句，也可以空着。';$('festival-dialog').showModal();}
$('festival').onclick=()=>{if(!ready||acting)return;data.position={x:actor.position.x,z:actor.position.z};const err=festivalError(data);if(err==='先走到月潭栈桥。'){if(go(MAPS.garden.stations.bottle,{kind:'festival'}))say('走到月潭栈桥去。');return;}if(err){say(err);return;}openFestival();};
$('festival-close').onclick=()=>$('festival-dialog').close();
$('festival-go').onclick=()=>{data.position={x:actor.position.x,z:actor.position.z};const err=festivalError(data);if(err){say(err);$('festival-dialog').close();return;}const wish=$('festival-text').value.trim();const before=data;data=holdFestival(data,wish);$('festival-dialog').close();if(data===before){say('这一盏没放成。');return;}data=spendTime(data,'festival');ui();save();say('两盏灯从栈桥漂出去了'+(wish?'，那句话跟着灯走了':'')+'。灯笼捞回来放在盒子里，家里哪件家具上都摆得出来。');};
...
// 第一天少露点按钮（她 2026-09-18：「前十分钟太满」）：他带路那七步没走完，只露他带的那几颗和回家睡觉那几颗。
// ⚠️只是藏，不是禁：按钮的逻辑一行不动，带完路（或把带路关掉）就全出来。试玩档带路默认关着，看到的还是全部。
const FIRST_DAY_KEEP=new Set(['well','garden','travel','board-train','sow','notes','board','sit-pond','give-flower','rest','enter-home','walk-together','follow-him','lie-down','wake-player','wake-companion']);
function firstDay(){const step=guideStep(data);return !!step&&!String(step.id).startsWith('season:');}
function trimFirstDay(){const trim=firstDay();document.body.classList.toggle('first-day',trim);for(const b of document.querySelectorAll('#panel-content .actions button,#panel-content .secondary-actions button'))b.classList.toggle('first-day-hide',trim&&!FIRST_DAY_KEEP.has(b.id));$('first-day-note').hidden=!trim;}
...
let sayingTo='';
// 走近了跟邻居说句话：先开一个【临时打字框】，她说完 TA 回一句，都走气泡。
// ⚠️不做成一屏聊天：这是路上站住说两句，不是一个会话窗口。
function openNeighborBox(){
 ...
 const box=$('neighbor-box'),input=$('neighbor-input');
 box.hidden=false;input.value='';input.placeholder='跟'+nb.name+'说点什么';
 input.focus();
}
function closeNeighborBox(){$('neighbor-box').hidden=true;$('neighbor-input').blur();}
async function sayToNeighbor(){
 if(!ready||acting||loadingMap||sayingTo)return;
 const said=$('neighbor-input').value.trim().slice(0,80);
 data.position={x:actor.position.x,z:actor.position.z};
 const nb=neighborNear(data,NEIGHBOR_REACH);
 if(!nb){say('身边没有邻居。');closeNeighborBox();return;}
 const err=neighborTalkError(data,nb.charId);if(err){say(err);closeNeighborBox();return;}
 closeNeighborBox();
 // 她说的那句先冒出来（和她在庭院里说话走同一个 speak，不另写一套）
 if(said)speak([said],'me');
 sayingTo=nb.charId;refreshLeisure();
 try{
  const lines=await host.neighborSay({charId:nb.charId,door:{...nb.door},view:neighborView(data,nb.charId),said});
  // ⚠️回来之后重新核一遍：这一枪打了十几秒，她可能已经走开、或者换了一天
  if(neighborTalkError(data,nb.charId)){say('说话的劲头过去了，明天再找 TA。');return;}
  data=noteNeighborTalk(data,nb.charId,said||lines[0]);save();ui();
  speak(lines,nb.charId);
 }catch(e){say(e&&e.message||'这次没听清 TA 说什么。');}
 finally{sayingTo='';refreshLeisure();}
}
$('neighbor-say').onclick=openNeighborBox;
$('neighbor-send').onclick=sayToNeighbor;
$('neighbor-cancel').onclick=closeNeighborBox;
$('neighbor-input').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();sayToNeighbor();}
 else if(e.key==='Escape'){e.preventDefault();closeNeighborBox();}};
function refreshLeisure(){trimFirstDay();
 ...
 {const nb=neighborNear(data,NEIGHBOR_REACH);const err=nb?neighborTalkError(data,nb.charId):'身边没有邻居。';
  ...
  if((!nb||err||sayingTo)&&!$('neighbor-box').hidden)closeNeighborBox();
 ...
 $('give-flower').hidden=data.map!==data.companion.map&&!neighborNear(data,NEIGHBOR_HAND);$('give-flower').disabled=!!acting;$('give-flower').title=giftGate();
 $('festival').hidden=!(festivalDay(data.day)&&data.map==='garden'&&!festivalDone(data));$('festival').disabled=!!acting||!festivalOpen(data);$('festival').textContent=festivalOpen(data)?(festivalMissing(data).length?'放灯会的灯（还差'+festivalMissing(data).map(n=>n.label).join('、')+'）':'放灯会的灯'):'灯会天黑才开';$('festival').title=festivalError(data);
...
// 平面图（她 2026-09-18：「略丑，做好看一点，也可以放大缩小」）：地在 village-map.mjs 里画，这儿只标点和人。
// ⚠️图上只标她真会去的那几处，不是把 sites 整张表倒出来：广场／摊位／湖的两个岸挨着大地方，全标字会叠在一起。
const MAP_SPOTS=[
 ['site','home','家'],['site','hall'],['site','museum'],['site','pond'],['site','bridge'],
 ['site','neighbor1'],['site','neighbor2'],['site','neighbor3'],['site','oldTower'],['site','lakeNorth'],['site','lakeEast'],['site','market','灯串集市'],['site','railway'],['site','watermill'],
 ['station','garden','花圃'],['station','well','井'],['station','craft','锅'],['station','board','公告栏'],['station','travel','去林地']
];
const mapZoom=installMapZoom($('map-plan'),$('map-view'),{zoomIn:$('map-zoom-in'),zoomOut:$('map-zoom-out'),reset:$('map-zoom-reset')});
function openMap(){
 const plan=villagePlan(),esc=t=>String(t).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));
 const at=plan.at;
 let svg=plan.open;
 ...
 for(const [kind,id,name] of MAP_SPOTS){
  ...
  hitAreas+='<circle class="spot-hit spot-tap" data-x="'+pos.x+'" data-z="'+pos.z+'" data-name="'+esc(label)+'" cx="'+p.x+'" cy="'+p.y+'" r="18"/>';
  markers+='<g class="spot-hit" role="button" tabindex="0" data-x="'+pos.x+'" data-z="'+pos.z+'" data-label="'+esc(label)+'">'
    +'<title>走过去：'+esc(label)+'</title>'
    +'<circle class="spot" cx="'+p.x+'" cy="'+p.y+'" r="'+(kind==='site'?4.5:3.2)+'"/>'
 ...
 for(const [cls,who,person] of [['me','你',{map:data.map,position:data.position}],['them',data.companion.name,data.companion]]){
  ...
  svg+='<circle class="ring '+cls+'" cx="'+p.x+'" cy="'+p.y+'" r="11"/><circle class="'+cls+'" cx="'+p.x+'" cy="'+p.y+'" r="6"/>'
    +'<text class="who-label '+cls+'" x="'+p.x+'" y="'+(Number(p.y)-9).toFixed(1)+'" text-anchor="middle">'+esc(who)+'</text>';
  rows.push(who+'在'+MAPS.garden.name+'。');
 }
 svg+=plan.close;
 $('map-view').innerHTML=svg;
 $('map-where').textContent=rows.join('')+(hidden?'雾大，远处那 '+hidden+' 处这会儿看不清。':'')+'点一处，他会带你走过去——是走过去，不是一下子到。';
 $('map-dialog').showModal();
 // 开图先把她站的地方放到中间、放大一点（viewBox 是 meet 的，先算出留白）
 {const me=data.map==='garden'?at(data.position):null;if(me){const r=$('map-plan').getBoundingClientRect(),k=Math.min(r.width/plan.box.w,r.height/plan.box.h),ox=(r.width-plan.box.w*k)/2,oy=(r.height-plan.box.h*k)/2;mapZoom.focus(ox+Number(me.x)*k,oy+Number(me.y)*k,2);}else mapZoom.reset();}
}
...
// ⚠️听的是 village-map 发的 maptap，不是 click：外框捕获着指针，click 落不到那颗点上；拖过的不发
$('map-plan').addEventListener('maptap',e=>{
...
function meetTick(dt){
 ...
 for(let i=0;i<here.length;i++)for(let j=i+1;j<here.length;j++){
  ...
  // 路上撞见：这两位站住说两句。⚠️只有【她在场看得见】才打这一枪——
  //   背着她聊天，钱花了、体验一点没有。近不近由 PAIR_WATCH 判，不在这儿另定一个数。
  if(a.id!=='me'&&b.id!=='me')pairTalk(a,b);
 }
}
// 一对一天一次，现打一枪；她得同一张图、离得够近
let pairAsking='';
async function pairTalk(a,b){
 if(pairAsking||!host||!host.pairSay||!actor)return;
 const me={x:actor.position.x,z:actor.position.z};
 const watcher=data.map===a.map&&Math.hypot(me.x-a.at.x,me.z-a.at.z)<=PAIR_WATCH;
 if(pairTalkError(data,a.id,b.id,watcher))return;
 pairAsking=a.id+'|'+b.id;
 try{
  const lines=await host.pairSay({a:a.id,b:b.id,view:pairView(data,a.id,b.id)});
  // ⚠️回来之后重新核一遍：这一枪打了十几秒，她可能已经走开、或者换了一天
  if(pairTalkError(data,a.id,b.id,data.map===a.map))return;
  data=notePairTalk(data,a.id,b.id,lines[0]&&lines[0].text);save();ui();
  for(const line of lines)if(line.text)speak([line.text],line.who);
 }catch(e){}
 finally{pairAsking='';}
}
...
// 他报方向那四句：问一次就存着（宿主那侧存进这一档），转环时不再打枪
let starVoice=null,starVoiceAsked=false;
async function pullStarVoice(){
 if(starVoiceAsked||!host||!host.starVoice)return;
 starVoiceAsked=true;
 try{const v=await host.starVoice();if(v){starVoice=v;drawStar();}}catch(e){}
}
function drawStar(){
 ...
 //   差多少、往哪边【永远是代码算的】——和台上那一位看到的是同一个数。
 //   他那一枪只换【前半句怎么说】，而且一位角色问一次就存在这一档里；
 //   没问到（没配线路、正忙、抛错）就退回白话那四句，这一层永远说得出话。
 if(live&&!watching&&!starDone(data))speak(starReading(data,starVoice));
...
function openStar(role){
 ...
 data=takeStarRole(data,role);ui();save();drawStar();$('star-dialog').showModal();
 if(role==='dial')pullStarVoice();   // 她转环＝他在台上报方向，这时候才去问他那四句
 return true;
}
...
// 手机那条标题栏浮在场景上面，预览这一条也要让开它，不然小人的头被栏压住
// （她 2026-09-18：「这个也太高了」）。⚠️还是同一个数：setHeadClear 报进来的那个。
let headClear=0;
...
function drawPreview(){
 ...
 const clear=Math.max(0,Math.min(headClear,innerHeight*PREVIEW_BAND-60));
 const w=Math.max(1,innerWidth),h=Math.max(1,Math.round(innerHeight*PREVIEW_BAND-clear)),y=Math.max(0,innerHeight-h-clear);
...
function openSeatSite(id){const seat=seatsOf(data.map)[id],site=MAPS[data.map].sites?.[id];if(!seat||!site||seat.piece)return false;
 ...
 if(id==='railway'&&host?.travel){const board=document.createElement('button');board.textContent='登上列车';board.disabled=!!acting;board.onclick=()=>{$('spot-dialog').close();if(go(MAPS.garden.station.target,{kind:'boardTrain'}))say('走到站台，准备上车。');};list.append(board);}
...
// 坐面到底有多高，【量模型】，不猜（她 2026-09-18：「基本上所有坐下的都对不上」）。
// ⚠️家具的高度 rules 里 80 件只有 7 件写了，靠一张按种类猜的表迟早对不上某一件。
//   从坐垫那一点朝下打一条射线，打到什么就是什么——和水磨那张长桌用的是同一招。
//   量不到才退回 world.mjs 的 SEAT_RISE（那是兜底，不是依据）。
const seatTopCache=new Map();
function seatTop(key,point){
 const id=data.map+':'+key;
 if(seatTopCache.has(id))return seatTopCache.get(id);
 const view=mapViews[data.map];if(!view||!view.root)return null;
 view.root.updateMatrixWorld(true);
 const r=new THREE.Raycaster(new THREE.Vector3(point.x,4,point.z),new THREE.Vector3(0,-1,0));
 const floor=floorHeight(data.map,point,data);
 const hit=r.intersectObject(view.root,true).find(h=>h.point.y>floor+.1&&h.point.y<floor+1.2);
 const rise=hit?hit.point.y-floor:null;
 if(rise!=null)seatTopCache.set(id,rise);
 return rise;
}
...
const TAP_PAD=.9;   // 手指比判定圈宽这么多（她 2026-09-18：「范围能不能扩大点」）
function tapMap(clientX,clientY){chasing=false;const r=canvas.getBoundingClientRect();mouse.set((clientX-r.left)/r.width*2-1,-(clientY-r.top)/r.height*2+1);ray.setFromCamera(mouse,camera);
 // ⚠️他挡在井前面的时候，点井不能变成点他（她 2026-09-18：「他叫我打水我点击不了，一点就点到他身上」）。
 //   所以【他排在地上那些东西后面】：手指底下有能做的事就做那件事，什么都没有才算点他。
 //   点他这件事本来就还有两处入口（头顶那块名牌、行动栏那条同行者），不会因此够不着。
 const tappedCompanion=!!(companionAvatar?.root.visible&&ray.intersectObject(companionAvatar.root,true).length);{const frameId=placedKeepsakes.pick(ray);if(frameId&&host?.openFrame){host.openFrame(frameId);return;}}if(lakeView.pickBottle(ray)){request('bottle');return;}const good=marketView.pick(ray);if(good){buyGood(good);return;}const box=pickFurniture(ray);if(box&&openNear(box))return;const point=groundPoint(data.map,ray.ray.origin,ray.ray.direction,data);if(point){const picked=mapViews[data.map]?.pick?.(ray);const hit=picked?{kind:'gather',id:picked}:nearInteraction(data.map,point,data.depth,TAP_PAD);if(hit){if(hit.kind==='bed'){$('bed-select').value=hit.id;if($('panel-content').hidden)$('panel-toggle').click();say('选好了'+MAPS.home.beds[hit.id].label+'，在行动栏选休息安排。');return;}if(hit.kind==='visit'&&openSeatSite(hit.id))return;if(hit.kind==='visit'&&data.map==='museum'){say(MAPS.museum.sites[hit.id]?.text||'');openMuseum();return;}const key=spotKeyOf(hit);if(key&&openSpot(key))return;if(hit.kind==='seed'){$('magic-action').click();}else request(hit.kind,hit.id);return;}const furn=furnitureAtPoint(data.map,point);if(furn&&openNear(furn))return;
  // 点在座位上就是要坐下（她 2026-09-18：「这个池塘坐不了」——池边一直有一处座位，只是点不着）
  {const id=seatAt(data.map,point);if(id&&!acting){if(openSeatSite(id))return;sitAt(id);return;}}
  if(tappedCompanion){openCompanion();return;}if(walkable(point.x,point.z,data.map,data))go({x:point.x,z:point.z});}else if(tappedCompanion)openCompanion();}
...
// 'me' ／ 'companion' ／ 某位邻居的 charId。
// ⚠️原来只分「我」和「他」：邻居说的那句会挂到【同行者】头顶上——
//   看起来就是同行者在说别人的话。谁说的，气泡就挂谁头上。
let bubbleWho='companion';
const bubbleEl=()=>$(bubbleWho==='me'?'player-bubble':'companion-bubble');
// 这会儿说话的那位站在哪儿（邻居走的是 crew 里那具小人）
function bubbleAt(){
 if(bubbleWho==='companion'||bubbleWho==='me')return null;
 const n=restoreNeighbors(data.neighbors).find(x=>String(x.charId)===String(bubbleWho));
 return n&&n.map===data.map?n.position:null;
}
// 夜里十一点，手头的事做完就自己回家（她 2026-09-18：「到了23点能不能做完手头上的
// 事自动传送回家，然后点击回床上睡才到下一天」）。⚠️只送回家，不替她睡：
// 躺下那一下仍旧要她自己点床——一起睡的话还得等他也上床（world 的 actionError('rest')）。
...
function bedtimeCheck(){
 ...
 bedtimeDay=data.day;
 path=[];task=null;moving=false;targetRing.visible=false;
 data=leaveSeatState({...data,map:'home',position:{...MAPS.home.spawn}});
 companionController.reset();showMap();save();ui();
 say('天太晚了，你回到了小屋。点床铺躺下，这一天就过去了。');
}
// 念出来（她 2026-09-19：「能不能设置语音开关开了就每个气泡念完再到下一个气泡念」）。
// ⚠️走的是聊天里那条同一个 ttsSpeak（宿主那侧，自带缓存）：另写一套合成就是又开一处要付钱的地方。
// 只念他那只气泡——她自己说的话是她刚打出来的，没必要再念一遍给她听。
// 念不了（没配语音 API、他没有音色、合成失败）就原样退回定时那一套，不卡住。
// 一句话有两副样子：气泡上那副要收拾干净（换行压平、封到 120 字），
// 念出来／存回房间那副得是【他原样说的那一句】。
// ⚠️两副必须分开留着：拿气泡那副去合成，等她回房间点重听时，房间里存的是原样那一句，
//   两串字不一样＝缓存钥匙不一样＝又花一次钱（她 2026-09-19 问「存回房间也是缓存了的吧」）。
// 队里下一条先合成掉；她自己说的那几只不念，也就不用预热。
function warmNext(){
 if(!voiceOn||!host||!host.warmAloud||bubbleWho==='me')return;
 const next=bubbleQueue[0];
 if(next)try{host.warmAloud(next.say);}catch(e){}
}
function nextBubble(){
 const item=bubbleQueue.shift();
 if(!item){bubbleText='';bubbleUntil=0;$('companion-bubble').textContent='';$('player-bubble').textContent='';return;}
 const line=item.show;
 ...
 if(voiceOn&&host&&host.readAloud&&bubbleWho!=='me'){
  ...
  host.readAloud(item.say).then(ok=>{if(mine!==bubbleTurn)return;if(!ok){fallback();return;}
...
function speak(lines,who='companion'){
 const from=who==='me'?'me':(who&&restoreNeighbors(data.neighbors).some(x=>String(x.charId)===String(who))?String(who):'companion');
 const rows=(Array.isArray(lines)?lines:[lines])
  .map(x=>({show:bubbleShow(x),say:bubbleSay(x)})).filter(r=>r.show).slice(0,12);
 // ⚠️同一个人还在说的时候【接在后面】，不要把没冒完的那几句冲掉
 //   （她 2026-09-18：「有时候感觉最后一个气泡不会显示」——就是被下一次 speak 冲掉的）。
 const talking=bubbleWho===from&&(bubbleQueue.length||(bubbleText&&Date.now()<bubbleUntil));
 if(talking){bubbleQueue=[...bubbleQueue,...rows].slice(0,12);return;}
 clearTimeout(bubbleTimer);bubbleTurn++;try{host&&host.stopAloud&&host.stopAloud();}catch(e){}
...
function openCompanion(){if(!ready)return;const c=data.companion;$('companion-input').value=c.name;$('companion-input').disabled=!!boundPartner;drawSchedule();$('companion-detail').textContent=`现在在${MAPS[c.map].name}，${companionController.view().status}。${lastCompanionEvent||''}`;$('companion-follow').setAttribute('aria-pressed',c.mode==='follow');$('companion-routine').setAttribute('aria-pressed',c.mode==='routine');$('companion-wait').disabled=!!acting;
 // 他在同一张图上才递得着（和行动栏那颗同一个条件，不另写一份）；
 // 递不出去的时候不做成死按钮——按钮上直接写着为什么
 {const near=data.map===data.companion.map,gate=giftGate();
  $('companion-give').hidden=!near;$('companion-give').disabled=!!acting;
  $('companion-give').textContent=gate||('递一样东西给'+c.name);}
...
// 递东西这件事她会先在【他这一页】上找（她 2026-09-18：「怎么把手上的东西给他啊」）。
// ⚠️不在这儿另开一条路：点下去就是行动栏那颗，挑什么、递不递得出去还是那一处说了算。
$('companion-give').onclick=()=>{$('companion-dialog').close();$('give-flower').click();};
...
function updateWorld(elapsed,dt){
 ...
 drawNeighbors(dt);
 const c=data.companion,v=companionController.view(),root=companionAvatar.root;root.visible=c.map===data.map;root.position.set(c.position.x,.08,c.position.z);if(v.moving||v.gesture==='water'||v.gesture==='sit')root.rotation.y+=Math.atan2(Math.sin(v.heading-root.rotation.y),Math.cos(v.heading-root.rotation.y))*Math.min(1,dt*(onLakeIce(c.map,c.position,data)?4:12));{const seatNow=v.gesture==='sit'&&data.seat?seatsOf(data.map)[data.seat]:null,share=seatNow?.companion;
  companionAvatar.animate(clock,{skating:onLakeIce(c.map,c.position,data),moving:!paused&&v.moving,gesture:v.gesture,progress:(clock*.3)%1,height:floorHeight(c.map,c.position,data)+(share&&share.rise&&!v.moving?(seatNow.piece?(seatTop(data.seat,share)??share.rise):share.rise):0),sleepPose:sleepPose(data,'companion')});}
 ...
 // 说话的是邻居时，这只气泡改挂到那位头顶上（他和她各有各的那只，不占用）
 const other=bubbleAt();
 let bx=x,by=y;
 if(other){const q=new THREE.Vector3(other.x,1.95,other.z).project(camera);
  bx=(q.x+1)*innerWidth/2;by=(1-q.y)*innerHeight/2;}
 const canShow=bubbleWho==='companion'?!offscreen:!!other;
 bubble.hidden=!speaking||bubbleWho==='me'||!canShow;
 if(!bubble.hidden){bubble.style.left=bx+'px';bubble.style.top=bubbleTop(bubble,by)+'px';}
 // 她自己那只：挂在她头顶上，跟着她走
...
let last=performance.now(),lastRender=0;function frame(now){requestAnimationFrame(frame);if(document.hidden){last=now;return;}const elapsed=Math.min((now-last)/1000,1),dt=Math.min(elapsed,.05);last=now;clock+=dt;if(loadingMap){renderer.render(scene,camera);return;}
 ...
 const myRise=mySeat?(mySeat.piece?(seatTop(data.seat,mySeat)??mySeat.rise??0):(mySeat.rise||0)):0;
 if(mySeat&&!moving){actor.rotation.y=mySeat.heading??actor.rotation.y;
  actor.position.x+=(mySeat.x-actor.position.x)*Math.min(1,dt*8);
  actor.position.z+=(mySeat.z-actor.position.z)*Math.min(1,dt*8);}
 playerAvatar.animate(clock,{moving,skating:onLakeIce(data.map,actor.position,data),gesture:data.seat?seatActivity:actionGesture(acting),seated:!!data.seat,progress:acting?acting.time/actionDuration(acting):0,height:floorHeight(data.map,{x:actor.position.x,z:actor.position.z},data)+(mySeat&&!moving?myRise:0),sleepPose:sleepPose(data)});
 if(festivalWait&&!acting&&!isMenuOpen()&&companionNearby(data)){data.position={x:actor.position.x,z:actor.position.z};openFestival();}
 ...
 updateWorld(elapsed,dt);if(data.map==='garden')arrival.update(dt);else arrival.hide();
...
window.FairyGardenGame={
 ...
 setHeadClear:px=>{const n=Math.max(0,Math.min(300,Number(px)||0));
  headClear=n;document.documentElement.style.setProperty('--head-clear',n+'px');return n;},
 giftCatalogue:()=>giftCatalogue(),
 ...
 getTravelFrame:id=>{const t=[...(data.things||[]),...(data.collection||[])].find(x=>(x.id===id||x.sourceId===id)&&x.recipe==='travelframe');return t?JSON.parse(JSON.stringify(t)):null;},
 noteTravelBack:(sourceId,who,text,name)=>{const before=data;data=noteTravelBack(data,sourceId,who,text,name);if(!save()){data=before;throw Error('背面没有保存成功，请重试。');}ui();return true;},
 receiveTravelArt:item=>{const before=data;data=receiveTravelArt(data,item);if(!save()){data=before;throw Error('相框没有保存成功，请重试。');}ui();return true;},
 ...
 getDyes:who=>{const look=neighborOf(data,who)?.look||(who==='companion'?data.companion.look:data.look)||{};return dyesOf(look,who==='me'?DEFAULT_LOOK:COMPANION_LOOK);},
 ...
 // ⚠️不是只在进门那一瞬间试一次：原来那一份写在宿主的 ready() 里，那会儿角色还没就位
 //   就永远错过，他一直留着那头长卷发。现在谁就位了就补谁，补过就再不动（她挑过的最大）。
 ensureLook:(who,ta,extra)=>{if(!ready)return false;
  const n=neighborOf(data,who),cur=n?n.look:who==='companion'?(data.companion&&data.companion.look):data.look;
  if(cur&&cur.hair)return false;
  return applyLook(who,{...lookForTa(ta),...(extra||{})});},
