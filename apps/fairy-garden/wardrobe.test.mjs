...
test('outfit changes keep each palette and all body sliders through save and resident restore',()=>{
 let look=mergeLook({skin:'#f8dfcc',hairColor:'#e6b5ce',cloth:'#abcdef',dims:{height:1.2}},{outfit:'garden'});
 ...
 assert.equal(outfitColors(s.look).cloth,'#998877');assert.equal(outfitColors(mergeLook(s.look,{outfit:'garden'})).trim,'#aabbcc');assert.equal(outfitColors(mergeLook(s.look,{outfit:'traveler'})).cloth,'#abcdef');assert.deepEqual(s.look.dims,{height:1.2,waist:.9});assert.deepEqual(s.neighbors[0].look,s.look);assert.equal(s.look.skin,'#f8dfcc');assert.equal(s.look.hairColor,'#e6b5ce');assert.equal(outfitColors(s.companion.look).boots,'#123456');
});
