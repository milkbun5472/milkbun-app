...
test('empty flowerbeds have ground geometry matching the relocated right-side collision footprints',()=>{
 const b=readFileSync(new URL('./village-ground.glb',import.meta.url)),j=JSON.parse(b.subarray(20,20+b.readUInt32LE(12)).toString());
 const plots=MAPS.garden.flowerbeds;assert.equal(plots.length,2);assert.deepEqual(plots.map(p=>[+p.x.toFixed(2),p.z,p.w,p.d]),[[-7.2,6.3,1.85,1],[-7.2,5.1,1.85,1]]);for(const p of plots)assert.equal(walkable(p.former.x,p.former.z,'garden'),true,'former invisible wall removed');
...
test('flowerbed stations remain reachable and watering/harvesting use the existing state writer',()=>{
 const original=freshState();assert.deepEqual(MAPS.garden.stations.note,MAPS.garden.stations.garden);assert.deepEqual(MAPS.garden.stations.sow,MAPS.garden.stations.garden);for(const action of ['garden','note','sow'])assert.ok(findPath(original.position,MAPS.garden.stations[action],'garden')?.length);
