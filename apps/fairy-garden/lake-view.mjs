import {activeWaterLights,gameMinute} from './workshop.mjs?v=fg-c92c6cbdbec99917';
import {makeIceView} from './ice-view.mjs?v=fg-c92c6cbdbec99917';
import * as T from 'three';
import {MAPS,driftError,seasonOf} from './world.mjs?v=fg-c92c6cbdbec99917';
...
export function makeLakeView(){
 ...
 const lights=new T.Group();root.add(lights);const lanterns=[];
 for(let i=0;i<4;i++){
  const g=new T.Group(),mat=new T.MeshStandardMaterial({color:'#f6d58d',emissive:'#efb855',emissiveIntensity:.7,roughness:.8});
  const base=new T.Mesh(new T.CylinderGeometry(.20,.24,.055,8),cork);g.add(base);
  const shade=new T.Mesh(new T.CylinderGeometry(.13,.16,.24,8,1,true),mat);shade.position.y=.14;g.add(shade);
  const flame=new T.Mesh(new T.SphereGeometry(.045,8,6),new T.MeshBasicMaterial({color:'#fff5c7'}));flame.scale.y=1.7;flame.position.y=.17;g.add(flame);
