...
import {MAPS,NODES,freshState,perform,targetFor,actionError,deepestAllowed,restoreState,walkable,fillVein,SHARD_KINDS} from './world.mjs';
...
test('下去、刨一处、再往下、爬上来：一圈走得通，时间跟着走',()=>{
 ...
 // v69.27 起矿脉刨出来的是【碎片】，不是矿（她定的方向：物资换成关于这个人的东西）
...
test('深处才有月石，而且它同时是钥匙和收获',()=>{
 ...
 assert.equal((s.shards||[]).length,0,'「沉下去的东西」是钥匙，不该同时又给一片碎片');
});

// 星砂是 v69.27 之前的产物：不再新产出，但旧存档里攒着的还能花掉（别浪费她的东西）
test('旧存档里的星砂照旧能换月露',()=>{
...
test('碎片从池子里取，池子空了也不让她空手',()=>{
 let s={...freshState(),map:'depths',depth:2};
 s=fillVein(s,[{kind:'memory',text:'你那天把伞往我这边偏了一点。',whole:true},{kind:'不认识的',text:'x'}]);
 assert.equal(s.vein.length,1,'认不出种类的一律丢掉');
 const vein=NODES.filter(n=>n.map==='depths'&&n.depth===2&&n.kind!=='stone');
 s=act(s,'gather',vein[0].id);
 assert.equal(s.shards[0].text,'你那天把伞往我这边偏了一点。');
...
test('每一层各刨各的，层数存得住',()=>{
 ...
 assert.equal(restoreState({version:8,map:'depths',depth:4}).depth,4);
 assert.equal(restoreState({version:8,map:'garden',depth:4}).depth,0,'人在地面上就不该还记着层数');
});
