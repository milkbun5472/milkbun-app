...
export function makeSurroundings(){
 ...
 const ground=new T.Mesh(new T.PlaneGeometry(160,160),new T.MeshStandardMaterial({map:tex,roughness:1}));ground.material.userData.seasonRole='ground';ground.rotation.x=-Math.PI/2;ground.position.y=.08;ground.receiveShadow=true;root.add(ground);
 ...
 leaves.userData.seasonRole='leaf';
 ...
 return {root,update(map,tint,config={}){const key=map+tint;if(last===key)return;last=key;ground.visible=config.ground!=='asset';const scale=Math.max(1,(config.radius||5.12)/5.12);trunks.scale.set(scale,1,scale);crowns.scale.set(scale,1,scale);}};
}
