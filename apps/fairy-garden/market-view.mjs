...
import {MAPS,MARKET_GOODS,MARKET_RARE,marketStock,marketOpen,marketError} from './world.mjs?v=fg-21a6d69b08509d10';
...
export function makeMarketView(){
 ...
 const byStall={};for(const id of [...Object.keys(MARKET_GOODS),...Object.keys(MARKET_RARE)]){(byStall[STALL_OF[id]]=byStall[STALL_OF[id]]||[]).push(id);}
 for(const [kind,ids] of Object.entries(byStall)){const s=stalls[kind];if(!s)continue;ids.forEach((id,i)=>{const g=new T.Group();const along=(i-(ids.length-1)/2)*.55;const dx=Math.cos(s.heading)*along,dz=-Math.sin(s.heading)*along;g.position.set(s.x+dx,.92,s.z+dz);
  const shape=id==='seed'?new T.IcosahedronGeometry(.1,2):id==='whole'?new T.OctahedronGeometry(.12):id==='dew'?new T.SphereGeometry(.11,10,8):id==='sand'?new T.IcosahedronGeometry(.11,0):id==='flower'?new T.SphereGeometry(.1,10,7):id==='mushroom'?new T.ConeGeometry(.11,.16,10):new T.BoxGeometry(.2,.09,.14);
  const color=id==='seed'?'#c2a2ee':id==='whole'?'#f2e6b8':id==='dew'?'#bfe8ea':id==='sand'?'#e9d7a6':id==='flower'?'#d6c8f0':id==='mushroom'?'#cfe6a4':'#9fb783';
