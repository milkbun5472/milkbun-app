...
export function createDistrictStream({chunks=[],loadAsset,attach,detach,dispose,now=()=>Date.now()}){
 ...
 async function load(c){if(closed||loaded.has(c.id)||pending.has(c.id))return pending.get(c.id);const work=(async()=>{try{const root=await loadAsset(c.asset);if(closed||distance(c)>range+11){dispose(root);return;}attach(root);loaded.set(c.id,root);failures.delete(c.id);}catch(e){failures.set(c.id,now()+5000);throw e;}finally{pending.delete(c.id);}})();pending.set(c.id,work);return work;}
