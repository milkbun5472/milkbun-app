...
test('他能做的每一件事，脚都真的走得到',()=>{const open=withOpenPaths();
 // ⚠️有几格看季节（冰面只有冬天有湖面可站），所以按【这一格成立的那天】去走
 const dayFor=id=>id==='ice'?43:1;
 for(const [id,a] of Object.entries(ACTIVITIES)){const at={...open,day:dayFor(id)};
  assert.ok(findPath(MAPS[a.map].spawn,a.target,a.map,[],at)?.length,id+' 走不到：'+JSON.stringify(a));}});
