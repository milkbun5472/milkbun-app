import './rules.js?v=fg-8819c0c2e674d772';
...
import {createNavigator} from './navigation.mjs?v=fg-8819c0c2e674d772';
...
export function donateError(s, id){
  if (s.map !== 'garden') return '收藏馆在公共厅里。';
  const t = (s.things || []).find(x => x.id === id);
  if (!t) return '先挑一样东西。';
...
export function actionError(s,kind,id){if(kind==='bed')return s.map==='home'&&Object.hasOwn(MAPS.home.beds,id)?'':'先回家选一张床。';if(kind==='sit')return MAPS[s.map]?.seats?.pond?'':'这里没有池边座位。';if(kind==='visit')return MAPS[s.map]?.sites?.[id]?'':'这里还没有开放这处地方。';
 ...
 // 走到收藏馆：捐东西在弹层里做，perform 不动状态
 if(kind==='museum')return s.map!=='garden'?'收藏馆在公共厅里。':'';
...
const ACTION_NAMES={bed:'回卧室休息',enter:'回小屋歇脚',sit:'在池边坐下',visit:'散步到访',well:'取水',garden:'照料或采收月光花',brew:'炼月露',gather:'采集',seed:'一起唤醒种子',star:'照料或采收星铃花',lamp:'制作星铃灯',travel:'穿过小路',craft:'在锅前做东西',dive:'下到井里',deeper:'再往下一层',ladder:'从井里上来',note:'收花笺',board:'看公告栏',museum:'去收藏馆'};
