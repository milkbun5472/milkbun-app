...
test('v4 village migration preserves inventories, dates, journal and forest positions, only relocating old garden coordinates',()=>{const old={...freshState(),version:4,epoch:'old-save',day:47,minute:850,water:2,herbs:17,harvest:29,position:{x:1,z:3},magic:{...freshState().magic,lamps:3,seeds:1},journal:[{day:46,partner:'旧同行者',actions:{well:2}}]};const next=restoreState(old);assert.deepEqual(next.position,START);for(const k of ['epoch','day','minute','water','herbs','harvest','magic'])assert.deepEqual(next[k],old[k]);assert.equal(next.journal[0].actions.well,2);assert.equal(next.version,8);assert.deepEqual(restoreState(next),next);const forest=restoreState({...old,map:'forest',position:{x:0,z:3},companion:{...old.companion,map:'forest',position:{x:2,z:3}}});assert.deepEqual(forest.position,{x:0,z:3});assert.deepEqual(forest.companion.position,{x:2,z:3});});
