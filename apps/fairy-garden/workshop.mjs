...
export function restoreWaterLights(raw){return (Array.isArray(raw)?raw:[]).filter(x=>Number.isInteger(x?.at)&&x.at>=420).slice(-4).map(x=>({at:x.at,together:x.together===true,where:x.where==='deck'?'deck':'island'}));}
...
export function releaseWaterLight(s,together,where='island'){if(waterLightError(s))return s;return {...s,waterLights:[...activeWaterLights(s),{at:gameMinute(s),together:!!together,where:where==='deck'?'deck':'island'}].slice(-4)};}
