...
(function (root, factory) {
  ...
  // gpt 的原话：**「这个世界不是围着我男朋友转，但因为我认识TA，所以我听什么都可能联想到TA。」**
  //
  //   远景     跟TA完全无关。这一档负责让世界不围着TA转。       ~25%
  //   生活半径 不写TA，但内容自然经过TA的地方、TA碰过的事物。   ~60%  ← 命根子在这一层
  //   痕迹     TA真的在这个台上留下过一点东西。                ~15%，且一天一个台最多一条
  //
  // ⚠️距离由**代码**掷（跟「今天哪个频率有台」同一条道理）。
  //   写成「今天TA出现的概率是 15%」交给模型自己拿捏，它会塌到一边去——
  //   要么天天是TA，要么一次都没有。
  // ⚠️**不许判定**。痕迹那一条不署名、不标注，界面上也永远不写「这是谁」。
  //   「到底是不是TA」这个悬着的问号就是这一档的全部价值；一旦坐实，它就死了。
  const DISTANCE = { FAR: "far", ORBIT: "orbit", TRACE: "trace" };
  const DIST_ZH = { far: "远景", orbit: "生活半径", trace: "TA留下的痕迹" };
  function rollDistances(n, ...parts) {
    ...
    for (let i = 0; i < k; i++) {
      ...
      // 一天一个台最多一条痕迹。多了就不是「诶，这个跟TA有关」，是蹲男朋友告白
      if (d === DISTANCE.TRACE) { if (traced) d = DISTANCE.ORBIT; else traced = true; }
      out.push(d);
    }
    // 一条远景都没有＝整个台都在围着TA转，那正是要防的另一头
  ...
  const DISTANCE_RULE = [
    ...
    "",
    "· **远景**——跟TA完全无关。照常写这片地方自己的事，这一档就是让这个世界不围着TA转的那一档。",
    "· **生活半径**——⚠️**不要写关于TA的内容**。写的还是这片地方的事，只是让它自然**经过**下面",
    "  「TA的生活半径」里那些东西：TA常在的地方、TA最近碰过的事物、TA那一行的活儿、TA听的那类歌。",
    "  播的人不认识TA，也不知道有TA这个人；是**听的人自己**会「诶，这个跟TA有关」。",
    "  判据一句：**把那个人整个删掉，这一条还成立吗？**成立才算写对了。",
    "· **TA留下的痕迹**——TA真的在这个台上留下过一点东西（点了首歌、丢了样东西、留了句话、",
    "  某个地方被人顺口提到）。两条要紧的：",
    "  ① **不署名、不点名、也不在话里暗示「你懂的」。**听的人自己怀疑是不是TA，那是这一条的全部价值；",
    "     一旦坐实，它就从「可能是TA」变成「系统通知你是TA」，两回事。",
    "  ② 写成那种**本来就会一整天反复播**的东西——寻物启事、一次点播、一条转告。",
    "     它越小越好：越小，听的人越会自己往下想。写成告白、写成TA专门讲给谁听的话，就全塌了。"
  ].join("\n");

  // ---- 这片地方 + TA的生活半径 ----
  // 她 2026-09-10：「太完全和 char 世界无关那也不好听」。
  // 电台是【公共设施】，不是一个角色。所以接进来的不是任何一个人，是TA脚下这片地方，
  // 以及**TA生活半径上那些东西**——地方、店、楼、活儿、歌、这几天正在传的事。
  ...
  function worldBedText(bed) {
    ...
    if (orbit.length) rows.push("\n【TA的生活半径】这些是那个人这几天真的沾过的地方和事物："
      + "\n" + orbit.map(x => "· " + x.slice(0, 60)).join("\n")
      + "\n⚠️这一份**不是让你写TA**，是让「生活半径」那一档的内容从这些东西旁边经过。别整份用完，挑得上的那几样就够。");
    ...
    rows.push("⚠️这些台是公共设施，**不认识任何具体的人**：不许提某个人的名字、不许说TA的私事和行踪，"
  ...
  //
  // 三个距离里，远景和生活半径都有料：这片地方、TA的生活半径、今天有人打进来说的话。
  // **只有痕迹那一档是空手的**——DISTANCE_RULE 要它写「TA真的在这个台上留下过一点东西」，
  // 可 bed 里唯一跟TA沾边的那一份还明写着「不是让你写TA」。于是它只能凭空编，
  // 编出来的东西跟这片地方没有任何一根线连着，听的人也就不会「诶」一下。
  //
  // 而时间线电台那边满手是料：TA在广播里讲过的话、有人打进去说过的话，
  // **而且那边已经定好了什么叫「真的发生过」——播出去的才算数**（v67.58）。
  // 所以这一份只收那边播出去的句子，没播的在那边等于没发生，在这边也不许拿来用。
  //
  // ⚠️料是料，不是剧情。整段情节搬到这个台上，等于替听的人把「这是谁」坐实了，
  //   那正是 DISTANCE_RULE 第①条要挡的东西——所以这一份只许取出最小的那一样。
  function traceBedText(bed) {
    const list = (bed && Array.isArray(bed.traces) ? bed.traces : []).map(S).map(x => x.trim()).filter(Boolean);
    if (!list.length) return "";
    return "\n\n【「TA留下的痕迹」那一档能用的料】这几句是别处的广播里真的播出去过的话——"
      + "有的是TA自己说的，有的是有人打进去说的：\n"
      + list.slice(0, 8).map(x => "· " + x.slice(0, 90)).join("\n")
      + "\n从里面挑**最小的一样**：一件东西、一个地名、一首歌、一句被人记住的话。"
      + "写成寻物启事、一次点播、一条转告那种本来就会一整天反复播的东西。"
      + "\n⚠️**别把它当故事讲出来**：整段情节搬上去，就等于告诉听的人这是谁。"
      + "照旧不署名、不点名、不暗示「你懂的」——听的人自己怀疑，那是这一档的全部价值。"
      + "\n这几句一条都用不上的时候，就照这个台自己的路数另写一样TA可能留下的小东西。";
  }
  ...
  function buildScheduleInstruction(st, now, seen, bed, dists, thread) {
    ...
    return [
      ...
      "",
      DISTANCE_RULE,
      // 今天一条痕迹都没掷到，这一份就不发——「这一轮根本用不上」是砍层唯一的合法理由
      //（施工规则/four-surfaces-same-context.md 结尾那条）。
      list.indexOf(DISTANCE.TRACE) >= 0 ? traceBedText(bed) : "",
      "",
