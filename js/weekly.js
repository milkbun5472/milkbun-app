...
(function () {
  ...
  // 记者是本刊【固定的那一个】：每周、每个角色，都是TA在问。
  // ⚠️原来只有三个形容词（机灵／擅长挖料／俏皮）——答的人有原话样本、问的人只有一串标签，
  //   于是整篇读成【有声音的人】对【没声音的人】。八卦小报里记者的嗓子本来是好看的那一半。
  // ⚠️不给TA名字：NAME_GUARD 明写「只能用本周出场人物里那几个真名」，
  //   多一个名字就是多一个会被写进正文的名字。TA署名就是「本刊记者」。
  // ⚠️这里只写【TA怎么问】，不写任何示例问题——示例问题会被逐字抄到每个角色头上
  ...
  // 腔调是壳，事是瓤（她 2026-09-24：「周刊有时候不讲人话」——截图里「双生管理方盘尾增持」「年轻封爵领受闭门羹」
  //   「理性霸权审判」，读完不知道是谁、干了什么）。病根是每版的声纹都在教它【怎么译】，没有一句说【译完还得认得出】。
  //   一句判据，所有媒体版（单版、整批）共用这一处。
  const PLAIN_CORE =
    "\n【读得懂是底线】这一版的腔调只管【怎么说】，不许把【说的是什么】也藏起来。"
    + "判据：她读完每一篇，能不能用一句大白话复述出「谁、对谁、干了哪件具体的事」？复述不出来，就是腔调把事吃掉了。"
    + "所以：人要让她一眼认出是谁（真名，或者只属于这个人的那个细节）；事情里那个具体的东西、那句具体的话，要原样出现至少一次——"
    + "腔调是加在这件认得出的事上面的那层滤镜，不是拿术语把它整个换掉。一段里黑话堆到压住了事，就删几个术语，把那件事本身摆回来。";

  // ---- 媒体腔（world book 层：世界观 + 声纹 + 禁止 + 缺席句）----------
  const VOICES = [
    {
      id: "victorian", name: "维多利亚社交小报",
      world:
        "设定：摄政／维多利亚时代的上流社交圈。你是这个圈子里一份匿名社交小报的笔者，把本周发生的事，当作上流社会的社交事件来报道。\n" +
        "声纹：\n① 爱用「某位不便具名的绅士／淑女」这类欲盖弥彰的指代——遮得越体面，越要让读者一眼猜出是谁（带上只属于他的那个细节）。\n" +
    ...
    },
    {
      ...
      world:
        ...
        "禁止：① 绝不承认这是小事——哪怕只是「TA发了句早安」也必须无比严肃地对待。② 不出现任何轻佻、卖萌语气。③ 不用表情符号或网络梗。",
      absent: "论亲密关系中的沉默：本周的集体缺席意味着什么"
    },
    {
      id: "naturalist", name: "自然观察笔记",
      world:
        "设定：一位博物学家在野外蹲守，把这几个人当作某种值得记录的物种来观察。\n" +
        "声纹：\n① 全程用观察笔记体：日期、天气、冷静记述，「该个体」「本群落」「初次记录到」——个体头一回出场就挂上真名，之后才叫「该个体」。\n" +
  ...
  function voiceOf(id) {
    return knownVoice(id) || { id: normalizeVoiceId(id) || "unknown", name: "未知文风", world: "", absent: "" };
  }
  ...
  function interviewPickFor(issueKey, allIds, pastIssues, weekStart) {
    ...
    past.forEach(function (iss) {
      ...
      // 就把这个人从袋子里划掉了,下一轮反而轮不到TA（她 2026-08-18 点名）
  ...
  async function genInterview(active, char, material, userName, reportLabel, interviewer) {
    ...
    const sys =
      ...
      (material && material.trim() ? material : characterText(char, "（本期报道窗口没有收录到可引用片段。这只表示本期缺稿；绝不等于最近没聊天、没人理他、关系变淡或久未联系。）")) +
      "\n\n【任务 · 产出两块】\n" +
      "① 专访：你作为一个真的想问出东西的记者，就本周记录做一段 4~6 轮 Q&A。采访必须像现场交锋，不是拿现成答案倒推一道送分题。\n" +
      "· 采访主题优先属于 " + char.name + characterText(char, " 自己：这一周的生活、工作/学业、兴趣、观察、烦恼、选择或怪念头。只有素材本身真的涉及关系时才可顺带追问，整篇绝不能默认围绕他和 ") + uName + " 的关系。\n" +
      "· 有记录时，先找记录里尚有解释空间的矛盾、代价或动机再发问；问题只给必要背景，不许把答案、结论或角色原话先塞进题干。\n" +
      characterText(char, "· 没有记录时，做一篇不依赖具体事件的人物近况访谈：从角色自己的生活与兴趣切入。不得声称他这周没和 ") + uName + " 聊、被冷落、感情淡了，也不得编造具体发生过的事。\n" +
      ...
      characterText(char, "  ⚠️绝大多数轮次【不该有】动作。只有当这句话真的让他手上或脸上动了一下时才写，没有就一个括号都别加。\n") +
      characterText(char, "  ⚠️括号里只许是镜头拍得到的那一下（伸手、别过脸、把杯子放回去这一类具体动作）；情绪注解（他有些无奈／语气冷下来／苦笑）一律不许写——那是替观众下判断，不是拍到的画面。\n") +
      "  ⚠️一条回答里至多一个括号，括号里至多十来个字。\n" +
      // ⚠️「每条都两三句」是生成感最明显的那个破绽
      characterText(char, "· 回答的长短要不齐：至少一轮短到一句、甚至半句（不想说、懒得说、一个字顶回去都算），至少一轮是他真的说开了的一大段。全篇每条都两三句，那是稿子不是采访。\n") +
  ...
  async function genDeskPage(active, globalText, stats, uName, personasBlock, empty, letterAuthors) {
    ...
    const sys = ANTI_CLICHE + "\n\n" + CB() +
      ...
      "\n· 必须是【这个人自己的立场和口气】:TA在意什么、替谁说话、嘴硬还是直说,全照TA的人设与本周表现。" +
  ...
  async function genLetters(active, personasBlock, globalText, uName, empty, letterAuthors) {
    ...
    const sys = ANTI_CLICHE + "\n\n" + CB() + CHARCARD_RULE +
      ...
      "\n· 只能由上面这些角色署名，每人恰好一封；用户「" + uName + "」不是写信人，绝不能替用户写信。" +
      "\n· 信里必须是【这个人自己的立场和口气】：TA在意的点、TA会替谁说话、TA嘴硬还是直说，全照TA的人设与本周表现。" +
  ...
  const themedLook = L => pagePalette("weekly", L, { paper: "bg", pale: "bg2", muted: "sub" });
  function lookOf(id) {
    const L = themedLook(VOICE_LOOK[id] || VOICE_LOOK.editorial);
  ...
  function coverSkin(key) {
    ...
    const tint = pageColor("weekly", "bg", COVER_TINTS[Math.floor(r() * COVER_TINTS.length)]);
    return { skin: sk, style: sk.css(tint), sections: SECTION_BRICKS.map(b => pagePalette("weekly", b, { solid: "tint", pale: "bg2", on: "bg2" })) };
  }
  ...
  const sectionLooks = () => Object.fromEntries(Object.entries(SECTION_LOOK_BASE).map(([key, look]) => [key, themedLook(look)]));
  function pageLook(sub, medias) {
    ...
    return sectionLooks()[(sub && sub.kind) || "contents"] || sectionLooks().contents;
  }
  ...
  function WeeklyHead(props) {
    const L = props.look || sectionLooks().contents;
    return h(Head, { zh: props.zh || "周刊", en: props.en, onBack: props.onBack, right: props.right,
      ink: L.ink, subInk: L.muted, lineInk: pageColor("weekly", "line", paletteAlpha(L.tint, "22")), bg: L.paper });
  }
    ...
    return h("div", { style: { position: "relative", marginBottom: 20 } },
      ...
      h("div", { style: { textAlign: "center", padding: "13px 0 9px" } },
        h("div", { style: { display: "flex", alignItems: "center", justifyContent: "center", gap: 9 } },
          ...
          h("span", { style: { fontFamily: F_BODY, letterSpacing: ".1em", fontSize: 8.5, color: t.fog, whiteSpace: "nowrap" } }, "周刊"),
          h("span", { style: { flex: 1, height: 1, background: t.line } })),
    ...
    return h("div", { style: Object.assign({ position: "relative", overflow: "hidden", width: "100%", minHeight: "100vh", padding: "0 20px 40px" }, ck.style) },
      ...
      h("div", { style: { display: "grid", gridTemplateColumns: "repeat(12,minmax(0,1fr))", columnGap: 6, alignItems: "end", padding: "7px 0 16px", borderBottom: "1px solid rgba(35,32,25,.32)" } },
        h("div", { style: { gridColumn: "1 / 9" } },
          h("div", { style: { fontFamily: F_BODY, letterSpacing: ".08em", fontSize: 7.5, color: sub } }, "一个人的周刊"),
        ...
        h("div", { style: { gridColumn: "9 / 13", paddingBottom: 2, borderLeft: "1px solid " + progressTrack, paddingLeft: 9 } },
          h("div", { style: { fontFamily: F_BODY, fontSize: 7.5, letterSpacing: ".08em", color: sub } }, "第 " + (props.num || "—")),
        ...
        laid.map(function (L, i) {
          ...
          return h("button", { key: i, onClick: L.it.onOpen, className: "text-left active:opacity-60",
            ...
            h("div", { style: { display: "flex", alignItems: "center", justifyContent: c.align === "right" ? "flex-end" : "flex-start", gap: 6, marginBottom: 6 } },
              ...
              // ⚠️原来这儿还挂一行小字英文（版面的洋名）压在中文标题上面。
              //   号码 + 横线已经给足编辑部的节奏，那一段是多余的（no-english-titles）。
              null),
      ...
      additions.length ? h("section", { "data-weekly-cover-additions": "true", style: { marginTop: 22, borderTop: "1px solid " + progressTrack, paddingTop: 13 } },
        h("div", { style: { display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, marginBottom: 10 } },
          h("div", { style: { fontFamily: F_BODY, fontSize: 7.5, letterSpacing: ".08em", color: sub } }, "这期新加的"),
          ...
          additions.map(function (A, i) {
            ...
            return h("button", { key: A.it.id || A.index, onClick: A.it.onOpen, className: "text-left active:opacity-60",
              ...
              h("div", { style: { display: "flex", alignItems: "center", gap: 6, marginBottom: 9 } },
                ...
                h("span", { style: { minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: F_BODY, fontSize: 7, letterSpacing: ".08em", color: metaInk } }, A.it.en || "副刊")),
      ...
      h("div", { style: { display: "flex", alignItems: "center", gap: 10, marginTop: 15 } },
        ...
        h("div", { style: { fontFamily: F_BODY, fontSize: 7.5, letterSpacing: ".08em", color: sub } }, "点标题进去看")));
  }
  ...
  function WeeklyToolsSheet(props) {
    ...
    const baseButton = { width: "100%", minHeight: 116, padding: "18px 16px", background: "rgba(255,255,255,.28)", border: "1px solid " + pageColor("weekly", "line", paletteAlpha(L.tint, "45")), color: L.ink, fontFamily: F_BODY, fontSize: 16, textAlign: "left", position: "relative", overflow: "hidden" };
    let body = null;
    if (mode === "voices") {
      body = h("div", null,
    ...
    return h("div", { "data-weekly-space": "tools", style: Object.assign({ position: "absolute", inset: 0, zIndex: 50, color: L.ink, overflow: "hidden" }, pageBackground(L)) },
      h("div", { className: "h-full flex flex-col" },
        ...
        h("div", { className: "flex-1 min-h-0 overflow-y-auto", style: { padding: "24px 22px calc(env(safe-area-inset-bottom) + 28px)" } },
          ...
          h("div", { style: { position: "relative", padding: "18px 16px 22px", border: "1px solid " + pageColor("weekly", "line", paletteAlpha(L.tint, "55")), boxShadow: "8px 8px 0 " + L.pale } },
  ...
  function CoverSection(props) {
    ...
    return h("div", null,
      ...
      c.editorNote ? h("div", { style: { width: "76%", margin: "20px 0 0 auto", padding: "12px 14px", background: pageColor("weekly", "line", paletteAlpha(L.tint, "12")), borderLeft: "4px solid " + L.tint, fontFamily: "'STKaiti','KaiTi',serif", fontSize: 13, color: L.muted, lineHeight: 1.75 } }, "编者按 · " + c.editorNote) : null);
  }
  ...
  function InterviewEntry(props) {
    ...
    return h("div", { style: { position: "relative" } },
      ...
      (e.interview && e.interview.qa || []).map(function (qa, i) {
        return h("div", { key: i, style: { display: "grid", gridTemplateColumns: i % 2 ? "1fr 37px" : "37px 1fr", gap: 13, marginBottom: 24, paddingBottom: 20, borderBottom: "1px solid " + pageColor("weekly", "line", paletteAlpha(L.tint, "33")) } },
          ...
          h("div", { style: { gridColumn: i % 2 ? 1 : 2, marginTop: 7, fontFamily: "'Songti SC',Georgia,serif", fontSize: 15.5, color: L.ink, lineHeight: 1.82 } },
              ...
              // ⚠️朗读只念TA说的话：括号里那几下是动作，念出来就成了「TA伸手」这种旁白
      ...
      e.paparazzi && (e.paparazzi.title || e.paparazzi.body) ? h("div", { style: { margin: "26px -10px 0 36px", padding: "17px 18px", background: L.tint, color: "#fff" } },
        h("div", { style: { fontFamily: F_BODY, letterSpacing: ".08em", fontSize: 8, opacity: .7, marginBottom: 8 } }, "边角闲话"),
  ...
  function MediaDetail(props) {
    ...
    // ⚠️她 2026-09-01：「TA金句在下面单独重复了一次，而不是直接把文章里那句做成大字号 bold」。
  ...
  function IssueView(props) {
    ...
    // ---- 详情视图 ----
    let headZh = "本期", headEn = "第 " + num + " 期", detail = null;
    if (sub && sub.kind === "letters" && lettersSec) {
      headZh = "读者来信"; headEn = "";
      const LL = sectionLooks().letters;
      detail = h("div", { style: { margin: "0 -10px", color: LL.ink } },
        h("div", { style: { display: "grid", gridTemplateColumns: "82px 1fr", minHeight: 116, marginBottom: 26 } },
          ...
          h("div", { style: { background: LL.pale, padding: "19px 18px" } },
            h("div", { style: { fontFamily: F_BODY, fontSize: 8, letterSpacing: ".08em", color: LL.tint } }, "读者来信 · 03"),
      ...
      detail = h("div", { style: { margin: "0 -10px", color: DL.ink } },
        h("div", { style: { display: "grid", gridTemplateColumns: "1fr 92px", minHeight: 112, marginBottom: 25 } },
          h("div", { style: { padding: "19px 18px", background: DL.tint, color: "#fff" } },
            h("div", { style: { fontFamily: F_BODY, fontSize: 8, letterSpacing: ".1em", opacity: .72 } }, "资料室 · 02"),
      ...
      const sec = medias.find(function (s) { return s.id === sub.id; });
      if (sec) { headZh = voiceOf(sec.voiceId).name; headEn = ""; detail = h(MediaDetail, { sec: sec, busy: busyUnit === sec.id, onRegen: function () { regenMedia(sec); } }); }
    }
  ...
  function NewsroomHome(props) {
    ...
    return h("div", { "data-weekly-space": "newsroom", style: { minHeight: "100%", padding: "18px 20px calc(env(safe-area-inset-bottom) + 34px)", color: L.ink } },
      h("div", { style: { display: "grid", gridTemplateColumns: "1fr auto", gap: 14, alignItems: "end", borderBottom: "1px solid " + pageColor("weekly", "line", paletteAlpha(L.tint, "70")), paddingBottom: 13 } },
        ...
        h("div", { style: { fontFamily: F_BODY, fontSize: 8, letterSpacing: ".1em", color: L.muted, textAlign: "right", lineHeight: 1.7 } }, "周一到周日", h("br"), props.label || "")),
      h("div", { style: { position: "relative", minHeight: 410, margin: "25px 0 20px", padding: "19px 13px 24px", background: "linear-gradient(145deg," + paletteAlpha(L.pale, "aa") + ",rgba(255,255,255,.20))", border: "1px solid " + pageColor("weekly", "line", paletteAlpha(L.tint, "35")), overflow: "hidden" } },
        ...
        h("div", { "aria-hidden": "true", style: { position: "absolute", right: 24, bottom: 17, width: 84, height: 84, border: "1px solid " + pageColor("weekly", "line", paletteAlpha(L.tint, "55")), borderRadius: "50%" } }),
        h("div", { style: { position: "relative", width: "86%", maxWidth: 335, margin: "0 auto", paddingTop: 14 } },
          ...
          h("button", { onClick: hasIssue ? props.onRead : props.onGenerate, disabled: props.busy, className: "w-full text-left active:opacity-80", style: { position: "relative", minHeight: 320, padding: "22px 20px", background: L.paper, border: "1px solid " + pageColor("weekly", "line", paletteAlpha(L.tint, "77")), color: L.ink, boxShadow: "0 15px 34px rgba(48,38,29,.17)", overflow: "hidden" } },
            h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "start", borderBottom: "5px solid " + L.tint, paddingBottom: 13 } },
              ...
              h("div", { style: { borderLeft: "1px solid " + pageColor("weekly", "line", paletteAlpha(L.tint, "66")), paddingLeft: 13, fontFamily: F_BODY, fontSize: 8, letterSpacing: ".1em", lineHeight: 1.8, color: L.muted } }, "第 ", num, h("br"), props.label || "")),
            hasIssue ? h("div", null,
              h("div", { style: { background: L.tint, color: "#fffaf3", margin: "22px -20px 0", padding: "16px 20px 18px" } },
                h("div", { style: { fontFamily: F_BODY, fontSize: 8, letterSpacing: ".1em", opacity: .72 } }, "这一周 · 已装订"),
            ...
            h("div", { style: { position: "absolute", right: 18, bottom: 17, fontFamily: F_BODY, fontSize: 8, letterSpacing: ".1em", color: L.tint } }, props.busy ? "付印中…" : hasIssue ? "点开来读 →" : "点一下发刊 →"))),
