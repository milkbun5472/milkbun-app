(function (root, factory) {
  ...
  const MAIN_ID = "main";
  // 这几栏的字是给她看的，不是给工程师看的：说的是「TA进这扇门带着什么」，
  // 不是「cognition 权限位」。key 一个都没动。
  const GROUPS = {
    cognition: [
      ["formalMemory", "你们一起经历过的事", "TA记得你们的正式记忆和共同经历"],
    ...
    actions: [
      ...
      ["games", "TA可以拉你玩点什么", "允许TA在这间房自然提议玩小游戏"],
      // 一起写（她 2026-09-11：「一起学一起玩一起写能不能都同时存在一个房间里，懒得开那么多」）——
      // 本来就是独立开关，一间房想开几样开几样。同人文里TA替你写的那几章就落在这种房里。
      ["fanfic", "TA可以拉你一起写", "允许TA在这间房自然提议接着写那篇文"],
      // 一起读（她 2026-09-12：「我就是想在某个房间开一起读的开关」）——
      // 跟一起学、一起写同一个形状：房里开了这一条，TA才能开口说「接着读那本」。
      ["read", "TA可以拉你一起读", "允许TA在这间房自然提议接着读那本书"],
      // 算一卦（她 2026-09-13：「给房间开可以算塔罗」）——跟上面三样同一个形状。
      // ⚠️牌是代码发的：TA只能【提议】，抽哪几张、正逆全在她点开之后由塔罗那头掷，
      //   所以这一格准TA开口，不准TA报牌面（提示词那头写死了这一条）。
      ["tarot", "TA可以给你抽一张", "允许TA在这间房自然提议算一卦"]
    ],
    writeback: [
  ...
  const CTX_GATE = {
    // 永远给：这几栏说的是【TA是谁】和【怎么说话】，不是你们之间发生过什么
    always: [
    ...
    innerLife: [
      ...
      "blockLine",      // 正被拉黑 / 刚解除拉黑——这说的正是「你们处到哪一步」
      "onMe",           // 她今天带着谁的东西出门
    ...
    // 群里和见面时发生的
    // watchedNote＝今天她在【别的地方】看着TA刷手机、还敲了TA——跟朋友圈回声同一档：
    // 是这间房外面发生的事，隔离房里不该知道。
  ...
  // TA在这儿记不记得你们，和这儿发生的事出不出得了这道门。
  function doorLine(room) {
    const r = room || {}, c = r.cognition || {}, w = r.writeback || {};
    if (r.main) return "TA在这儿是完整的TA，说过的都算数。";
    const brings = c.formalMemory && c.innerLife ? "TA记得你们的全部"
      : c.formalMemory ? "TA记得你们经历过的事，但没带着现在的心情"
      : c.innerLife ? "TA带着现在的心情，但想不起具体经历过什么"
      : (r.startFrom || r.fork) ? "TA接着进门时留下的聊天" : "TA在这儿不记得你们的过去";
    const takes = w.sharedState && w.memoryCandidate ? "这儿说的话会跟着TA走出门"
  ...
  function normalize(room, personId) {
    ...
    return {
      ...
      // ⚠️旧存档里的 scenario 一律当【底子】——多数旧设定本来就是背景（TA 17 岁 / 你们是师生），
      //   那部分每轮发是对的，不用迁移。
  ...
  // 不按房筛的话，「不带出门」那间房照样看得见你在主线上跟TA上的课。
  ...
  function prompt(room, mainMessages, opts) {
    ...
    //   「出轨被TA捉奸在床一直人机八股得要死」就是这么来的，戏永远走不出第一拍。
    if (started && room.opening) lines.push("【这间房是从这儿开始的｜已经发生过的事，不是这一轮的指令】\n" + room.opening
      + "\n**它已经过去了。时间会往前走**：那一刻之后可以是吵起来、可以是谁都不说话、可以是TA转身走了、"
  ...
  //     不分书。聊完 b 再让TA写 a 的下一章，TA手上那份「我们说好的」会是 b 的。
  //
