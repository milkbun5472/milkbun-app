...
(function () {
  ...
  function MonopolyGame(props) {
    ...
    else if(pending&&pending.kind==="auction"){const tile=MONO_BOARD[pending.tile],me=players.find(function(p){return p.isUser;}),names=(pending.activeKeys||[]).map(function(k){const p=players.find(function(x){return x.key===k;});return p&&p.name;}).filter(Boolean);action=h("div",null,h("div",{style:{fontFamily:F_BODY,fontSize:13,color:t.ink,textAlign:"center",marginBottom:4}},"🔨 【"+tile.name+"】公开竞价 · 当前 $"+pending.ask),h("div",{style:{fontFamily:F_BODY,fontSize:10.5,color:t.fog,textAlign:"center",marginBottom:9}},"仍在场："+names.join("、")+" · 每轮 +$"+pending.step),h("div",{style:{display:"flex",gap:9}},h("button",{disabled:!me||me.cash<pending.ask,onClick:function(){decideAuction(true);},style:{flex:1,padding:12,borderRadius:11,background:t.ink,color:"#fff",opacity:(!me||me.cash<pending.ask)?.4:1}},"举牌 $"+pending.ask),h("button",{onClick:function(){decideAuction(false);},style:{flex:1,padding:12,borderRadius:11,background:t.bg2,border:"1px solid "+t.line,color:t.ink}},"退出竞价")));}
