...
(function () {
  ...
  function MonopolyGame(props) {
    ...
    function runAuction(ps,os,tileIndex,excludedKey,ls,eventParts){const tile=MONO_BOARD[tileIndex],floor=Math.ceil(tile.price*.55/10)*10,bidders=ps.map(function(x){if(x.bankrupt||x.key===excludedKey)return null;const st=monoStyle(x),g=monoGroupProgress(x,tile,os),premium=1+st.risk*.08+g.own*.09+(g.completes?.16:0),cap=Math.floor(Math.min(x.cash-Math.max(90,st.reserve*.7),tile.price*premium)/10)*10;return cap>=floor?{p:x,cap:cap,tie:monoHash(x.key+":"+tileIndex+":"+moves)}:null;}).filter(Boolean);if(!bidders.length){ls.push({type:"sys",say:tile.name+" 流拍，暂时留在银行。"});return;}bidders.sort(function(a,b){return b.cap-a.cap||b.tie-a.tie;});const top=bidders[0],second=bidders[1],bid=Math.min(top.cap,Math.max(floor,second?second.cap+10:floor)),win=top.p;win.cash-=bid;os[tileIndex]=win.key;ls.push({type:"event",say:"🔨 "+tile.name+" 进入拍卖，"+win.name+" 以 $"+bid+" 拿下。"});eventParts.push(win.name+"在拍卖中抢走"+tile.name+"，成交$"+bid);}
