...
(function () {
  ...
  function MonopolyGame(props) {
    ...
    const [logs,setLogs]=useState(sv&&sv.logs?sv.logs:[]), [busy,setBusy]=useState(false), [pending,setPending]=useState(null), [winner,setWinner]=useState(sv?sv.winner:null), [error,setError]=useState(""), [tableSay,setTableSay]=useState(""), [boardOpen,setBoardOpen]=useState(true);
    const pAvatar=avatarFor(t);
    ...
    const board=boardOpen?h("div",{style:{display:"grid",gridTemplateColumns:"repeat(11,minmax(0,1fr))",gridTemplateRows:"repeat(11,20px)",gap:2,padding:"4px 7px",flexShrink:0}},h("div",{style:{gridColumn:"2 / 11",gridRow:"2 / 11",borderRadius:13,background:t.bg2,border:"1px solid "+t.line,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:6}},h("div",{style:{fontSize:18}},"🏙️"),h("div",{style:{fontFamily:F_DISPLAY,fontSize:13,color:t.ink,marginTop:1}},cur?(cur.name+(cur.isUser?"（你）":"")):"经典城市棋盘"),h("div",{style:{fontFamily:F_BODY,fontSize:9.5,color:t.sub,marginTop:2}},"第 "+(moves+1)+" / "+maxMoves+" 手 · "+players.filter(function(p){return !p.bankrupt;}).length+" 人在场"),h("button",{onClick:function(){setBoardOpen(false);},style:{fontFamily:F_BODY,fontSize:9,color:t.tint,marginTop:5,padding:"3px 9px",border:"1px solid "+t.line,borderRadius:999}},"收起棋盘 · 看对话")),MONO_BOARD.map(function(tile,i){const here=players.filter(function(p){return !p.bankrupt&&p.pos===i;}),own=owners[i],owner=players.find(function(p){return p.key===own;}),lv=levels[i]||0,pos=monoGridPos(i);return h("div",{key:i,title:tile.name+(tile.price?" $"+tile.price:""),style:Object.assign({},pos,{borderRadius:4,padding:"0 1px",background:tile.type==="property"?(tile.color+"35"):t.bg2,border:"1px solid "+(i===(cur&&cur.pos)?t.tint:t.line),textAlign:"center",overflow:"hidden",minWidth:0})},h("div",{style:{fontFamily:F_BODY,fontSize:5.8,color:t.sub,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}},tile.icon||"▪",tile.name),h("div",{style:{fontSize:5.5,color:t.fog,whiteSpace:"nowrap",overflow:"hidden"}},tile.price?(owner?(owner.name.slice(0,2)+(lv?" L"+lv:"")):("$"+tile.price)):(tile.type==="station"?"直达":"")),h("div",{style:{fontSize:6,height:6,whiteSpace:"nowrap"}},here.map(function(p){return h("span",{key:p.key,title:p.name},p.isUser?"🔴":"●");})));})):h("button",{onClick:function(){setBoardOpen(true);},style:{flexShrink:0,margin:"3px 12px 5px",padding:"8px 12px",borderRadius:11,background:t.bg2,border:"1px solid "+t.line,display:"flex",justifyContent:"space-between",fontFamily:F_BODY,fontSize:11.5,color:t.sub}},h("span",null,"🏙️ 棋盘已收起 · 第 "+(moves+1)+" / "+maxMoves+" 手"),h("span",{style:{color:t.tint}},"展开棋盘"));
    const roster=h("div",{style:{display:"flex",gap:7,overflowX:"auto",padding:"4px 12px 8px",flexShrink:0,minHeight:55}},players.map(function(p,i){return h("div",{key:p.key,style:{minWidth:106,padding:"7px",borderRadius:10,background:i===turn&&!p.bankrupt?t.tint+"18":t.bg2,border:"1px solid "+(i===turn?t.tint:t.line),opacity:p.bankrupt?.45:1}},h("div",{style:{display:"flex",alignItems:"center",gap:5}},pAvatar(p,25),h("div",{style:{fontFamily:F_BODY,fontSize:11.5,color:t.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}},p.name+(p.isUser?"(你)":""))),h("div",{style:{fontFamily:"monospace",fontSize:10.5,color:t.sub,marginTop:4}},p.bankrupt?"已破产":"$"+p.cash+" · 地"+propertyCount(p.key)+(p.jailed?" · 关押":"")));}));
