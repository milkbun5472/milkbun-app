...
(function () {
  ...
  const MONO_BOARD = [
    ...
    { type: "property", name: "春日街", group: "晨曦", price: 180, rent: 45, color: "#d98f86" },
    { type: "chance", name: "机会", icon: "🎴" },
    { type: "property", name: "月桂巷", group: "晨曦", price: 220, rent: 55, color: "#d98f86" },
    { type: "tax", name: "维修费", amount: 100, icon: "🧾" },
    { type: "property", name: "海风路", group: "海岸", price: 260, rent: 65, color: "#71a7b7" },
    { type: "chance", name: "机会", icon: "🎴" },
    { type: "property", name: "星港", group: "海岸", price: 300, rent: 75, color: "#71a7b7" },
    { type: "bonus", name: "休息站", amount: 60, icon: "☕" },
    { type: "property", name: "银杏街", group: "云城", price: 340, rent: 85, color: "#967ab4" },
    { type: "chance", name: "机会", icon: "🎴" },
    { type: "property", name: "云顶路", group: "云城", price: 380, rent: 95, color: "#967ab4" },
    { type: "tax", name: "城市基金", amount: 120, icon: "🏛️" },
    { type: "property", name: "灯塔湾", group: "蔷薇", price: 420, rent: 110, color: "#b86976" },
    { type: "chance", name: "机会", icon: "🎴" },
    { type: "property", name: "玫瑰广场", group: "蔷薇", price: 460, rent: 125, color: "#b86976" }
  ];
  const MONO_CHANCE = [
  ...
  function MonopolyGame(props) {
    ...
    useEffect(function(){ if(phase!=="play"||!players.length)return; saveGameSnap("monopoly",{config:cfg,players:monoPlayerSnap(players),owners:owners,levels:levels,turn:turn,moves:moves,logs:logs.slice(-60),winner:winner,label:"第 "+(moves+1)+" 手 · "+players.filter(function(p){return !p.bankrupt;}).length+" 人在场"}); },[players,owners,levels,turn,moves,logs,phase,winner]);
    ...
    function finishIfNeeded(ps,os,ls,nextMoves){const alive=ps.filter(function(p){return !p.bankrupt;}); if(alive.length<=1||nextMoves>=45){const ranked=ps.slice().sort(function(a,b){return monoNetWorth(b,os,ls)-monoNetWorth(a,os,ls);}); setWinner(ranked[0]);setPhase("result");clearGameSave("monopoly");return true;} return false;}
    ...
    async function playTurn(){if(busy||phase!=="play"||pending)return;setBusy(true);const ps=players.map(function(p){return Object.assign({},p);}), os=Object.assign({},owners), lv=Object.assign({},levels), p=ps[turn];if(!p||p.bankrupt){setTurn(monoAdvance(ps,turn));setBusy(false);return;}const d1=1+Math.floor(Math.random()*6),d2=1+Math.floor(Math.random()*6),roll=d1+d2,extra=d1===d2,mv=monoMove(p.pos,roll);p.pos=mv.pos;if(mv.passed)p.cash+=200*mv.passed;const tile=MONO_BOARD[p.pos], base=p.name+" 掷出 "+d1+" + "+d2+" = "+roll+(extra?"（双骰）":"")+"，走到【"+tile.name+"】"+(mv.passed?"，经过起点领 $"+(200*mv.passed):"")+"。";const ls=[{type:"move",name:p.name,say:base}], eventParts=[];
