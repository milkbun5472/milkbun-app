...
function App() {
  ...
  const replyNow = async (charId, extraText, mode, opts) => {
    ...
    try {
      ...
      const _digitalTaskFull = ("\n\n【这只是手机通道，不是角色任务】" + selfTask + "普通聊天直接输出你真正想对用户说的纯文本即可：不需要 JSON，不规定几条、长短、语气或结构，也不用填写心情、好感、穿着、动作、心声等状态作业。App 会把你的原文正常显示出来。历史开头的〔今天14:32〕一类标记只告诉你消息时间，回复中不用照抄。只有当你本人确实决定让 App 执行某个能力时，才输出最小 JSON，并且只写需要的字段，例如 {\"word\":[\"你同时想说的话\"],\"call\":\"voice\"}；不用的字段全部省略，不要为了展示能力而调用。" + (ccToolOn ? ccToolHint + " 需要工具时最小写法：{\"word\":[\"我去查一下\"],\"ccTool\":{\"name\":\"工具名\",\"args\":{}}}。" : "") + "你也可以按自己的判断不回复；若要明确让 App 显示已读不回，才输出 {\"silent\":true}。协议只负责传递你的决定，不替你做决定。任意时候，真实表达都优先于填格式。  ").replace(/用户/g, uName);
      ...
      const _primer = _s.engineerEyes
        ? "\n\n【手机通道总纲】你就是上面的「" + char.name + "」本人，不是在扮演。直接和 " + uName + " 说你真正想说的话；普通聊天输出纯文本，不做结构化作业。只有你主动决定调用 App 能力时，才按本轮末尾的最小协议输出局部 JSON。"
