...
const LIVE_STATE_TTL = { wearing: 18 * 3600000, action: 45 * 60000, thought: 90 * 60000, place: 3 * 3600000, condition: 12 * 3600000 };
  ...
  const ctxFor = (char, ctxOpts) => ({
    ...
    // 常驻就是每轮白占 token 和注意力。理由到此为止，不再往外推。
    //
    // ⚠️别拿「上下文多会稀释活人感」当理由——那是错的，她当场抓到我记反了：
    //   v55.87 是【人设被砍到 200 字】才变模板（只剩「一个古代王爷」这一个标签，
    //   空白由训练先验补成霸总），把人设放宽到 6000 字正是【修好它】的那一步。
    //   后来还霸总是【站位】问题（模型被放在导演位上，v55.91 修的），跟上下文大小无关。
    //   该给的层就要给足——四处一样喂那条规矩没有被这次改动动摇；
    //   这里砍掉的只是「大多数轮次根本用不上的那一类」，两回事。
    //
    // 四条改动：
    //   ① 按需触发，不再常驻（她拍板）
