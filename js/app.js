    ...
    try {
      ...
      if (_askedRecord && typeof window.ledgerAddByChar === "function") {
        ...
        capState.push("ledger：她让你【替她记一笔账】时才填。"
          + (_recordLedgerChoices ? "currency 只能从这几个里挑：" + _recordLedgerChoices.currencies.join(" / ")
              + "；category 只能从这几个里挑——支出：" + _recordLedgerChoices.expense.join(" ") + "；收入：" + _recordLedgerChoices.income.join(" ") + "。"
            : "")
      ...
      const _digitalRecordHint = _askedRecord && (openCaps.includes("memo") || openCaps.includes("ledger"))
        ? "\n【本轮可用的真实记录字段】Lisa 这几条消息里明确请你替她记录。你本人决定照办时，必须在 JSON 里加入对应字段，不能只在 word 里说‘记好了’："
          + (openCaps.includes("memo")
            ...
            : "")
          + (openCaps.includes("ledger")
            ? "ledger:{\"type\":\"expense或income\",\"amount\":正数,\"currency\":\"币种\",\"category\":\"分类\",\"date\":\"YYYY-MM-DD或省略\",\"note\":\"缘由\"}。"
...
laterPromise:{"minutes":数字,"about":"回来要说/要做的事"}=【约回】——只有你这一轮【真的说了】「等我开完会再找你」「忙完这阵找你」「到家给你打电话」这类话时才填，minutes 是从现在起大约多久（开个会 60、忙一下午 240、下班后 480…），about 一句话写清回来是为了什么。没说过就【省略】，绝不许为了制造互动硬填。${_biRuleLine}`;
  ...
  const replyGroup = async (groupId, rgOpts = {}) => {
    ...
    try {
      ...
      // 上网（v58.74）：同思考链——群聊一次调用写完所有人，在场任一成员开着就带上工具。
      // 四处一样喂：单聊那条链（replyNow）也是按这个开关走的，别只写在一处。
      const _gWantWeb = members.some(c => {
        const _cs = settingsFor(c.id) || {};
  ...
  const savePhoneApp = (charId, key, d) => {
    ...
    // 提示词里已经说了「这一轮 visits 给空数组」，但那只是降概率。
  ...
  // ⚠️匿名信箱是【第七处】（她 2026-09-03：「还是语气不对为啥」）。
  // 病根和 v60.27 通话、v61.16 穿书那两次一字不差：这一层规则是【靠调用点一条条 push 的】，
  // 不是 buildBundle 白送的——所以换个入口就一条都没有，而且不会留下任何能 grep 的痕迹。
    ...
    try {
      ...
      const d = await runProbe(apiFor(char.id), ctxFor(char), {
        ...
        instruction: "你的匿名树洞(网名:" + nn + ")里来了 " + qs.length + " 条陌生人的提问,你一条条看下来。\n"
          ...
          + "· 不是每条都要盖章定性。可以反问回去(但别把人家的原话原样抛回来当问句,那是回声,见下)、只挑其中半句接、嗤一声、说点只沾一点边的事、也可以承认自己也不知道。\n"
  ...
  const openAnonBox = async char => {
    ...
    try {
      ...
      const d = await runProbe(apiFor(char.id), ctxFor(char), {
        ...
        voice: true,
        instruction: "你(网名:" + nn + ")打开了自己的匿名箱,里面积着 " + pend.length + " 条没看过的提问,全部来自【同一个匿名的人】"
          + (mask ? "(马甲:" + mask.name + (mask.bio ? ",签名「" + mask.bio + "」" : "") + ")" : "") + "。\n"
          + "【提问】\n" + list + "\n"
          + "逐条应对,顺序照上面来。你不知道这人是谁,但一口气看完这些会有自己的判断:"
          + "【不是每条都得答】——被问到痛处、问得太近、或者这人让你不舒服,可以只回半句、装没看见、顶回去;"
          + "真不想答的那条把 skip 置为 true,answer 留空,note 里写一句你当时的反应。\n"
          + "【别把自己答成一台判词机器】这是你在手机上一条条划过去随手打的字,不是在给人下结论:\n"
          + "· 长短要不一样——有的只值两三个字或一个『嗯』,有的你会难得多打两三行;绝不许每条都是同样长度、同样节奏的一句陈述句。\n"
          + "· 不是每条都要盖章定性。可以反问回去(但别把人家的原话原样抛回来当问句,那是回声,见下)、可以只挑其中半句接、可以嗤一声、可以说点跟问题只沾一点边的事、也可以承认自己也不知道。\n"
          + "· 你此刻的心情写在上面——烦的时候就短、就冲;松快的时候话会多一点。让它真的影响这几条,别每条都是同一个温度。\n"
          + "· 说人话:别端着解释道理、别写成箴言警句、别用『其实/本质上/无非是』这种给人上课的开场。\n"
          + "如果看完这些你心里冒出了「这人是不是我认识的某某」,把那个念头写进 guess(一句,可以猜错、可以猜得很离谱);没冒出来就留空。"
          + anonBans(),
        schemaHint: "{\"items\":[{\"answer\":\"回答(skip 为 true 时留空)\",\"skip\":false,\"note\":\"skip 为 true 时你当时的反应(一句,可空)\"}],\"guess\":\"你心里那句猜测(可空)\"}"
      });
  ...
  const askAnon = async (char, q) => {
    ...
    try {
      const d = await runProbe(apiFor(char.id), ctxFor(char), {
        voice: true,
        instruction: "有人匿名向你提问：「" + q + "」。你不知道问的其实是 " + ((profile && profile.name) || "用户") + "。\n"
          + "就当是手机上随手回一条匿名提问：想认真答就认真答，不想答就敷衍、顶回去或只接半句；"
          + "长短随你此刻的心情，别写成一句端着的判词。",
  ...
  if (!loaded) body = /*#__PURE__*/React.createElement(Empty, {
    ...
    anonBusy: anonBusy,
    onRefreshAnonPersona: refreshAnonPersona,
    onGenAnonQuestion: genNetizenQ,
    onAskAnon: askAnon,
