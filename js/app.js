...
function App() {
  ...
  const LIVE_STATE_TTL = { wearing: 18 * 3600000, action: 3 * 3600000, thought: 90 * 60000, place: 3 * 3600000 };
      ...
      if (photoScene && photoKind && typeof imgApiReady === "function" && imgApiReady() && (char.appearance || char.refPhoto)) {
        ...
        (async () => {
          try {
            ...
            const prompt = buildPhotoPrompt(char, (freshPlace ? "（此刻人在：" + freshPlace + "）" : "") + photoScene, st, { kind: photoKind, me, contRef: !!contBlobKey, contRefIndex: contBlobKey ? refs.length : 0 });
            const out = await generateSelfieImage(prompt, refs.length ? refs : null, { contRef: !!contBlobKey });
            // 合照锁脸降级要说出来,别让「两个陌生人」看起来像生成成功
            if (out && out.degraded) toast((out.degraded === "duo-single-ref" ? "只锁了 " + char.name + " 的脸" : "没用上参考照") + (out.refError ? "：" + out.refError : ""), 9000);
            if (out.blob) {
