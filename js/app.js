      ...
      if (photoScene && photoKind && typeof imgApiReady === "function" && imgApiReady() && (char.appearance || char.refPhoto)) {
        ...
        (async () => {
          try {
            ...
            // 经典直通模式:整条管线回滚到 2026-08-21 之前验证过锁脸的原样(老prompt+老请求形状)
            const classicImg = typeof loadImgApi === "function" && loadImgApi().classicMode === true;
            const prompt = (refs.length && !classicImg) ? buildReferencePhotoPrompt(char, sceneForPhoto, st, photoOpts) : buildPhotoPrompt(char, sceneForPhoto, st, photoOpts);
          ...
          if (gPhotoScene && gPhotoKind && typeof imgApiReady === "function" && imgApiReady() && (spk.appearance || spk.refPhoto)) {
            ...
            (async () => {
              try {
                ...
                const gClassicImg = typeof loadImgApi === "function" && loadImgApi().classicMode === true;
                const prompt = (refs.length && !gClassicImg) ? buildReferencePhotoPrompt(spk, gPhotoScene, st, gPhotoOpts) : buildPhotoPrompt(spk, gPhotoScene, st, gPhotoOpts);
