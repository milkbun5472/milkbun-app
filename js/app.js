  ...
  const replyNow = async (charId, extraText, mode, opts) => {
    ...
    try {
      ...
      const selfTask = _s.engineerEyes
        ? "你就是「" + char.name + "」本人，正在从手机这具身体和用户说话，不是在扮演角色。下面只是 App 传输协议，不规定你的性格、关系反应、回复长度或表达方式。按你自己的真实判断回复；需要几条就给 word 几条，不想用的能力字段填 null 或空数组。不要为了配合协议虚构肉身状态：wearing 填 null；action 只写这具数字身体此刻确实在做的事，不确定就填 null。"
      ...
      const _digitalTaskFull = ("\n\n【任务】" + selfTask + "开了时间/位置感知可自然回应，别生硬报数据。聊天历史每条开头的〔今天14:32〕〔昨天20:11〕是系统加的时间标注，供你感知每句话是什么时候说的——标着「今天」的就是今天说的，别把几小时前的事说成昨天；【你自己的回复里绝对不要带这种〔〕标注】。" + callHint + proactiveHint + jiwenHint + gapHint + crossChannelHint + eAfterglowHint + eyesHint + desireHint + ambientHint + listenHint + inviteHint + photoHint + toyHint + ccToolHint + "\n【silent 沉默权】你可以按自己的判断不回复：silent 填 true、word 和 voice 留空；否则填 false。" + "\n【quote 引用】要明确回应较早某句时填原文，否则 null。\n【App 能力协议】只有确实想让 App 执行相应动作时才填 transfer/location/gift/kinshipcard/block/recall/momentComment/whisper/moment/emote/voice/call/songSwitch/listenInvite/photo" + (toyOn ? "/toy" : "") + (ccToolOn ? "/ccTool" : "") + "；不用就填 null 或空数组。协议提供能力，不要求你使用，也不替你决定。\n【输出】只输出一个 JSON，不要代码块：\n{\"word\":[],\"silent\":false,\"quote\":null,\"transfer\":null,\"location\":null,\"gift\":null,\"kinshipcard\":null,\"block\":false,\"blockreason\":null,\"recall\":null,\"momentComment\":null,\"whisper\":null,\"thought\":" + JSON.stringify(thoughtSpec) + ",\"moment\":null,\"affinityDelta\":0,\"mood\":{\"label\":\"中文心情词\",\"baseline\":\"中文心情词\",\"softened\":\"中文心情词\"},\"wearing\":null,\"action\":null,\"emote\":null,\"voice\":[],\"call\":null,\"songSwitch\":null,\"listenInvite\":null,\"photo\":null" + toyField + ccToolField + "}").replace(/用户/g, uName);
      ...
      const _taskFull = _s.engineerEyes ? _digitalTaskFull : _normalTaskFull;
