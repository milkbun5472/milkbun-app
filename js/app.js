...
function App() {
  ...
  const genOfflineFrom = async (charId, workSess) => {
    ...
    try {
      ...
      const res = await generateOffline(offlineApiFor(charId), oCtx, { ...workSess, msgs: _timelineMsgs, hasOnlineInterlude: _onlineInterlude.length > 0, imageDataUrls: offImageDataUrls, priorSummary: workSess.summary || "", narr: osNarr(charId), taste: workSess.taste || osTaste(charId), maxTokens: osFor(charId).maxTokens, minWords: osFor(charId).minWords, toyOn: offToyOn, rerollAvoid: workSess.rerollAvoid || "" });
  ...
  const replyNow = async (charId, extraText, mode, opts) => {
    ...
    try {
      ...
      const digitalToyHint = toyOn
        ? "此刻配件已经由用户当场授权并连到她身上。你决定实际控制时，额外加入 \"toy\":{\"pattern\":\"teasing|steady|wave|pulse|edge\",\"intensity\":1到20,\"duration\":1到30,\"reason\":\"原因\"}；不用时省略。是否使用、何时使用、用什么节奏由你自己决定。"
        : "";
      ...
      // 普通角色的旧心情只是本轮起点，不是要求模型照抄的答案；每轮按最新互动重判。
      const moodUpdateHint = "\n【实时心情】mood.label 写你读完用户最新消息、作出本次回应后的心情。每轮重新判断，别因为上面给了旧心情就默认照抄：这句话真的让情绪变化就换成新的中文短词；没有真实变化才保持原词。";
...
能力字段只在本轮开放且角色实际决定触发时填写，未触发直接省略。历史中的〔今天14:32〕等标记只表示时间，不得写进 word。`;
      ...
      const _normalTaskV2 = ("\n\n【本轮】先以「" + char.name + "」本人此刻的真实反应回复上面的消息；聊天先发生，状态随后记录。" + paceHint + callHint + proactiveHint + jiwenHint + gapHint + crossChannelHint + eAfterglowHint + desireHint + capabilityHint).replace(/用户/g, uName);
      ...
      const system = _singleHistoryLayout ? (bundleStable + (_s.engineerEyes ? "" : _normalProtocolStable) + _primer) : (bundle + (_s.engineerEyes ? "" : _normalProtocolStable) + _taskFull);
      const g = [];
