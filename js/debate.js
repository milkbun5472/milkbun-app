...
(function () {
  ...
  // 人设是给了的，可人设说的是「TA是谁」，没说「TA不可能懂什么」——模型手上有一整套
  ...
  const WORLD_WORDS =
    "【各人只能用自己那个世界里有的东西讲道理】\n" +
    "这几位未必是同一个时代、同一个行当的人。判据一句话：**这个词、这件事，TA这辈子有没有可能听说过？**\n" +
    "· 听都没听说过的名词，一个都不许从TA嘴里出来——不是「少用」，是TA根本没有这个词。\n" +
    "· TA要说理，就用TA真见过、摸过、经手过的东西打比方：TA每天干的活、TA那个身份操心的事、TA那地方有的物件。\n" +
    "· 【不许全场一起现代化】：只要有一位是古代/异世界的人，其余人也不能默认「大家都懂现代那一套」，\n" +
    "  更不许由谁给TA补一课再往下吵。谁也不给谁当翻译。\n" +
    "· 听不懂对方在说什么，【那本身就是这场吵架的一部分】：可以嗤之以鼻、可以叫TA说人话、\n" +
    "  可以硬把话题拽回自己懂的那块地。反过来也一样，现代人不必迁就古人改说文言，TA就用TA平时的说法。\n" +
  ...
  function settleVotes(raw, voters, targets, hist) {
  ...
  async function genRound(active, meta, uName, worldbook, o) {
    ...
    const sys = AC() + CB() +
      ...
      ",\"focus\":{\"issue\":\"还没吵拢的那个分歧，1~2句\"}}。turns 顺序同上、每个角色一条" + (benchBlock ? "；side 至多两条，名字必须是场边名单里的；votes 场边每人一条" : "") + "。不要路人、不要昵称、不要弹幕；别加旁白别 markdown。";
    ...
    const votes = benchBlock ? settleVotes(p.votes, (o.bench || []).map(function (c) { return c.name; }), targets, hist) : [];
