...
(function () {
  ...
  // 场景桌不是另一套剧情账本,只是把守密人本来藏在正文里的「这一幕怎么玩」亮给玩家。
  // type 决定桌面语言;objective/stakes 都是玩家此刻已经知道的,绝不装秘典内容。
  const SCENE_TYPE_ZH = { investigate: "调查", social: "交涉", danger: "险境", travel: "赶路", interlude: "休整", explore: "探索", general: "场景" };
  const SCENE_TYPE_ALIAS = { investigation: "investigate", mystery: "investigate", talk: "social", negotiation: "social", combat: "danger", chase: "danger", rest: "interlude", camp: "interlude", exploration: "explore" };
  const APPROACH_ZH = { safe: "稳妥", bold: "冒险", ally: "队友", clever: "机巧", open: "自由" };
  function normSceneMeta(raw, camp) {
    const src = raw && raw.sceneMeta && typeof raw.sceneMeta === "object" ? raw.sceneMeta : (raw || {});
    let type = String(src.type || src.sceneType || "").trim().toLowerCase();
    type = SCENE_TYPE_ALIAS[type] || type;
    if (!SCENE_TYPE_ZH[type]) type = (camp && camp.sceneMeta && SCENE_TYPE_ZH[camp.sceneMeta.type]) ? camp.sceneMeta.type : "general";
    const cur = camp && camp.stages && camp.stages[camp.stageIdx || 0];
    const prev = camp && camp.sceneMeta || {};
    const objective = String(src.objective || src.sceneObjective || prev.objective || (cur && cur.goal) || "看看眼前会发生什么").trim().slice(0, 64);
  ...
  const BOND_START = 50, BOND_STEP = 5, BOND_HIGH = 70, BOND_LOW = 30;
  const bondVal = m => (m && m.key !== "user") ? (Number.isFinite(m.bond) ? m.bond : BOND_START) : null;
  const bondZh = v => v >= 85 ? "过命" : v >= BOND_HIGH ? "交底" : v > 55 ? "投缘" : v >= 45 ? "同行" : v > BOND_LOW ? "生分" : "离心";
  // 羁绊给的不是形容词,是【骰面上的差别】:处得好的队友替你出手时真的更卖力。
  const bondBoost = m => { const v = bondVal(m); return v == null ? 0 : v >= BOND_HIGH ? 5 : v <= BOND_LOW ? -5 : 0; };
  function normSiteActions(arr) {
    const seen = {};
    return (Array.isArray(arr) ? arr : []).map(x => typeof x === "string" ? x : (x && (x.text || x.label))).map(x => String(x || "").trim().slice(0, 24)).filter(x => x && !seen[x] && (seen[x] = true)).slice(0, 3);
  }
  ...
  function normChoices(arr, party) {
    return (Array.isArray(arr) ? arr : []).slice(0, 4).map(c => {
      if (typeof c === "string") return { text: c.trim(), check: null, need: null, approach: "open", risk: "", payoff: "" };
      ...
      // 队友手上的牌(她 2026-09-03 点的):物品早就带持有人了,但守密人从不主动让队友掏东西。
      // use 写明【谁掏什么】,点了就真的从那个人身上扣一件——不是只在正文里说说。
      let use = null;
      if (c.use && typeof c.use === "object") {
        const un = itemNameNorm(c.use.name);
        const uw = c.use.who ? findMember(party, c.use.who) : null;
        if (un) use = { name: un, who: uw ? uw.name : null };
      }
  ...
  function exploreMenu(camp) {
    ...
    return {
      ...
      siteActions: normSiteActions((camp.siteActions || {})[here]).filter(x => done.indexOf(x) < 0)
    };
  }
  ...
  // ---- 离线这几天(她 9/4 问「4 要是我一周不玩咋办」) ----
  // 隔了一天以上回来,世界没停:钟按真实天数走,但【至多两格、永远不走满】——
  // 走满是要爆发成事件的,那得她在场。一周不玩也只是两格,回来时压力更近了,不是天塌了。
  // 最紧的那座先走(filled 最高的),同一座走不到 max-1 就换下一座;哪座都走不了就不走。
  const AWAY_CLOCK_MAX = 2;
  const AWAY_DAY_MAX = 7;    // 团内时间最多跟着跳 7 天,再久也只算一周
  const awayClocks = (clocks, days) => {
    const out = (clocks || []).map(c2 => Object.assign({}, c2));
    const moved = [];
    let left = Math.min(AWAY_CLOCK_MAX, Math.max(0, Math.floor(days)));
    while (left > 0) {
      const cand = out.filter(c2 => c2.filled < c2.max - 1).sort((a, b) => b.filled - a.filled)[0];
      if (!cand) break;
      const from = cand.filled;
      cand.filled += 1;
      const rec = moved.find(m => m.name === cand.name);
      if (rec) rec.to = cand.filled; else moved.push({ name: cand.name, from, to: cand.filled, max: cand.max });
      left -= 1;
    }
    return { clocks: out, moved };
  };
  // 行动表的一行:名字(字符串)或 {who, act, stat};只认队伍成员,同一人只留一行,
  // stat 不合法就当不用过检定;玩家那一行永远没有 stat——她那一下由她自己选
  function normOrder(raw, party) {
    const seen = {};
    return (Array.isArray(raw) ? raw : []).map(x => {
      const who = typeof x === "string" ? x : x && typeof x === "object" ? x.who || x.name : "";
      const m = who ? findMember(party, who) : null;
      if (!m || seen[m.name]) return null;
      seen[m.name] = true;
      const act = typeof x === "object" && x ? String(x.act || "").trim().slice(0, 24) : "";
      const stat = typeof x === "object" && x && STAT_ZH[x.stat] ? x.stat : null;
      return { who: m.name, act, stat: m.key === "user" ? null : stat };
  ...
  function applyTurnPayload(camp, p, opts) {
    ...
    const notes = [];
    const chips = [];
    next.sceneMeta = normSceneMeta(p, camp);
    if (opts && SCENE_TYPE_ZH[opts.sceneType]) next.sceneMeta.type = opts.sceneType;
    next.sceneTrail = (camp.sceneTrail || []).concat([next.sceneMeta.type]).slice(-5);
    ...
    // 这一拍她点的选项写明了「谁掏什么」:真的从那个人身上扣一件(扣不着就算了,
    // 守密人那侧的规则会让硬来付代价)。摆在 lose 后面,顺序不重要,写在这儿是为了看得见。
    const used = opts && opts.useItem;
    if (used && used.name) {
      const uw = used.who ? findMember(next.party, used.who) : null;
      let i = uw ? items.findIndex(it => it.name === used.name && it.holder === uw.name) : -1;
      if (i < 0) i = items.findIndex(it => it.name === used.name);
      if (i >= 0) {
        const holder = items[i].holder;
        if (items[i].n > 1) items[i].n -= 1; else items.splice(i, 1);
        notes.push((holder !== "队伍" ? holder + "掏出" : "用掉") + "「" + used.name + "」");
        chips.push({ k: "lose", txt: "🃏 " + (holder !== "队伍" ? holder + "掏出·" : "用·") + used.name });
      }
    }
    next.items = items;
    ...
    const away = opts && opts.away && opts.away.days > 0 ? opts.away : null;
    const walked = away ? awayClocks(camp.clocks, away.days) : null;
    const clocks = walked ? walked.clocks : (camp.clocks || []).map(c2 => Object.assign({}, c2));
    if (walked) walked.moved.forEach(mv => { notes.push("⏰" + mv.name + " " + mv.to + "/" + mv.max + "(这几天)"); chips.push({ k: "clue", txt: "⏰ " + mv.name + " " + mv.to + "/" + mv.max + "·这几天" }); });
    (Array.isArray(away ? null : p.clock) ? p.clock : []).forEach(row => {
      if (!row || typeof row !== "object") return;
    ...
    // 险境行动表(她 9/4 说跑团单调;原来只是「简化先攻」一枚角标):危险/交战拍守密人给每人一行
    // {who, act, stat}——谁、这一拍要做的那一下、要过哪项检定。存成 table,她一动整桌一起结算:
    // 队友各掷各的骰(pickChoice 里自动掷),她那一行走她自己的选项。别的拍一律撤桌
    const table = normOrder(p.order, next.party);
    next.table = table.length >= 2 ? table : null;
    if (table.length >= 2) chips.push({ k: "lose", txt: "⚔ 顺序:" + table.map(r => r.who).join("→") });
    ...
    (Array.isArray(p.npc) ? p.npc : []).forEach(row => {
      ...
      // 人情账(她 2026-09-03 点的):名册原来只有身份/立场/生死,是一本通讯录;
      // 加上「谁欠谁的」它才是一张筹码表——欠着的可以被催,也可以拿去换。
      const dbt = String(row.debt || "").trim().toLowerCase();
      if (dbt === "clear") {
        if (n.debt) { delete n.debt; notes.push("与「" + n.name + "」两清"); chips.push({ k: "hpup", txt: "🤝 " + n.name + "·两清" }); }
      } else if (dbt === "owe" || dbt === "owed") {
        const dn = String(row.debtNote || "").trim().slice(0, 24);
        const was = n.debt && n.debt.side;
        n.debt = { side: dbt, note: dn };
        if (was !== dbt) {
          const zh = dbt === "owe" ? "你欠" + n.name : n.name + "欠你";
          notes.push(zh + (dn ? "·" + dn : ""));
          chips.push({ k: "clue", txt: "🤝 " + zh + (dn ? "·" + dn : "") });
        }
      }
    });
    next.npcs = npcs;
    // 羁绊:只从【真的发生过的一件事】长出来——why 必填,不填就整条丢掉
    // (不然模型会每拍随手 +1,那不是关系,是计数器)。玩家自己头上不挂羁绊。
    const bondSum = {};
    (Array.isArray(p.bond) ? p.bond : []).forEach(row => {
      if (!row || typeof row !== "object") return;
      const m = findMember(next.party, row.name);
      const d = Math.round(Number(row.delta) || 0);
      const why = String(row.why || "").trim().slice(0, 24);
      if (!m || m.key === "user" || !d || !why) return;
    ...
    // 那是记账,不是相处
    Object.keys(bondSum).slice(2).forEach(k => delete bondSum[k]);
    next.party.forEach(m => {
      const row = bondSum[m.name];
      if (!row) return;
      const step = Math.max(-2, Math.min(2, row.d));
      if (!step) return;
      const before = bondVal(m);
      m.bond = Math.max(0, Math.min(100, before + step * BOND_STEP));
      if (m.bond === before) return;
      m.bondLog = (m.bondLog || []).concat([{ why: row.why, d: step, ts: Date.now() }]).slice(-3);
      notes.push(m.name + " 羁绊" + (step > 0 ? "+" : "") + (m.bond - before) + "(" + row.why + ")");
      chips.push({ k: step > 0 ? "hpup" : "hp", txt: "🔗 " + m.name + (step > 0 ? "+" : "") + (m.bond - before) + " · " + row.why });
    });
    ...
    if (away) {
      const cur0 = camp.time || { day: 1, part: "晨" };
      next.time = { day: cur0.day + Math.min(AWAY_DAY_MAX, Math.max(1, Math.floor(away.days))), part: cur0.part };
      chips.push({ k: "lose", txt: "🕯 第" + next.time.day + "日·" + next.time.part + "·隔了" + Math.floor(away.days) + "天" });
    }
    if (p.time && typeof p.time === "object") {
      const PARTS = ["晨", "午", "暮", "夜"];
      const cur = (away ? next.time : camp.time) || { day: 1, part: "晨" };
    ...
    // 每个地点自己的三件可做之事只在第一次抵达/第一次拿到时落桌;之后守密人即使
    // 换措辞也不覆盖,免得玩家刚准备点,下一拍按钮全变了。点过的动作单独记 done。
    const hereAfter = next.pos || next.place || "";
    const site = normSiteActions(p.siteActions);
    next.siteActions = Object.assign({}, camp.siteActions || {});
    if (hereAfter && site.length && !normSiteActions(next.siteActions[hereAfter]).length) next.siteActions[hereAfter] = site;
    next.siteDone = Object.assign({}, camp.siteDone || {});
    if (opts && opts.siteAction && hereAfter) {
      const oldDone = Array.isArray(next.siteDone[hereAfter]) ? next.siteDone[hereAfter].slice() : [];
      if (oldDone.indexOf(opts.siteAction) < 0) oldDone.push(opts.siteAction);
    ...
    if (opts && opts.rest) {
      const seen = {};
      const threads = (Array.isArray(p.night) ? p.night : []).map(row => {
        if (!row || typeof row !== "object") return null;
        const m = findMember(next.party, row.who);
        const open = String(row.open || row.text || "").trim().slice(0, 60);
        if (!m || m.key === "user" || m.hp <= 0 || !open || seen[m.name]) return null;
        seen[m.name] = true;
        return { who: m.name, open, taken: false };
      }).filter(Boolean).slice(0, 4);
      next.night = threads.length ? { threads, at: (camp.msgs || []).length } : null;
    } else if (opts && opts.night) {
      next.night = camp.night ? Object.assign({}, camp.night, { threads: camp.night.threads.map(th => th.who === opts.night ? Object.assign({}, th, { taken: true }) : th) }) : null;
    } else if (camp.night) next.night = null;
    ...
    // 分头行动(她 9/4 说跑团单调):另一路回来只带一份【他们口里的说法】(report),
    // 真相(truth)守密人自留、她落幕才看得到——照私念该瞒的瞒。没写 report 就当他们什么都没说
    let apart = null;
    if (opts && opts.split && p.apart && typeof p.apart === "object") {
      const report = String(p.apart.report || "").trim().slice(0, 400);
      const truth = String(p.apart.truth || "").trim().slice(0, 600);
      if (report) { apart = { who: opts.split.who.slice(), report, truth, hidden: !!p.apart.hidden && !!truth }; chips.push({ k: "clue", txt: "⑂ " + opts.split.who.join("、") + "回来了" }); }
    }
    // 捎来的信:只有名册里有人情账(欠/被欠)、还活着的那位才写得出来;别人写的一律丢
    let letter = null;
    if (away && p.letter && typeof p.letter === "object") {
      const from = String(p.letter.from || "").trim();
      const text = String(p.letter.text || "").trim().slice(0, 200);
      const n = from ? (next.npcs || []).find(x => x.name === from || x.name.indexOf(from) >= 0 || from.indexOf(x.name) >= 0) : null;
      if (n && n.alive !== false && n.debt && text) { letter = { from: n.name, text }; chips.push({ k: "clue", txt: "✉ " + n.name + "捎来一封信" }); }
    }
    ...
    return { camp: next, chips, sysLine: notes.join(" · "), gate: gateOut || null, letter, apart };
  }
  ...
  function foldHist(msgs) {
    ...
    (msgs || []).forEach(m => {
      ...
      const content = m.role === "chat"
        ...
        : m.role === "letter" ? "〔" + m.from + " 捎来的信〕" + m.content
        : m.role === "apart" ? "〔分头·" + (m.who || []).join("、") + "那边真正发生的(玩家只听到了他们的说法)〕" + (m.truth || "(未记)") + "\n〔他们回来对玩家说的〕" + m.content
  ...
  function TrpgApp(props) {
    ...
    const [splitOpen, setSplitOpen] = useState(false); // 分头行动:挑谁走另一路、去办什么
    const [splitWho, setSplitWho] = useState([]);
    const [splitTask, setSplitTask] = useState("");
    const [dice, setDice] = useState(false);     // 剧情骰:下一回合注入一个意外,一次性
    const [plusOpen, setPlusOpen] = useState(false);
    const [handOpen, setHandOpen] = useState(false); // 玩家手牌:把物品/线索/队友主动带进下一次行动
    const [played, setPlayed] = useState({ item: null, clue: null, ally: null });
    ...
    const [lullDue, setLullDue] = useState(false); // 翻章之后欠着一拍幕间(见 confirmStage)
    ...
    useEffect(() => { setHandOpen(false); setPlayed({ item: null, clue: null, ally: null }); }, [playId]);
    // 这一夜过去了(队伍动身,话头被清掉)——夜谈模式跟着退出,别把下一句白天的话还当成夜里说的
    useEffect(() => { if (nightWith && !(camp && camp.night)) setNightWith(null); }, [camp && camp.night]);
    ...
    const partyBlock = c => c.party.map(m => {
      ...
      const line = m.name + ":HP " + m.hp + "/" + (m.maxHp || 100) + " · " + STATS.map(([k, zh]) => zh + m.stats[k]).join(" ")
        ...
        + (bv == null ? "" : " · 羁绊" + bv + "(" + bondZh(bv) + ")" + ((m.bondLog || []).length ? "·记着:" + m.bondLog.slice(-2).map(x => x.why).join("、") : ""))
    ...
    const acceptDraft = () => {
      const firstScene = normSceneMeta(draft.sceneMeta || {}, { stages: draft.stages, stageIdx: 0 });
    ...
    // 羁绊不是形容词,是骰面上的差别:处得好的队友替你出手更卖力,处僵了的没怎么使劲。
    // 加成全在这一个口子里算,别处只显示。
    const ceremonyEff = c => Math.min(95, c.base + (c.feat ? 15 : 0) + (c.assist ? 10 : 0) + (c.bargainOn ? 15 : 0) + (c.bond || 0));
    const runCeremony = (member, chk) => new Promise(resolve => {
      ...
      setCeremony({ mKey: member.key, who: member.name, statKey: chk.stat, statZh: STAT_ZH[chk.stat], base: member.stats[chk.stat], feat: feat ? feat.name : null, assist: null, mates, bond: bondBoost(member), bargainText: chk.bargain || null, bargainOn: false, vs: chk.vs || null, harm: !!chk.harm, fate: member.fate || 0, phase: "ready", roll: 0, grade: null, vsRoll: null, vsGrade: null, opposed: null, harmRoll: null, rerolled: false, spent: [], resolve });
    });
    const ceremonyFinish = () => setCeremony(c => {
      if (c && c.resolve) c.resolve({ roll: c.roll, rolls: c.rolls || [c.roll], grade: c.grade, effVal: ceremonyEff(c), feat: c.feat, assist: c.assist, bond: c.bond || 0, bargainOn: c.bargainOn, bargainText: c.bargainText, vs: c.vs, vsRoll: c.vsRoll, vsGrade: c.vsGrade, opposed: c.opposed, harmRoll: c.harmRoll, spent: c.spent });
      return null;
    });
    ...
    const autoRoll = (m, stat) => {
      const roll = 1 + Math.floor(Math.random() * 100);
      const bond = bondBoost(m);
      const effVal = Math.max(1, Math.min(95, ((m.stats && m.stats[stat]) || 1) + bond));
      return { roll, rolls: [roll], grade: gradeCheck(roll, effVal), effVal, feat: null, assist: null, bond, bargainOn: false, bargainText: null, vs: null, vsRoll: null, vsGrade: null, opposed: null, harmRoll: null, spent: [] };
    };
    const rollLine = (fated, m, chk, res) => {
      const boost = [res.feat ? "专长·" + res.feat + "+15" : null, res.assist ? res.assist.name + "协力+10" : null, res.bargainOn ? "魔鬼交易+15" : null,
        res.bond > 0 ? "羁绊+5" : res.bond < 0 ? "离心-5" : null].filter(Boolean);
    ...
    const gmSys = c => {
      ...
      return [narrativeCore(),
        ...
        ABILITY_RULE,
        "【羁绊·队友这一路怎么看你】每位队友有一个只属于这场团的羁绊值(0-100,见下方状态表)。它【必须影响你怎么演这个人】:\n"
          + "· " + BOND_HIGH + " 以上=交底:会替 " + uName + " 冒险、当众替 Ta 说话、把自己的私念主动摊开、在选项里主动请缨;\n"
          + "· 45-" + BOND_HIGH + "=同行:公事公办,该出力出力,但心里那句话不说;\n"
    ...
    const turn = async (declaration, extra, mode) => {
      ...
      try {
        ...
        // 松紧节奏(她 9/1 提的:探索得有戏里的间歇,不能被戴夫盘问到一半按暂停跑去吃雪糕):
        // 连着三拍都在紧(掷过骰/掉血/钟在走),这一拍守密人必须写一个喘气的间歇——
        // 危机暂时甩开、到了能落脚的地方,选项全是探索,不开新危机、不要骰子。
        // 间歇拍记 lull,连拍数从它之后重数
        const tenseStreak = (() => {
          let n = 0;
          for (let i = liveMsgs.length - 1; i >= 0; i--) {
            ...
            if (m.lull) break;
            const prev = liveMsgs.slice(0, i).reverse().find(x => x.role === "gm" || x.role === "roll");
            const tense = (prev && prev.role === "roll") || (m.chips || []).some(ch => /HP-|⏰/.test(ch.txt || ""));
            if (!tense) break;
            n++;
          }
          return n;
        })();
        // 「打出一张牌」只是给这一拍带了个附注,它仍旧是普通拍——别让它顺手吃掉喘气拍
        const specialMode = mode && (typeof mode === "string" || mode.talk || mode.night || mode.pov || mode.away || mode.split || mode.travel || mode.explore || mode.seed);
        const wantLull = !specialMode && !dice && tenseStreak >= 3 && !camp.ended;
        ...
        update(list => list.map(c => {
          ...
          const forcedScene = (mode === "rest" || mode === "lull" || (mode && (mode.night || mode.away))) ? "interlude" : mode && mode.talk ? "social" : mode && mode.travel ? "travel" : exploring ? "explore" : null;
          const r = applyTurnPayload(c, p, { travelTo: mode && mode.travel, nodes: nodesOf(c), roll: (extra || []).filter(x => x && x.role === "roll").slice(-1)[0] || null, calm: mode === "rest" || mode === "lull" || !!(mode && (mode.talk || mode.night || mode.away)) || (exploring && !seed), away: mode && mode.away ? { days: mode.away } : null, split: (mode && mode.split) || null, explore: mode && mode.explore, rest: mode === "rest", night: (mode && mode.night) || null, pov: (mode && mode.pov) || null, povPick: (mode && mode.povPick) || null, siteAction: siteAction || null, sceneType: forcedScene, useItem: (mode && mode.use) || null });
          const nc = r.camp;
          // 数值角标钉在这一拍的正文上(chips),不再另发一条居中系统行;
          // 旧存档里已有的 sys 行仍照常渲染
          const msgs = c.msgs.concat([{ id: rid("rm_"), role: "gm", content: p.scene, ts: Date.now(), sceneType: nc.sceneMeta.type, pov: nc.pov || undefined, chips: r.chips.length ? r.chips : undefined, snap: { hp: nc.party.reduce((m, x) => (m[x.name] = x.hp, m), {}), fate: nc.party.reduce((m, x) => (m[x.name] = x.fate, m), {}), items: nc.items, clues: nc.clues, stageIdx: nc.stageIdx, place: nc.place, pos: nc.pos || "", visited: (nc.visited || []).slice(), gauge: nc.gauge ? nc.gauge.val : null, clocks: (nc.clocks || []).map(x => Object.assign({}, x)), quests: (nc.quests || []).map(x => Object.assign({}, x)), seeds: (nc.sideSeeds || []).map(x => Object.assign({}, x)), npcs: (nc.npcs || []).map(x => Object.assign({}, x)), time: nc.time ? Object.assign({}, nc.time) : null, effects: nc.party.reduce((m, x) => (m[x.name] = (x.effects || []).map(e => Object.assign({}, e)), m), {}), choices: nc.choices, sceneMeta: nc.sceneMeta, sceneTrail: (nc.sceneTrail || []).slice(), siteActions: Object.assign({}, nc.siteActions || {}), siteDone: Object.assign({}, nc.siteDone || {}) } }]);
          ...
          // 捎来的信单独一张纸,不混进正文——它是那个人写的,不是守密人说的
          if (r.letter) msgs.push({ id: rid("rm_"), role: "letter", from: r.letter.from, content: r.letter.text, ts: Date.now() });
          // 另一路回来的说法单独一张卡;真相跟着存,落幕才亮
          if (r.apart) msgs.push({ id: rid("rm_"), role: "apart", who: r.apart.who, content: r.apart.report, truth: r.apart.truth, hidden: r.apart.hidden, ts: Date.now() });
    ...
    const playedCue = () => {
      const bits = [played.item ? "物品「" + played.item + "」" : null, played.clue ? "线索「" + played.clue + "」" : null, played.ally ? "请「" + played.ally + "」主导或协力" : null].filter(Boolean);
      return bits.length ? "〔主动调用〕" + bits.join(" + ") : "";
    };
    const clearPlayed = () => { setPlayed({ item: null, clue: null, ally: null }); setHandOpen(false); };
    const withPlayed = text => {
      const cue = playedCue();
      return cue ? cue + "\n" + text : text;
    };
    const consumePlayedTail = () => {
      const cue = playedCue();
      clearPlayed();
      return cue ? "\n" + cue : "";
    };
    const pickChoice = async (c, force) => {
      ...
      if (c.need && !hasItem(camp.items, c.need)) return turn(c.text + "(没有「" + c.need + "」,硬闯)" + consumePlayedTail());
      ...
      const tableRolls = (camp.table || []).filter(r => r.stat).map(r => {
        const m = findMember(camp.party, r.who);
        if (!m || m.key === "user" || m.hp <= 0) return null;
        const res = autoRoll(m, r.stat);
        return Object.assign({ id: rid("rm_"), role: "roll", content: (r.act ? "〔" + r.act + "〕" : "") + rollLine("", m, { stat: r.stat }, res), ts: Date.now() }, rollRec(m, { stat: r.stat }, res));
      }).filter(Boolean);
      if (!c.check) { const txt = withPlayed(c.text) + useTail; clearPlayed(); return turn(txt, tableRolls.length ? tableRolls : null, mixMode || (nightWith ? { night: nightWith } : null)); }
      ...
      const txt = withPlayed(c.text) + useTail; clearPlayed();
      turn(txt, tableRolls.concat([Object.assign({ id: rid("rm_"), role: "roll", content: line, ts: Date.now() }, rollRec(m, c.check, res))]), mixMode);
    };
    const send = () => { const text = input.trim(); if (!text) return; setInput(""); if (chatMode) { clearPlayed(); return addBeat(text); } const ready = withPlayed(text); clearPlayed(); if (talkNpc) return turn(ready, null, { talk: talkNpc }); if (nightWith) return turn(ready, null, { night: nightWith }); turn(ready); };
    ...
    // 翻章之后自动来一拍幕间(她 2026-09-03 点的)。⚠不能在 confirmStage 里直接调 turn:
    // 那一刻闭包里的 camp 还是旧的(stageIdx 没加),幕间会被当成上一章的戏来写。
    // 所以只挂一个待办(lullDue,和别的状态一起声明在上头),等 React 把新的 camp
    // 渲染出来,effect 里再开这一拍。
    useEffect(() => {
      if (!lullDue || !camp || busy || camp.ended) return;
      setLullDue(false);
      turn("(这一章就此翻过——动身之前,队伍先松一口气)", null, "lull");
    }, [lullDue, camp && camp.stageIdx, busy]);
    // 否决要留回执(Codex 抓的:以前点「还没有」只清横幅,守密人不知道被否过,
    // 下一拍可能又报完成):否决写成一条幕后事实进史,守密人下一拍就看得见
    const confirmStage = (ok, withLull) => (withLull && ok && setLullDue(true), update(list => list.map(c => c.id !== camp.id ? c : ok
    ...
    const branchFrom = msg => {
      ...
      const nc = Object.assign({}, camp, {
        ...
        sceneMeta: snap.sceneMeta ? Object.assign({}, snap.sceneMeta) : normSceneMeta({}, camp),
    ...
    if (view === "play" && camp) {
      ...
      const panel = panelOpen && h("div", null,
        ...
        camp.party.map(m => h("div", { key: m.key, style: { marginBottom: 6 } },
          ...
          bondVal(m) != null ? (() => {
            ...
            return h("div", { style: { marginTop: 3 } },
              h("div", { style: { display: "flex", alignItems: "center", gap: 5, fontFamily: F_BODY, fontSize: 10, color: col } },
                h("span", null, "🔗 " + bondZh(bv)),
                h("div", { style: { flex: 1, height: 2, background: t.line, borderRadius: 2, overflow: "hidden" } },
                  h("div", { style: { width: bv + "%", height: "100%", background: col } })),
                hi ? h("span", null, "替你出手 +5") : lo ? h("span", null, "没怎么使劲 −5") : null),
              (m.bondLog || []).length ? h("div", { style: { fontFamily: F_BODY, fontSize: 9.5, color: t.fog, marginTop: 1 } }, "记着:" + m.bondLog.slice(-2).map(x => x.why).join(" · ")) : null);
          })() : null,
        ...
        (camp.guesses || []).map(g => h("div", { key: g.id, style: { marginBottom: 5, padding: "6px 8px", borderRadius: 8, border: "1px dashed " + t.line } },
          ...
          h("div", { style: { display: "flex", alignItems: "center", gap: 8, marginTop: 3 } },
            ...
            !camp.ended ? h("button", { onClick: () => { setPanelOpen(false); turn("〔验证推测〕我拿着现有线索去验证:「" + g.text + "」"); }, disabled: busy, style: { fontFamily: F_BODY, fontSize: 10, color: "#5a7d5a", background: "none", border: "1px solid #5a7d5a55", borderRadius: 8, padding: "2px 8px" } }, "拿去验证") : null,
      ...
      const resume = (!camp.ended && lastTs && Date.now() - lastTs > 12 * 3600 * 1000 && resumeSeen !== camp.id) ? (() => {
        ...
        // 离线这几天:隔了一天以上、而且这个世界有东西会动(钟或人情账),才递这一张
        const awayDays = Math.floor((Date.now() - lastTs) / 86400000);
        const worldMoves = (camp.clocks || []).length || (camp.npcs || []).some(n => n.alive !== false && n.debt);
        const offerAway = awayDays >= 1 && worldMoves && camp.awayAck !== lastTs;
        const ackAway = () => update(list => list.map(c => c.id !== camp.id ? c : Object.assign({}, c, { awayAck: lastTs })));
        return h("div", { style: Object.assign({}, S.card, { margin: "8px 14px", borderStyle: "dashed" }) },
          ...
          offerAway ? h("div", { style: { fontFamily: F_BODY, fontSize: 11, color: t.sub, marginTop: 4 } }, "你走了 " + awayDays + " 天,这个世界没停——" + ((camp.clocks || []).length ? "钟会跟着走(最多两格,不会走满)" : "有人可能捎信来") + "。") : null,
          h("div", { style: { display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" } },
            offerAway ? h("button", { onClick: () => { setResumeSeen(camp.id); ackAway(); turn("(隔了 " + awayDays + " 天回来)", null, { away: awayDays }); }, disabled: busy, style: S.btn(true) }, "看看这几天发生了什么") : null,
            h("button", { onClick: () => { setResumeSeen(camp.id); if (offerAway) ackAway(); }, style: S.btn(!offerAway) }, offerAway ? "当没发生过,接上" : "接上,继续")));
      })() : null;
      ...
      const banner = endAsk ? h("div", { style: Object.assign({}, S.card, { margin: "8px 14px", borderColor: t.ink }) },
        ...
        : camp.pendingStage ? h("div", { style: Object.assign({}, S.card, { margin: "8px 14px", borderColor: t.ink }) },
          ...
          h("div", { style: { display: "flex", gap: 8, marginTop: 8 } },
            h("button", { onClick: () => confirmStage(true, true), style: S.btn(true) }, "翻过这一章 · 歇一段"),
            h("button", { onClick: () => confirmStage(true, false), style: S.btn(false) }, "翻过·直接接着演"),
      ...
      const flow = camp.msgs.map(m => m.role === "photo"
        ...
        ? h("div", { key: m.id, style: { margin: "10px 14px", padding: "10px 13px", borderRadius: 14, border: "1px dashed " + t.line, background: "rgba(255,255,255,.4)" } },
            ...
            camp.ended && m.truth ? h("div", { style: { marginTop: 8, paddingTop: 8, borderTop: "1px dashed " + t.line } },
              h("div", { style: { fontFamily: F_BODY, fontSize: 10, letterSpacing: 2, color: m.hidden ? "#a4442e" : t.fog, marginBottom: 4 } }, m.hidden ? "那边真正发生的(他们没全说)" : "那边真正发生的(他们说的是实话)"),
      ...
      const sceneMeta = normSceneMeta(camp.sceneMeta || {}, camp);
      const liveClock = (camp.clocks || []).slice().sort((a, b) => (b.filled / b.max) - (a.filled / a.max))[0];
      const sceneTone = { investigate: "#7b6949", social: "#6c728c", danger: "#a4442e", travel: "#557269", interlude: "#8a6d3b", explore: "#5a7d5a", general: t.sub }[sceneMeta.type] || t.sub;
      const sceneRibbon = h("div", { style: { flexShrink: 0, margin: "5px 14px 3px", padding: "8px 10px", borderRadius: 12, background: "rgba(255,255,255,.43)", border: "1px solid " + t.line, boxShadow: "0 2px 7px rgba(46,38,29,.045)" } },
        h("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
          h("span", { style: { flexShrink: 0, fontFamily: F_BODY, fontSize: 9.5, letterSpacing: 2, color: sceneTone, border: "1px solid " + sceneTone + "66", borderRadius: 999, padding: "2px 7px" } }, SCENE_TYPE_ZH[sceneMeta.type] || "场景"),
        ...
        : h("div", { style: { borderTop: "1px solid " + t.line, background: "rgba(255,255,255,.36)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)" } }, [
            ...
            camp.choices.map((c, i) => {
              ...
              return h("button", { key: i, onClick: () => pickChoice(c), disabled: busy, style: { flex: "0 0 min(74vw, 286px)", scrollSnapAlign: "start", textAlign: "left", padding: "9px 11px", borderRadius: 12, border: "1px solid " + tone + "66", borderLeft: "3px solid " + tone, background: "rgba(255,255,255,.54)", color: t.ink, opacity: lacking ? 0.72 : 1, boxShadow: "0 2px 7px rgba(46,38,29,.045)" } },
                ...
                (c.payoff || c.risk || c.need || c.use) ? h("div", { style: { marginTop: 5, fontFamily: F_BODY, fontSize: 9.8, lineHeight: 1.45, color: t.sub } },
                  ...
                  c.risk ? h("span", { style: { color: c.approach === "bold" ? "#a4442e" : t.sub } }, "险 · " + c.risk) : null,
          ...
          : stuck ? h("div", { key: "ch", style: { display: "flex", flexWrap: "wrap", gap: 7, padding: "8px 14px 0" } }, (() => {
              ...
              return [
                ...
                ex.siteActions.map(act => h("button", { key: "exa" + act, onClick: () => { const sd = Math.random() < EXPLORE_ODDS.search ? pickSeed(camp, regionOfNode(camp, ex.here)) : null; turn("(在「" + ex.here + "」" + act + ")", null, { explore: "site", siteAction: act, seed: sd }); }, disabled: busy, style: Object.assign({}, S.btn(false), { borderColor: "#7b694977" }) }, "◇ " + act)),
                ...
                (() => { const mates = camp.party.filter(x => x.key !== "user" && x.hp > 0); if (!mates.length) return null; const mt = mates[camp.msgs.length % mates.length]; return h("button", { key: "exp", onClick: () => turn("(镜头转到" + mt.name + "那边)", null, { pov: mt.name }), disabled: busy, style: S.btn(false) }, "👁 " + mt.name + "的一拍"); })(),
          ...
          handOpen ? h("div", { key: "hand", style: { maxHeight: 164, overflowY: "auto", padding: "8px 14px 2px", borderBottom: "1px dashed " + t.line } },
            ...
            [["item", "物品", itemsFix(camp.items).map(x => x.name)], ["clue", "线索", (camp.clues || []).slice(-8)], ["ally", "队友", camp.party.filter(x => x.key !== "user" && x.hp > 0).map(x => x.name)]].map(row => {
              const vals = row[2];
              if (!vals.length) return null;
              return h("div", { key: row[0], style: { display: "flex", alignItems: "center", gap: 5, marginBottom: 5, minWidth: 0 } },
          ...
          splitOpen ? h("div", { key: "sp", style: { padding: "8px 14px 0" } },
            ...
            h("div", { style: { display: "flex", gap: 8 } },
              ...
              h("button", { disabled: busy || !splitWho.length, onClick: () => {
                ...
                const rolls = who.map(n => { const m = findMember(camp.party, n); const stat = STATS.map(x => x[0]).sort((a, b) => ((m.stats || {})[b] || 0) - ((m.stats || {})[a] || 0))[0]; const res = autoRoll(m, stat); return Object.assign({ id: rid("rm_"), role: "roll", content: "〔分头·" + m.name + "用最拿手的「" + STAT_ZH[stat] + "」〕" + rollLine("", m, { stat }, res), ts: Date.now() }, rollRec(m, { stat }, res)); });
                const task = splitTask.trim().slice(0, 40);
