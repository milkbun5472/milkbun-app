...
//
// 这一页在现实里是什么？——**一本月事手账**。所以不是一列表单控件：
//   上面一张「这一轮」的卡（今天是第几天、还有几天），中间是今天这一页可以记的几栏，
//   底下是过往那几次的流水。
// ⚠️流量和痛感【不做成一排药丸】（tabs-not-plain-pills.md）：
//   流量是几滴水，痛感是一道由细到粗的横杠——形状本身就在说这一栏是什么。
const PERIOD_FLOW = [
  { k: "spot", zh: "点滴", n: 1 },
  { k: "light", zh: "少", n: 2 },
  { k: "mid", zh: "中", n: 3 },
...
const PERIOD_SYM = ["腰酸", "头疼", "小腹坠", "胸胀", "长痘", "水肿", "犯困", "没胃口", "很想吃", "拉肚子", "便秘", "睡不着", "怕冷", "出汗"];
const PERIOD_MOOD = ["还行", "低落", "烦躁", "想哭", "没力气", "焦虑", "特别黏人"];
function periodLogsOf(period) { const l = period && period.logs; return (l && typeof l === "object") ? l : {}; }
...
function periodDayOf(period, key) {
  ...
  for (let i = list.length - 1; i >= 0; i--) {
    ...
    if (d >= sd && d <= ed) return { p: list[i], day: Math.round((d - sd) / 86400000) + 1 };
  }
  return null;
}
// 几滴水：选中那一档亮几滴。⚠️不是一排药丸——形状本身就在说这一栏是「多少」。
function FlowDrops({ n, on, color, dim }) {
  return h("span", { style: { display: "inline-flex", gap: 2 } },
    [1, 2, 3, 4].map(function (i) {
      const lit = i <= n;
      return h("svg", { key: i, width: 9, height: 12, viewBox: "0 0 9 12", "aria-hidden": "true" },
        h("path", { d: "M4.5 1 C7 4.5 8 6.2 8 7.8 A3.5 3.5 0 0 1 1 7.8 C1 6.2 2 4.5 4.5 1 Z",
          fill: lit ? (on ? color : dim) : "transparent", stroke: lit ? (on ? color : dim) : dim, strokeWidth: 1 }));
    }));
}
function PeriodBook({ period, chars, daySel, onSave, onRecord, onBack }) {
  const t = useTheme();
  const per = period || {};
  const [cyc, setCyc] = useState(per.cycleLen || 28);
  ...
  const cur = periodDayOf(per, key);
  const last = list[list.length - 1] || null;
  const ACC = "#c25a4a";
  const saveLog = function (patch) {
    const next = Object.assign({}, logs);
    const row = Object.assign({}, next[key] || {}, patch);
    // 全空的那天不留一条空记录
    const empty = !row.flow && !row.pain && !(row.sym || []).length && !(row.mood || []).length && !String(row.note || "").trim();
    if (empty) delete next[key]; else next[key] = row;
    onSave({ logs: next });
  };
  const toggleIn = function (field, v) {
    const had = (log[field] || []).indexOf(v) >= 0;
  ...
  const sec = function (zh) { return h("div", { style: { fontFamily: F_BODY, fontSize: 10, letterSpacing: ".16em", color: t.fog, margin: "16px 0 8px" } }, zh); };
  const tag = function (v, on, click) {
    return h("button", { key: v, onClick: click, className: "active:opacity-70",
      style: { fontFamily: F_BODY, fontSize: 12, padding: "6px 11px", marginRight: 7, marginBottom: 7,
        color: on ? "#fff" : t.sub, background: on ? ACC : "transparent",
        border: "1px solid " + (on ? ACC : t.line), borderRadius: "10px 10px 10px 2px" } }, v);
  };
  const visSet = per.visibleTo || [];
  // 底纹铺在【最外那层外壳】上，Head 透上来（mobile-ui-layout.md §3.5）
  return h("div", { className: "h-full flex flex-col", style: (typeof pageSkin === "function" ? pageSkin("paper", t) : { background: t.bg }) },
    h(Head, { bg: "transparent", zh: "月事本",
      sub: cur ? ("这一轮第 " + cur.day + " 天") : (nextIn != null && nextIn >= 0 ? ("离下次大约还有 " + nextIn + " 天") : "还没记过"),
      onBack: onBack }),
    h("div", { className: "flex-1 min-h-0 overflow-y-auto px-5 pb-10" },
      // ── 这一轮 ──
      h("div", { style: { marginTop: 6, padding: "14px 15px", borderRadius: 16, background: t.bg2, border: "1px solid " + t.line } },
        h("div", { className: "flex items-baseline", style: { gap: 8 } },
          h("span", { style: { fontFamily: F_DISPLAY, fontSize: 26, color: cur ? ACC : t.ink, lineHeight: 1 } },
            cur ? ("第 " + cur.day + " 天") : (nextIn != null && nextIn >= 0 ? (nextIn + " 天") : "—")),
          h("span", { style: { fontFamily: F_BODY, fontSize: 11.5, color: t.fog } },
            cur ? ("这次从 " + String(cur.p.start).slice(5) + " 起" + (cur.p.end ? "，到 " + String(cur.p.end).slice(5) : "，还没记结束")) : (nextIn != null && nextIn >= 0 ? "之后大约来下一次" : "记一次开始，往后就有推算了"))),
        h("button", { onClick: function () { onRecord(key); }, className: "w-full active:opacity-70",
          style: { marginTop: 12, fontFamily: F_BODY, fontSize: 13, color: "#fff", background: ACC, borderRadius: 12, padding: "11px 0" } },
          (function () {
            if (list.some(function (x) { return x.start === key; })) return "取消：" + key.slice(5) + " 不是开始那天";
            if (list.some(function (x) { return x.end === key; })) return "取消：" + key.slice(5) + " 不是结束那天";
            const open = list.filter(function (x) { return !x.end && pKeyDate(x.start) < pKeyDate(key); })[0];
            return open ? ("把 " + key.slice(5) + " 记为这次的最后一天") : ("把 " + key.slice(5) + " 记为这次的第一天");
          })())),
      // ── 今天这一页 ──
      ...
      h("div", { className: "flex items-center", style: { gap: 10, marginBottom: 10 } },
        ...
        PERIOD_FLOW.map(function (f) {
          const on = log.flow === f.k;
          return h("button", { key: f.k, onClick: function () { saveLog({ flow: on ? "" : f.k }); }, className: "active:opacity-70",
            style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "4px 8px 5px", borderRadius: 10,
              background: on ? skinAlpha(ACC, "1a") : "transparent", border: "1px solid " + (on ? skinAlpha(ACC, "8c") : "transparent") } },
            h(FlowDrops, { n: f.n, on: on, color: ACC, dim: skinAlpha(t.ink, "38") }),
            h("span", { style: { fontFamily: F_BODY, fontSize: 10, color: on ? ACC : t.fog } }, f.zh));
        })),
      h("div", { className: "flex items-center", style: { gap: 10, marginBottom: 4 } },
        h("span", { style: { fontFamily: F_BODY, fontSize: 12, color: t.sub, width: 30 } }, "疼"),
        PERIOD_PAIN.map(function (pn) {
          const on = log.pain === pn.k;
          return h("button", { key: pn.k, onClick: function () { saveLog({ pain: on ? "" : pn.k }); }, className: "active:opacity-70",
            style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "6px 9px", borderRadius: 10,
              background: on ? skinAlpha(ACC, "1a") : "transparent", border: "1px solid " + (on ? skinAlpha(ACC, "8c") : "transparent") } },
            // 一道由细到粗的横杠：0 是一条发丝，3 是一整条
            h("span", { style: { display: "block", width: 20, height: Math.max(1, pn.n * 2.2), borderRadius: 2,
              background: pn.n ? (on ? ACC : skinAlpha(t.ink, "4d")) : "transparent",
              border: pn.n ? "none" : "1px dashed " + skinAlpha(t.ink, "4d") } }),
            h("span", { style: { fontFamily: F_BODY, fontSize: 10, color: on ? ACC : t.fog } }, pn.zh));
        })),
      sec("身上"),
      h("div", null, PERIOD_SYM.map(function (v) { return tag(v, (log.sym || []).indexOf(v) >= 0, function () { toggleIn("sym", v); }); })),
      sec("心情"),
      h("div", null, PERIOD_MOOD.map(function (v) { return tag(v, (log.mood || []).indexOf(v) >= 0, function () { toggleIn("mood", v); }); })),
      h("input", { value: log.note || "", onChange: function (e) { saveLog({ note: e.target.value.slice(0, 80) }); },
        placeholder: "还想记一句（吃了什么药、今天怎么样…）", className: "w-full outline-none",
        style: { marginTop: 12, padding: "10px 12px", borderRadius: 12, background: t.bg2, border: "1px solid " + t.line, fontFamily: F_BODY, fontSize: 13, color: t.ink } }),
      // ── 过往 ──
      sec("过往"),
      list.length ? h("div", null, list.slice().reverse().slice(0, 12).map(function (p, i, arr) {
        const nx = arr[i + 1];
        const gap = nx ? Math.round((pKeyDate(p.start) - pKeyDate(nx.start)) / 86400000) : null;
        return h("div", { key: p.start, className: "flex items-baseline", style: { gap: 8, padding: "9px 2px", borderBottom: "1px solid " + t.line } },
          h("span", { style: { fontFamily: F_DISPLAY, fontSize: 14, color: t.ink, width: 96 } }, String(p.start).slice(5) + (p.end ? " → " + String(p.end).slice(5) : " → 进行中")),
          h("span", { style: { fontFamily: F_BODY, fontSize: 11.5, color: t.sub } }, p.end ? (periodSpanLen(p, len) + " 天") : ""),
          h("span", { style: { flex: 1 } }),
          h("span", { style: { fontFamily: F_BODY, fontSize: 11, color: t.fog } }, gap ? ("距上次 " + gap + " 天") : ""));
      })) : h("div", { style: { fontFamily: F_BODY, fontSize: 12, color: t.fog, padding: "6px 2px" } }, "还没有记录。"),
      // ── 设置 ──
      sec("这本子怎么算"),
      h("div", { className: "flex", style: { gap: 10 } },
        ...
        h("label", { style: { flex: 1, fontFamily: F_BODY, fontSize: 11.5, color: t.fog } }, "经期长度（天）",
          h("input", { type: "number", value: len, onChange: function (e) { setLen(e.target.value); }, onBlur: function () { onSave({ periodLen: Number(len) || 5 }); }, className: "w-full outline-none",
            style: { marginTop: 4, padding: "9px 12px", borderRadius: 10, background: t.bg2, border: "1px solid " + t.line, fontFamily: F_BODY, fontSize: 14, color: t.ink } }))),
      h("button", { onClick: function () { setVisOpen(function (v) { return !v; }); }, className: "w-full active:opacity-70",
        style: { marginTop: 10, fontFamily: F_BODY, fontSize: 12.5, color: t.tint, border: "1px solid " + t.line, borderRadius: 12, padding: "11px 0" } },
        "谁能看到我的日历和经期（" + visSet.length + " 位）"),
      visOpen ? h("div", { style: { marginTop: 8 } },
        (chars || []).map(function (c) {
          const on = visSet.indexOf(c.id) >= 0;
          return h("button", { key: c.id, onClick: function () { onSave({ visibleTo: on ? visSet.filter(function (x) { return x !== c.id; }) : visSet.concat([c.id]) }); },
            className: "w-full flex items-center gap-3 active:opacity-70", style: { padding: "9px 0", borderBottom: "1px solid " + t.line } },
            h(Avatar, { character: c, size: 30, radius: 9 }),
            h("span", { style: { flex: 1, textAlign: "left", fontFamily: F_DISPLAY, fontSize: 14, color: t.ink } }, c.remark || c.name),
            h("span", { style: { fontFamily: F_BODY, fontSize: 11.5, color: on ? t.tint : t.fog } }, on ? "可见" : "不可见"));
        })) : null));
}
// ── 日历（v56.31 重做）──────────────────────────────────────────
...
function Calendar({ characters, calendar, calEvents, schedules, profile, period, busy, genWeekBusy, initialView, onBack, onSaveEvent, onDelEvent, onGenMonth, onSavePeriod, onRecordPeriod, onSaveTimed, onDelTimed, onGenWeek }) {
  ...
  if (pSet) return h(PeriodBook, {
    period: per, chars: chars, daySel: daySel,
    onSave: onSavePeriod, onRecord: onRecordPeriod, onBack: () => setPSet(false)
  });
...
// 每一件装饰自己那张【底】（她 2026-09-05：「每一个的封面也可以自定义，
// 不然都是现在米白背景好丑」）。不挑＝各款原样（胶卷是黑的、挂轴是宣纸色…），
...
const HOME_DECOR_GROUNDS = [
  ...
  { id: "#2b3a36", name: "墨绿" }, { id: "#3a2f3c", name: "紫檀" }, { id: "#8f4f45", name: "砖红" },
  { id: "#d9c9a8", name: "米金" }, { id: "#ffffff", name: "纯白" }
];
// 挑了深色的底，字必须跟着翻白——不然就是黑底黑字（v59.62 那次的另一种长相：
...
function decorGroundStyle(item) {
  var g = item && item.ground;
  if (!g) return null;
  if (typeof g === "object" && g.imageRef) {
    var src = typeof resolveImg === "function" ? resolveImg(g.imageRef) : g.imageRef;
    if (src) return { backgroundImage: "url(" + src + ")", backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat" };
    return null;   // 图丢了就当没挑，退回这一款自己的底，别落一块纯黑
  }
  var col = typeof g === "string" ? g : g.color;
...
function HomeDecorItem({ item, preset, now }) {
  const t = useTheme();
  // 底挑成深色（或放了一张图）之后，这一件就按深色画——ink / sub 全跟着走
  const dark = preset === "film" || decorGroundDark(item);
  const ink = dark ? "#f7f1e8" : t.ink;
  const sub = dark ? "rgba(247,241,232,.62)" : t.fog;
  const accent = item.accent || (dark ? "#d7b16b" : t.accent);
  // 这一件的【封面】。⚠️套在哪一层，得由每一款自己指名——
  //   有几款的纸是【里面那一层】（书签的纸、挂轴的纸、便利贴那张便条），
  ...
  const onGnd = function (base) { return gnd ? Object.assign({}, base, gnd, gShadow || {}) : base; };
  if (item.type === "photo") {
  ...
  if (item.type === "bookmark") {
    ...
    return h("div", { style: { width: "100%", height: "100%", minHeight: 120, position: "relative", display: "flex", justifyContent: "center", overflow: "hidden" } },
      ...
      h("div", { style: onGnd({ width: "78%", maxWidth: 62, marginTop: 15, marginBottom: 2, alignSelf: "center", background: bmPaper, border: "1px solid " + bmEdge,
  ...
  if (item.type === "scroll") {
    ...
    return h("div", { style: { width: "100%", height: "100%", minHeight: 150, position: "relative", display: "flex", flexDirection: "column", alignItems: "center", overflow: "hidden", paddingTop: 12 } },
      ...
      rod({ width: "88%" }),
      h("div", { style: onGnd({ width: "80%", flex: 1, minHeight: 0, background: scPaper, borderLeft: "1px solid rgba(120,96,64,.2)", borderRight: "1px solid rgba(120,96,64,.2)",
        ...
        vtext(title, { box: { height: "100%" }, ch: { fontFamily: F_DISPLAY, fontSize: 16, color: dark ? "#f2ece0" : "#4a3f30", letterSpacing: 0, marginBottom: 2 } }),
        detail ? vtext(detail, { box: { height: "78%", marginTop: "10%" }, ch: { fontFamily: F_BODY, fontSize: 9, color: dark ? "rgba(242,236,224,.72)" : "rgba(96,82,63,.72)", marginBottom: 2 } }) : null),
      rod({ width: "88%" }),
      // 轴头两端各露一小截
...
// list：只画这几种（做装饰那一页默认先露八种，全摊开是一堵墙）。不传＝全部。
function HomePhotoFrameGrid({ value, onChange, list }) {
  const t = useTheme();
  return h("div", { style: { display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 8 } },
    (list || HOME_PHOTO_FRAMES).map(function (p) {
...
function HomeDecorAppearanceEditor({ surface, borderMode, accent, align, badge, mark, tilt, ground, onSurface, onBorderMode, onAccent, onAlign, onBadge, onMark, onTilt, onGround, onGroundPhoto, busy }) {
  ...
  // 这一件自己那张【底】（她 2026-09-05：「每一个的封面也可以自定义，
  // 不然都是现在米白背景好丑」）。挑颜色，或者放一张自己的图。
  // 「原样」＝不挑：各款照旧用它本来那张底（胶卷是黑的、挂轴是宣纸色…）。
  var gImg = ground && typeof ground === "object" && ground.imageRef;
  var gCol = typeof ground === "string" ? ground : (ground && ground.color) || "";
...
function Home({
  ...
  const [decorFrameAll, setDecorFrameAll] = useState(false); // 相框二十来种，默认只露前八种
  ...
  const [styleDecorGround, setStyleDecorGround] = useState(null);
  ...
  function openStylePanel(key) {
    ...
    if (it && it.kind === "decor" && it.decor) {
      ...
      setStyleDecorGround(d.ground || null);
  ...
  // 半窗的代价是固定的（那条规矩的原话），而这一页装的东西只会越来越多。
  //
  // 改成整页 + 三段，一次只摊开一段：
  //   ① 是什么 ② 写什么 ③ 什么样子
    ...
    var section = function (id, no, name, summary, body) {
      ...
      return h("div", { style: { borderRadius: 16, border: "1px solid " + (open ? t.ink : t.line), background: t.bg, marginBottom: 10, overflow: "hidden" } },
        h("button", { onClick: function () { setDecorStep(open ? "" : id); }, className: "w-full active:opacity-70 flex items-center",
          ...
          h("span", { style: { width: 20, height: 20, borderRadius: 6, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
            border: "1px solid " + (open ? "rgba(255,255,255,.5)" : t.line), fontFamily: F_BODY, fontSize: 10.5 } }, no),
          h("span", { style: { fontFamily: F_DISPLAY, fontSize: 14.5, flexShrink: 0 } }, name),
...
function HomeCard({ card, profile, characters, onEditCard, onEditProfile, onOpenCodex }) {
  ...
  const stats = React.useMemo(() => {
    ...
    // 开屏只读这个起点。写 ts 不写天数：天数每天都变，起点一旦有了就不动。
    try { if (first) localStorage.setItem("x_firstDayTs", String(first)); } catch (e) {}
...
function ChatThread({
  ...
  messages.length === 0 && /*#__PURE__*/React.createElement(Empty, {
    ...
    if (m.kind === "studyinvite" || m.kind === "gameinvite" || m.kind === "readinvite" || m.kind === "tarotinvite") {
      const isGame = m.kind === "gameinvite", isRead = m.kind === "readinvite", isTarot = m.kind === "tarotinvite";
      const eyebrow = isTarot ? "算一卦" : isRead ? "一起读" : isGame ? "一起玩" : m.mode === "resume" ? "继续一起学" : "一起学邀请";
      const go = isTarot ? "去摊开这几张" : isRead ? "翻到那一页" : isGame ? "去摆这一局" : m.mode === "resume" ? "继续这门课" : "看看课程草案";
      return h("div", { key: i, className: "py-2 flex items-start gap-2 justify-start" },
        h(Avatar, { character: character, size: 34, radius: 10 }),
        h("div", { style: { maxWidth: "82%", background: t.bg2, border: "1px solid " + t.line, borderRadius: 14, padding: "11px 12px" } },
...
function useOfflineCustomStyles(t, styleKey, setStyleKey) {
  ...
  const saveCustomStyle = () => {
    ...
    if (!saveJSON("x_offlineStyles", next)) return toast("这次没保存成功，草稿还在");
  ...
  const importStyleFile = async e => {
    ...
    try {
      ...
      if (!saveJSON("x_offlineStyles", next)) throw new Error("保存失败，请重试");
      setCustomStyles(next); setStyleKey(key);
  ...
  return { customStyles, styleSheet, setStyleSheet, custOpen, setCustOpen, cName2, setCName2, cPrompt, setCPrompt, editingStyleKey, setEditingStyleKey, allStyles, curStyle, saveCustomStyle, delCustomStyle, editCustomStyle, styleImportControl, styleKey, setStyleKey };
}
function OfflineCustomStyleSection({ t, editor }) {
  const { customStyles, custOpen, setCustOpen, cName2, setCName2, cPrompt, setCPrompt, editingStyleKey, setEditingStyleKey, allStyles, curStyle, saveCustomStyle, delCustomStyle, editCustomStyle, styleImportControl, styleKey, setStyleKey } = editor;
  return h("div", { className: "pt-5", style: { borderTop: "1px solid " + t.line, marginTop: 18 } },
...
function ChatRoomSheet({ character, activeRoomId, sourceMessages, onCreateRoom, onClearRoom, onSelect, onClose, onSummarize, embedded }) {
  ...
  const enterNewRoom = async () => {
    ...
    setCreateBusy(true);
    try {
      const saved = await onCreateRoom(prepared);
      if (!saved) return window.__toast && window.__toast("这次没存好，房间还没创建");
      setRooms(Kit.list(character.id)); setDraft(saved); setCreating(false); onSelect(saved.id, true);
...
function ChatSettings({
  ...
  const innerLifeImpact = (() => {
    ...
    //   于是授权这件事本身名不副实；她直接拍板两层都开，只留诊断台那颗急停。
    //   所以这里【不再看 mode】，只看 emergencyOff：按下急停就整个退回观察。
  ...
  show("temper", { title: cNm + " 的底色 · 几个词", ...sec("temperament") },
    ...
    aShadowPanel && aShadowPanel.state && h("div", { style: { marginTop: 16, paddingTop: 14, borderTop: "1px solid " + t.line } },
      ...
      temperNumsOpen ? h("div", null,
        h("div", { className: "flex flex-wrap", style: { gap: 6, marginTop: 2 } }, Object.entries(aShadowPanel.state.emotion.current || {}).filter(([key]) => ((window.DongnianEmotionA && window.DongnianEmotionA.projectedAxes) || [key]).indexOf(key) >= 0).map(([key, value]) => h("span", { key, style: { fontFamily: F_BODY, fontSize: 10.5, color: t.sub, border: "1px solid " + t.line, borderRadius: 999, padding: "4px 8px" } }, (((window.DongnianEmotionA && window.DongnianEmotionA.displayLabels) || {})[key] || key) + " " + Number(value).toFixed(2)))),
  ...
  actDesc ? h("div", { className: "flex items-center justify-between pt-4" },
    h("div", { style: { paddingRight: 12 } },
      // ⚠️底下那段灰字删掉了（她 2026-09-12：「太长了给别人看不好」）。
      //   标题 + 两个选项本身就说得清；解释留在这儿给下一个改代码的人看，不占她的屏幕。
