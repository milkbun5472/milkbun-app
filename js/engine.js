...
// 开关这一侧的动描规则（拼进线上 runtime，跟 ONLINE_CHAT_RULE_V2 并排）。
// ⚠️它只解禁【括号里那一小行】，不解禁散文：一开这个就写成线下长段，等于把两套系统叠在一起。
function samePlaceActRule(uName) {
  return "【动作怎么写·整条用括号包住】\n"
    + "要写你此刻的动作、或者你这边的动静，就【单独给 word 一个元素、整条从头到尾用一对中文圆括号包住】，形如「（……）」，括号里只写那一下动作本身。"
    + "它会显示成居中的一行小字，不是你发出去的消息。\n"
    + "· 只在【处境真的变了】的时候写：起身、换个地方、伸手接过东西、被谁打断、手上正忙着。没有变化就一条都别写——一整轮全是说话完全正常。\n"
