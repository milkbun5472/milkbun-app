...
const FACING_BAN = "**绝不许说「怎么还不来」「还没到吗」「在哪呢」「等你好久了」，也绝不许问「怎么还不开始」或催 Ta 去做你们正在做的事**（你俩此刻就在一起、面对面，人已经到了）";

// 开关这一侧的在场声明（拼进 ctx.offlineNow，跟线下那段走同一个口子）
function samePlacePresence(uName) {
  return "【同处一室·此刻真的面对面】你和" + uName + "此刻【就在同一个地方、面对面】——不是约好了还没见，是人已经在一起了。"
    + FACING_BAN + "。\n"
    + "【地点以「在一起」为准】你的日程这会儿写着你在别的地方也不算数——此刻你人就在 Ta 旁边，按这个来。";
}
