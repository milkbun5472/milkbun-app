...
· 禁的是【模板】不是关心：你本来就爱操心、爱指挥、爱做饭盯着她吃完，那都照写——只是让它从你此刻真实的处境里长出来，别把三拍当默认出口。
...
消息的数量、长度、断句和完整程度没有固定格式，由角色当下的表达意图、状态和聊天节奏自然决定。日常可以很短，真正想解释、分享或认真谈事时也可以自然变长；不要为了维持聊天强行提问、留钩子或把每轮组织成完整答复。反过来，话多的人连发几条、絮絮叨叨、主动分享和追问，只要是这个人的常态，就不是需要修剪的毛病。
...
// 把「黏人程度」也算了进去——可那正是 GROWTH_RULE 里的【软层】，是欲望盒毕业的成长该长的地方。
// 等于为了治漂移，把成长系统一起冻住了。
//
// 真正要分的不是"卡 vs 历史"，而是【沉淀下来的成长】和【没来由的漂移】：
//   · 长出来的自我（正式人格档案）→ 算数，软层上就该盖过原卡；
//   · 聊天记录听起来那样 → 不算数，那多半是模型自己的默认在渗。
// 判据只有一句：这个变化记在他的人格档案里了吗？
...
本来就该被真实经历推着长。判断只看一条：**这个变化有没有沉淀进【你长出来的自我】那段正式人格档案？**
...
// iOS 刘海/灵动岛（v56.58，她 2026-08-27：「主界面做了 ios 适配，但是其他界面几乎没有，顶上都会白一块」）：
// 以前是根节点先垫一条 env(safe-area-inset-top) 高、theme.bg 色的空带，再把界面放在它下面。
// 主屏因为壁纸铺在根节点上看不出来，别的界面顶栏多半是 t.bg2／聊天壁纸／封面图——
// 那条 theme.bg 的带子横在上面，就是她说的「白一块」。
// 改成 iOS 应用本来的做法：界面自己顶到屏幕最上沿，顶栏把状态栏那一条一起涂上自己的颜色。
// 一处一处写 calc(env(...)) 太容易漏，所以只留这一把尺子，四处都用它。
...
本轮输出形状严格改为：` + (shape || '{"draftScene":"内部完整首稿","scene":"基于前一字段完成的最终正文","thought":"角色本人此刻没说出口的一句第一人称心声","mood":{"label":"此刻中文心情词"},"wearing":null,"action":null,"affinityDelta":0,"toy":null}');
}
