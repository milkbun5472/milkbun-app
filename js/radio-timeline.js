...
(function (root, factory) {
  ...
  // ------------------------------------------------------------
  // 原来整份 storyPrompt 钉死在【TA讲TA自己的经历】上，而且里头那句
  // 「广播那头没有人、没有听众」正好把「讲故事给人听」整个禁死了——
  // 她要「TA讲恐怖故事」那一档，缺的不是一句话，是**四件事同时得换**：
  //
  //   tell     讲TA自己的事，还是讲一个故事（故事里的人不是TA）
  //   audience 广播那头有没有人在听
  //   roots    「根基不改写」那条管谁：管TA本人，还是只管TA这个讲的人
  ...
  const SHOW_AXES = [
    { key: "tell", zh: "TA在讲什么", opts: [
      { id: "self", zh: "TA自己的经历" },
      { id: "story", zh: "一个故事（故事里的人不是TA）" }] },
    { key: "audience", zh: "广播那头有没有人", opts: [
      { id: "none", zh: "没有人，TA一个人在说" },
      { id: "listeners", zh: "有人在听，TA是讲给人听的" }] },
    { key: "roots", zh: "「根基不改写」管谁", opts: [
      { id: "self", zh: "管TA本人：来历、身份、关系照角色卡" },
      { id: "teller", zh: "只管TA这个讲的人：故事里的人随TA编" }] },
  ...
  // ------------------------------------------------------------
  // 匿名连线：她打进TA正在播的那档节目，TA不知道是谁（她 2026-09-12 排的第一条）
  // ------------------------------------------------------------
  // 现成先例是匿名箱（app.js 的 openAnonBox）：她戴着同一个马甲，TA只有这几句话，
  ...
  //   没播的那几句对谁都不存在：连TA自己下一章都读不到（见 airedCalls）。
  const CALLER = "caller";
  ...
  // 一通电话一个字都没播出去，它就没发生过——连TA自己都不知道有人打进来过。
  ...
  function callPrompt(branch, eraId, mask, say, anchor) {
    ...
    return [
      ...
      "⚠️这是**你在广播里对着一个人说话**，不是在讲故事，也不是在给人上课：不背道理、不写箴言、不用「其实／本质上／无非是」这种开场。TA问的话里有一半可能问偏了、问得莫名其妙、或者建立在一个错的前提上——那你就照你想的说，不必替TA把话圆回来。",
  ...
  // 她说的那句是她自己写的，模型只写TA这一头；两边合成同一个片段，播的时候按顺序往下走。
  ...
  function storyPrompt(branch, eraId) {
    ...
    return [
      ...
      //   于是「TA讲恐怖故事」这一档根本没有入口（她 2026-09-12 排的第三条）。
      story
      ...
      //   （文件头第一行：仅存在本分支，不写主线），本来就不必替TA把风险都绕开。
      //   所以这儿改成【钉根基、放事情】：来历身份关系不许改写，这一章里发生的事可以有代价。
      // ③【根基那条管谁】——TA讲的是别人的故事时，「来历身份关系不改写」
      //   要是照旧管到故事里的人头上，那就等于不许TA编故事了。
      story && show.roots === "teller"
        ...
        : "角色卡明确事实与世界规则是依据：TA的来历、身份、已有的关系这些根基不改写。除此之外，这一章里可以真的发生要紧的事——TA的决定可以有代价，事情可以往不好的方向去，可以留下收不回来的后果。这条分支是平行创作、只活在这条线上，不回流主线，不必替TA把风险都绕开。人物的选择从其性格与处境生长。",
      ...
      // ⚠️讲故事那一档里，摊开来写的那个人不是TA——这一段里的「TA」得换成故事里那个人，
      //   不然模型会把主角悄悄换回讲的人自己。
      (story
        ? "**这一章要是一件事，不是一段生平。**它自己有头有尾：开头落在某一天真正发生的一个场面上——哪件事从哪一刻起不对劲了、故事里那个人是怎么被卷进去的；中间TA得做点什么，而且做了要付出代价；结尾停在一个看得见的画面、或一句真说出口的话上，事情有了结果，TA也不再是开头那个人。"
        : "**这一章要是一件事，不是一段生平。**它自己有头有尾：开头落在某一天真正发生的一个场面上——哪件事从哪一刻起不对劲了、TA是怎么被卷进去的；中间TA得做点什么，而且做了要付出代价；结尾停在一个看得见的画面、或一句真说出口的话上，事情有了结果，TA也不再是开头那个人。")
        + "可以跨越时间与场景；本章落点之后，长线仍能往下走。⚠️判据：这一章单独播给一个没听过前文的人，TA能说出「这讲的是什么事、最后怎么了」——说不出来，就是还没成一件事。",
      "同一章里【轻重要分开】：要紧的那一两处摊开来写——TA当时看见什么、谁说了什么、TA手上在做什么、心里怎么翻的；过场的部分一句带过就行。⚠️别让全章每一句都是同一个分量、同一个速度。尤其别用「后来……再后来……」把关键的地方交代过去：被这样交代过去的那几句，正是故事本该发生的地方。",
      ...
      //   人称这件事下面那句「不是TA把耳罩拨开一点，是我把耳罩拨开一点」已经说得够清楚了。
      story
        ? "全篇是**TA一个人在讲**，用TA自己的措辞、TA会留意的东西、TA讲到这儿会停的地方。故事里的人物TA当然用第三人称讲——那是TA在讲别人；别人说的话嵌进来，标明说话者。**但不要另起一个旁白**：讲的人从头到尾是TA，不是一个没有身份的声音。"
        : "全篇只有TA一个人的声音，用TA自己的措辞和TA会留意的东西讲出来。**不要旁白、不要第三人称交代**——场景也由TA自己说（不是「TA把耳罩拨开一点」，是「我把耳罩拨开一点」）。故事里别人说的话可以嵌进来，标明说话者。",
      story
        ? "TA是在【讲一个故事】，不是现场直播。哪怕频率是「现在」，这一章也是一段讲完了的东西，有始有终；不是一句一句同步播报此刻正在发生什么。"
        : "TA是在【讲一段自己经历过的事】，不是现场直播。哪怕频率是「现在」，也是TA回过头来把这段讲完，有始有终；不是一句一句同步播报此刻正在发生什么。",
      ...
      show.audience === "listeners"
        ...
        : aired.length
          ...
          : "⚠️用户是收听者，不是这段独白的收件人——TA不是在对谁说话。广播那头没有人：没有听众、没有称呼（姐姐、宝宝、名字、你，一个都不要）、不提问、不索取回应、不停下来等人接话。用户就算作为一个人出现在TA讲的事里，那也只是TA故事里的一个人物，这段独白仍然不是说给她听的。",
      story
  ...
  function companionPrompt(branch, companionId, question, keepPlaying) {
    ...
    // ⚠️节目类型换了，这两句也得跟着换：TA讲的要是一个故事，那「不是你真实经历过的事实」
    //   这句话说得太轻了——那压根不是谁的经历，是广播里那个人编出来讲的。
    const story = showOf(branch).tell === "story";
    return [
      isSelf
        ? (story
          ? "广播里在讲故事的是另一条时间线上的你。**那不是谁的经历**，是TA在节目里讲的一个故事；你没讲过它，是从收音机里听来的。你此刻是坐在用户身边一起听的那个人。"
          : "广播里那个人是另一条时间线上的你。**你没经历过那些事**——那不是你的记忆，是从收音机里听来的另一种可能。你此刻是坐在用户身边一起听的那个人。")
        : (story
          ? "你是坐在用户身边的陪听者，不是广播中的人物。刚才播的是广播里那个人在TA的节目里讲的一个故事——不是谁真实经历过的事，也不是预言。"
