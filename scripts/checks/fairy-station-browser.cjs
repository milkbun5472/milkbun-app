...
(async()=>{const browser=await chromium.launch({headless:true,channel:'chrome'});try{
 ...
 const seed=await p.evaluate(async()=>{const w=await import('./world.mjs'),s=w.freshState();s.position={...w.START};s.minute=720;s.herbs=23;s.companion.mode='follow';s.companion.position={x:s.position.x+1,z:s.position.z+.4};return s});
 ...
 assert.ok(!await p.evaluate(()=>gardenDebug.getDistricts().loaded.includes('station')),'station is not loaded from home');
 if(await p.locator('#panel-content').isVisible())await p.locator('#panel-toggle').click();
 await p.locator('#map-open').click();await p.locator('[data-label="林边车站"]').click();
 await p.waitForFunction(()=>gardenDebug.getPlayer().z>27,null,{timeout:120000});await p.screenshot({path:out+'/station-game-route.png'});
 await p.waitForFunction(()=>{const a=gardenDebug.getPlayer();return Math.hypot(a.x-.6,a.z-40.65)<.23&&!a.moving},null,{timeout:120000});
 ...
 await p.waitForFunction(()=>gardenDebug.getDistricts().loaded.includes('station'));console.log('ARRIVED',await p.evaluate(()=>({player:gardenDebug.getPlayer(),companion:gardenDebug.getCompanion(),districts:gardenDebug.getDistricts()})));
 for(let n=0;n<2;n++)await p.locator('#zoom-out').click();await p.screenshot({path:out+'/station-game.png'});
 await p.reload();await p.waitForFunction(()=>window.gardenDebug?.getReady());await p.locator('#loading').waitFor({state:'detached'});const saved=await p.evaluate(()=>gardenDebug.getState());assert.equal(saved.herbs,23);assert.ok(Math.hypot(saved.position.x-.6,saved.position.z-40.65)<.23);
 ...
 await p.waitForFunction(()=>!gardenDebug.getDistricts().loaded.includes('station'));console.log('RETURNED',await p.evaluate(()=>gardenDebug.getDistricts()));
 // Winter/night uses the same save writer and outdoor renderer, in this isolated profile only.
 const winter=await p.evaluate(()=>{const s=gardenDebug.getState();s.position={x:.6,z:40.65};s.companion.position={x:1.7,z:40.65};s.day=43;s.minute=1260;return s});
 ...
 await p.setViewportSize({width:1100,height:900});while(await p.evaluate(()=>gardenDebug.getView().zoom>.75))await p.locator('#zoom-out').click();await p.screenshot({path:out+'/station-winter.png'});assert.deepEqual(errors,[]);console.log('PASS actual map route, companion, all stream boundaries, reload, mobile pan, return unload, winter/night',await p.evaluate(()=>gardenDebug.getRenderStats()));
 await ctx.close();
