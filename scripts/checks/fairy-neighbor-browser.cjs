...
(async()=>{const browser=await chromium.launch({headless:true,channel:'chrome'});try{
 ...
 const s=await p.evaluate(async map=>{const w=await import('./world.mjs'),s=w.freshState();s.position={...w.MAPS.garden.exits[map].target};s.companion.mode='follow';s.companion.position={x:s.position.x+1,z:s.position.z+.3};return s},map);
 ...
 await p.locator(`[data-door="${map}"]`).click();await entered();await follow();
 ...
 await walk({x:0,z:0});await p.waitForTimeout(300);await p.screenshot({path:out+'/'+map+'-desktop.png'});
 ...
 await p.reload();await p.waitForFunction(()=>window.gardenDebug?.getReady());assert.equal(await p.evaluate(()=>gardenDebug.getState().map),map);
 ...
 await p.locator(`[data-door="${map}"]`).click();await entered();await follow();await p.setViewportSize({width:390,height:844});
 if(await p.locator('#panel-content').isVisible())await p.locator('#panel-toggle').click();
