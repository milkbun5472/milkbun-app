...
await f.locator('[data-mode=together]').click();
...
await f.locator('#desk-message').fill('你刚才递给我的是哪片？');await f.locator('#desk-send').click();await f.waitForFunction(()=>document.querySelector('#desk-chat').textContent.includes('你刚才递给我的是哪片'));
...
await p.evaluate(()=>{window.originalPuzzleSave=saveJSON;window.saveJSON=(k,v)=>k==='x_fairyGarden:puzzle-test'?false:originalPuzzleSave(k,v);});await f.locator('[data-mode=self]').click();assert.deepEqual(await f.evaluate(()=>TrainGame.desk.debug().puzzle),saved);await p.evaluate(()=>{window.saveJSON=originalPuzzleSave;});
await f.locator('[data-mode=self]').click();assert.equal(await f.evaluate(()=>TrainGame.desk.debug().puzzle.cooperation.pending),null);
...
await f.locator('[data-mode=together]').click();await p.waitForTimeout(6500);assert.equal(await f.evaluate(id=>TrainGame.desk.debug().puzzle.pieces[id].locked,finalPiece),false);assert.match(await f.locator('#desk-cooperation-status').innerText(),/最后一片/);
...
await f.locator('#desk-new').click();await f.waitForFunction(()=>TrainGame.desk.debug().puzzle?.moves===0);await f.evaluate(async()=>{const {drop,target}=await import('./puzzle.mjs');const d=TrainGame.desk.debug().puzzle;for(let id=0;id<10;id++){const t=target(d,id);drop(d,id,t.x,t.y);}TrainGame.flush();});await f.locator('[data-mode=together]').click();await f.waitForFunction(()=>TrainGame.desk.debug().companionMove);const moving=await f.evaluate(async()=>{const {drop,target}=await import('./puzzle.mjs');const d=TrainGame.desk.debug(),other=d.puzzle.pieces.find(q=>!q.locked&&q.id!==d.companionMove.id),t=target(d.puzzle,other.id);drop(d.puzzle,other.id,t.x,t.y);TrainGame.flush();return d.companionMove.id;});await p.waitForTimeout(5000);assert.equal(await f.evaluate(id=>TrainGame.desk.debug().puzzle.pieces[id].locked,moving),false);assert.equal(await f.evaluate(()=>TrainGame.desk.debug().companionMove),null);
