...
(async()=>{const b=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 let f=p.frames().find(f=>f.url().includes('/apps/train/'));assert.equal(await f.locator('select:visible,input:visible').count(),0);/* v74.042 视角挪到路线那排；v74.058 底栏五个：聊聊/走动/拍照/拼图/相册 */assert.equal(await f.locator('[data-view]').count(),3);assert.equal(await f.locator('nav [data-view]').count(),0);assert.equal(await f.locator('nav button').count(),5);
