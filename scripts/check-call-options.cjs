...
(async () => {
  ...
  try {
    for (const width of [390, 320]) for (const mode of ['voice', 'video']) {
      ...
      await page.setContent('<div id="settings" style="position:relative;z-index:100;background:white;padding:12px"></div><div id="root" style="height:100vh;position:relative"></div>');
      ...
      await page.evaluate(mode => {
        ...
        window.settingsRender = ReactDOM.createRoot(document.getElementById('settings'));
        settingsRender.render(h(OnlineMediaSettings));
      ...
      // 窄屏控件不出界，只有消息区滚动。
      const layout = await page.evaluate(() => { const controls = [...document.querySelectorAll('[data-online-media-settings] button')].map(e => { const r = e.getBoundingClientRect(); return { left: r.left, right: r.right }; }); return { controls, width: innerWidth, overflow: document.documentElement.scrollWidth > innerWidth }; });
      ...
      await page.evaluate(() => {
        ...
        window.rootRender = ReactDOM.createRoot(document.getElementById('root'));
        const dial = document.createElement('button'); dial.id = 'dial'; dial.textContent = '测试拨打';
        dial.onclick = () => { draw({ audioSession: prepareCallAudio(), msgs: [], participants: [props.participants[0]], bye: null }); dial.remove(); };
        document.getElementById('settings').appendChild(dial);
      });
      ...
      // 真正 App startCall 写入方：拨打和接听都不能依赖通话页按钮或开麦。
      for (const caller of ['me', 'a']) {
        await page.evaluate(({ startCallBody, caller }) => {
        ...
        }, { startCallBody, caller });
        await page.locator('#start-real').click();
        await page.waitForFunction(() => !!document.querySelector('#root [aria-label="开启麦克风"]'));
        await page.evaluate(() => draw({ msgs: [{ ts: Date.now(), role: 'char', content: 'Automatic first line' }] }));
        await page.waitForFunction(() => requests.length === 1);
        await page.evaluate(() => pending.shift()());
        await page.waitForFunction(() => sources.length === 1);
        assert.equal(await page.locator('#root [data-online-translation-setting], #root [data-online-media-settings]').count(), 0);
