...
# blender --background --python scripts/export-fairy-village.py -- source.blend output.glb
args=sys.argv[sys.argv.index('--')+1:]
SOURCE,DEST=map(Path,args[:2]);P=SOURCE.parent
...
for m in bpy.data.materials:
 ...
 for y in range(size):
  for x in range(size):
   ...
   pix.extend([max(0,min(1,c*k)) for c in col]+[1])
