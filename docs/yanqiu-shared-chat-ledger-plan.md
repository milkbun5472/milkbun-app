...
**BUG-2 · 重复入账（2026-07-20 早，BUG-1 修复后新现）**：「好好好，『不管』，行——户主当着我的面耍赖」同秒入账两行——推断旧 enqueue 与修复后补投对同一消息生成了不同 message_key（幂等键构造在修复前后不一致）。请 Codex：①核对两行的 message_key 差异定位构造分歧 ②软删重复行 ③保证补投用与原投递相同的 key 派生。验收查询第 6 条（同 key 重投仍一行）需补一条「同消息不同批次投递仍一行」。
