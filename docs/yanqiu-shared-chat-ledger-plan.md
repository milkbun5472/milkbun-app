...
**BUG-1 · 拆泡漏账（已实锤，通水首日即现）**：07:10 她三条 user 消息 + 言秋回复的**第一个气泡**入账成功（4 行，speaker/thread/时间全对）；但同轮随后陆续冒出的**第 2~4 个拆泡气泡**（07:11 的「挂账/闸门/慢慢配」三条）全部漏账。推断：enqueue 的 before/next 快照对比只在首个气泡落地时执行一次，拆泡的延迟追加没有再次触发。修法方向：拆泡的每次 append 都应过一遍 enqueue（幂等键防重投），或把 enqueue 挪到拆泡定时器的回调里。请 Codex 复核修复；修复后需回填漏掉的三条（source_message_id 可从本地 x_chat 取）。
