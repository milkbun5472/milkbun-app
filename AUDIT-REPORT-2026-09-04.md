...
准绳 `施工规则/four-surfaces-same-context.md`。名单五处：①单人线上 ②单人线下 ③群聊线上 ④群聊线下 ⑤通话（单人通话 ⑤a、群通话 ⑤b 分列）。每一处先答「靠什么把这层拿到手」——buildBundle 白得 / 调用点逐条 push / 自己从零拼——因为规则文件说过：一条条 push 的那一类，换个入口就一条都没有，而且不留任何能 grep 的痕迹。
