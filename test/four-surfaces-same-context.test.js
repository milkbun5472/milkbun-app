...
test("群聊线上补上心情/好感/印象卡", () => {
  ...
  // v66.56：旁观群里她不在场，这一层不发（好感 87 的人一开口就想找她）——别冻在前半截上
  assert.match(now, /afSeg: o\.spectate \? "" : "\\n〔对 " \+ userName\(profile\) \+ " 的好感〕"/);
  // 心情要走平复逻辑，别把三天前那阵气当成此刻
  assert.match(now, /window\.MoodLabel\.settle\(\(moods\[c\.id\] \|\| \{\}\)\.label, \(moods\[c\.id\] \|\| \{\}\)\.ts, Date\.now\(\)\)/);
  // 群聊那一处真的用了这一份（不是各写各的）
  assert.match(app, /const _now = groupNowSegs\(c, \{ interop: gs\.memoryInterop, spectate: !!gs\.spectate \}\);/);
