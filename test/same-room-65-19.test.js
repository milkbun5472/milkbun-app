...
test("他那边落成同一种消息，只多一个 who 标明是他做的", () => {
  ...
  // v72.96：这一行搬去了旁白那一侧（assistant 那边看见自己写动作散文，会照着往 word 里写），
  //   但两种旁白必须分得开——他那条要点着他的名字。
  assert.match(app, /const _actRow = m\.who === "char";/, "喂回去时不再分是谁的动作了");
  assert.match(app, /"【场景旁注｜" \+ char\.name \+ " 当时正在做的动作，不是任何人说出口的话】/, "他那条没点名是他做的");
  assert.match(app, /"【无说话人的场景旁白｜不是" \+ uName \+ "说的话】/, "她那条的边界没了");
});
...
test("群聊也接上了：谁变了谁那几泡前面出一行，没变的不出", () => {
  ...
  const i = app.indexOf("_gActDesc && gActionNow && spk");
