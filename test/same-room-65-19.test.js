...
// 括号大法她那边和他那边共用同一个判据，开关按人存、顶栏一眼看得见。
...
test("括号动描只在开关开着时解禁，关着的时候线上一个字都不变", () => {
  assert.match(app, /const _sameRoom = !_s\.engineerEyes && sameRoomFor\(charId\);/);
  assert.match(app, /PERSONA_REGISTER_ANCHOR \+ \(_sameRoom \? "\\n\\n" \+ samePlaceActRule\(uName\) : ""\)/,
    "动描规则没挂在开关上，或者挤进了那三层中间");
  // 那三层的顺序不许被这一条打断（别处一堆测试盯着这条链）
  assert.match(app, /ONLINE_CHAT_RULE_V2 \+ "\\n\\n" \+ REGISTER_FOLLOWS_SCENE \+ "\\n\\n" \+ PERSONA_REGISTER_ANCHOR/);
});

test("动描规则该有的闸一个都不少，而且不塞内容示范", () => {
  const rule = grab(eng, "samePlaceActRule");
...
test("她那边两个入口共用一个出口，关着的时候还是普通消息", () => {
  ...
  assert.match(app, /const act = sameRoomFor\(charId\) \? actInner\(text\) : null;/);
  assert.match(app, /\? \{ role: "narration", kind: "narration", content: act, ts: Date\.now\(\), read: true \}/);
});

test("他那边落成同一种消息，只多一个 who 标明是他做的", () => {
  assert.match(app, /const _act = _sameRoom \? actInner\(words\[i\]\) : null;/, "他那边另写了一套判据");
  ...
  assert.match(app, /_sameRoom \? "【" \+ uName \+ "此刻做的动作／她那边的动静｜不是 Ta 说出口的话】/);
});

test("开关按人存、顶栏一眼看得见，旁白那行有挂点", () => {
  ...
  const ph = comp.match(/sameRoom \? "(发一条消息[^"]*)"/);
