...
test("生成几篇就有几个框，没填的明说自由发挥", () => {
  ...
  assert.match(fic, /props\.onConfirm\(n, chosenCP\(\), styleIds, !isGroup && twoRealChars\(\) && includeMe, briefs\.slice\(0, n\), authors\.filter/);
  assert.match(fic, /async function doGen\(n, cp, styleIds, includeMe, briefs, byAuthor, groupWay\)/);
