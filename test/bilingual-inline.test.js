...
test("自带中译走的还是气泡旁边那个译键，不另起一套 UI", () => {
  // 锚收到【函数名 + 它收了 zhReady】为止：整串参数表是会被加 prop 的东西
  // （施工规则/anchor-on-code.md；v72.24 加 inline 时这条就断过一次）
  assert.ok(/function TransText\(\{[^}]*\bzhReady\b[^}]*\}\)/.test(compCode), "TransText 没接自带中译");
