...
test("点哪一格进的都是同一页，只是先滚到那一栏", () => {
  ...
  const seg = screens.slice(i, i + 5600);
  ...
  // 顶上那排小标签点了也跳。⚠️v67.33 起这两处共用 goSec 那一支
  //（进来那一跳和点布标是同一件事，各写一遍改一处必漏另一处）
  assert.match(seg, /onClick: \(\) => \{ taken\.current = true; goSec\(x\.key\); \}/);
});
...
test("每一栏的内容仍然由 CarrySection 画，没有第二份渲染", () => {
  ...
  // ⚠️别钉整串参数表（v70.81 中间插了 onDeleteItem，v70.85 又加了 closetData/onClosetGift）：
  //   这条守的是「CarrySection 还是那个收 sectionKey/data/gifts 的组件」，不是参数的排列。
  assert.match(screens, /function CarrySection\(\{ char, sectionKey, data, gifts,/);
