...
test("普通角色真的收到断句这条，不是只躺在没人用的串里", () => {
  ...
  // ⚠️原来钉的是「想说三句就给三个元素」——那个「三」就是默认三个气泡的来源
  //   （施工规则/prompt-no-content-samples.md：留格式示范，删内容示范）。
  //   这条守的是【一个元素＝一条气泡】这个格式，不是那个数字。
  assert.match(app, /说了几句就给几个元素/);
  assert.ok(!/想说三句就给三个元素/.test(app), "又把那个可抄的数字写回 word 那一格了");
});
