...
test("第一次翻的时候不发这一块，别凭空多一段空指令", () => {
  [null, undefined, {}, { cards: [] }, { cards: [{ name: "无 quote" }] }].forEach(k =>
    assert.ok(!/上一轮TA已经说过这些/.test(spec(k)), "没有上一轮的时候不该出现这一块"));
});
...
test("只发句子本身，不把整份报告搬回去（健康是 ♻️，报告不跨轮累积）", () => {
  ...
  const block = spec(known).split("【上一轮TA已经说过这些")[1] || "";
...
test("别的 app 不吃这一块，而且写清楚了为什么", () => {
  ...
  ["notes", "wechat", "album", "tally"].forEach(k =>
    assert.ok(!/上一轮TA已经说过这些/.test(P.phoneProbeSpec(k, char, [], "", [], known).instruction),
      k + " 也吃到了健康那一块"));
  ...
  assert.ok(!/上一轮TA已经说过这些/.test(P.phoneProbeSpec("reading", char, [], "", [], { shelves: [{ name: "架子" }] }).instruction));
