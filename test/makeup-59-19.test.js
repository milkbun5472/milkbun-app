...
test("四条线都接上了：格子、路由、主线上下文、回流", () => {
  ...
  const tile = cut(scr, 'const sig = makeupSignalFor ? makeupSignalFor(partner.id)', 'tile("ifroom"');
  assert.match(tile, /zh: "和好间"/, "情侣空间没这一格");
  assert.match(tile, /cur \? "还没了结的那一段" : sig\.on \? sig\.why : "这会儿没什么事"/, "格子上不写为什么亮，跟没亮一样");
