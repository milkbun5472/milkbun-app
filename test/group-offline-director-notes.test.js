...
test("新便签默认三轮，只有成功生成后才扣轮次", () => {
  assert.match(app, /remaining: 3/);
...
test("界面显示剩余轮数、结束状态并支持删除", () => {
  assert.match(components, /短期导演便签 · 每次成功演绎消耗 1 轮/);
  assert.match(components, /还会影响接下来/);
  assert.match(components, /已结束 · 不再喂给模型/);
