...
test("群聊线上必须明说：同一个人可以连着放好几个对象", () => {
  ...
  // ⚠️原来这儿钉的是「一个人连发三条，和三个人各发一条」。那个「三」是整段里唯一的具体
  //   数字，会被当成默认条数抄走（她 2026-09-20：不挂例句世界书时默认都是三个气泡）。
  //   要守的是【连发 ≠ 几个人说话】这个区分，不是那个数字（见 no-bubble-count-anchor-71-98）。
  assert.match(rule, /一个人连着发好几条，和好几个人各发一条/, "把它和「几个人说话」明确分开");
  assert.ok(!/连发三条/.test(rule), "又把那个可抄的数字写回去了");
});
