...
test("沙盒里可以真的出事：钉的是根基，不是「只准编小事」", () => {
  ...
  assert.match(p, /TA的来历、身份、已有的关系这些根基不改写/, "根基照旧不许动");
  assert.match(p, /TA的决定可以有代价，事情可以往不好的方向去，可以留下收不回来的后果/);
  assert.match(p, /不必替TA把风险都绕开/);
});

test("人称那条不再带一串节拍——那串正是「流水账」的骨架", () => {
  const p = P();
  assert.ok(p.includes("全篇只有TA一个人的声音"), "人称这一层本身要在");
  assert.ok(!p.includes("我怎么到的那儿"), "节拍清单又回来了：它会被整串当成每一章的骨架照抄");
  assert.ok(!p.includes("我当时怎么判断"));
  // 人称说清楚靠的是这句对照，不是那串节拍
  assert.ok(p.includes("不是「TA把耳罩拨开一点」，是「我把耳罩拨开一点」"));
});
...
test("拆句归代码、成文归模型：提示词要一段一项，占位值也得跟着改", () => {
  ...
  assert.ok(app.includes(JSON.stringify('"text":"本章正文的一个自然段（TA的第一人称，整段照原样放，不要替播放器拆成一句一项）"').slice(1,-1)));
