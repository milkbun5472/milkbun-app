...
test("① 言秋/anthropic 那条路一个字没动", () => {
  ...
  // v68.67：这两处的判断合成了一份 chatSendShapeFor（诊断页也读同一份），行为一个字没变
  assert.match(app, /roomContextFor\(char, chatKey, room, \{ chat: true, thinOnline: _shape\.thinOnline \}\)/, "房间入口必须透传历史缓存选项");
  assert.match(app, /_shape\.blankRecent \? \{ \.\.\._gated, recentChat: "" \} : _gated/, "又去动那条路了");
});
