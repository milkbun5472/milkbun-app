...
// 她 2026-08-26：「jiwen 也没用宝宝都好久不找我的嘤…我主动消息绝对都开了的一个人没有」。
// 查下来病根：jiwenLastUserRef 是个纯内存 ref，每次开 app 都是空的 →
// 第一次 tick 时「用户最新消息比记录的新」永远成立 → resetConnection() 把思念清零。
// 她每隔一两小时开一次 app，积温就被清一次，永远到不了 0.35。
test("上次见过的用户消息时间要持久化，不然每次开 app 都清零", () => {
  assert.match(app, /const jiwenLastUserRef = useRef\(null\)/);
  assert.match(app, /loadJSON\("x_jiwenSeen", \{\}\)/);
  assert.match(app, /saveJSON\("x_jiwenSeen", m\)/);
  const i = app.indexOf("const seenTs = jiwenSeen()[char.id] || 0;");
  assert.ok(i > 0);
...
test("引擎攒得起来：普通对话八九个小时就该想找人", async () => {
  ...
  vm.runInContext(fs.readFileSync(R("jiwen.js"), "utf8"), ctx);
  const eng = ctx.createJiwen({
...
test("按 TA 今天的作息判醒没醒，没行程才退回 8-23", () => {
  ...
  const pi = app.indexOf("const jw = (typeof window !== \"undefined\" && window.__jiwen && window.__jiwen[cid])");
  const proactive = app.slice(pi, app.indexOf("return; // 一次一个，错峰", pi));
  assert.ok(!/hr < 8 \|\| hr > 23/.test(proactive), "积温这条路上写死的 8-23 要撤掉");
...
test("补记时算出「思念是哪一刻越过阈值的」，消息时间戳落在那个空档里", () => {
  ...
  assert.match(app, /jiwenCrossedRef\.current\[char\.id\] = crossed/);
...
test("聊天设置里有积温进度条，两道阈值都画出来", () => {
  assert.match(comp, /jiwenState && h\("div"/);
  ...
  assert.match(app, /jiwenState: \(typeof window !== "undefined" && window\.__jiwen/);
});
