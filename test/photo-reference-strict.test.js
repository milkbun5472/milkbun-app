...
test("聊天与小剧场共用经典锁脸管线，IF 人设不借用主线职业", () => {
  ...
  assert.match(app, /const prompt = isView \? buildScenePrompt\(char, photoScene, \{ forText: false \}\) : isPart \? buildScenePrompt\(char, photoScene, \{ body: true \}\) : buildPhotoPrompt\(char, sceneForPhoto, st, photoOpts\)/);
  assert.match(app, /const prompt = gIsView \? buildScenePrompt\(spk, gPhotoScene, \{ forText: false \}\) : gIsPart \? buildScenePrompt\(spk, gPhotoScene, \{ body: true \}\) : buildPhotoPrompt\(spk, gPhotoScene, st, gPhotoOpts\)/);
...
test("照片类型在审核降级后仍保持：自拍是自拍，抓拍仍允许抓拍", () => {
  // v72.38：「前置摄像头、一臂距离、自拍透视」这套写死的母题删了（她 2026-09-21）——
  // 这一条守的仍然是【降级之后自拍还是自拍】，所以改钉那个题目本身。
  assert.match(engine, /【TA 自己拍自己的一张】画面里只有 TA 一个人，没有别人在替 TA 拍；脸清楚可辨/);
  ...
  assert.match(prompt, /【TA 自己拍自己的一张】画面里只有 TA 一个人，没有别人在替 TA 拍/);
