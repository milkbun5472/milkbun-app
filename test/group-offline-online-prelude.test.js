...
const groupComponent = components.slice(components.indexOf("function GroupOfflineMode("), components.indexOf("function GroupOfflineMode(") + 25000);

assert(groupComponent.includes('const [sOnlineN, setSOnlineN] = useState(os.onlineCtxN != null ? os.onlineCtxN : 10);'), "online transition state must be declared inside GroupOfflineMode, not another component");
assert(groupComponent.includes('"入场前群聊条数"'), "group offline settings must expose the online transition count");
assert(groupComponent.includes('min: 0, max: 30, step: 1, onChange: setSOnlineN'), "online transition count must be bounded and disableable");
