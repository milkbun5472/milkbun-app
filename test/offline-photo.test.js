...
test("合照墙把线下那些也捞上来，并且不靠「今天进过线下」", () => {
  ...
  // v58.97 起还认照相馆那些、v59.17 起还认情侣空间生的图，所以桩里要给这两个 ref
  //（都给空的，这条只验线上/线下两路）
  const run = offlines => new Function("chats", "offlines", "loadJSON", "studioRef", "coupleShotsRef",
    "return (" + body.replace(/;\s*$/, "") + ");")(chats, offlines, k => disk[k] || [], { current: [] }, { current: [] });
  // ① 内存里已经加载过线下
  ...
  const c = new Function("chats", "offlines", "loadJSON", "studioRef", "coupleShotsRef", "return (" + body.replace(/;\s*$/, "") + ");")(
    { c1: [] }, {}, k => disk2[k] || [], { current: [] }, { current: [] })("c1");
