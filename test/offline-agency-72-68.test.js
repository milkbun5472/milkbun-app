...
test("把「谁的决定归谁」说清楚，而不是再加一条禁令", () => {
  ...
  assert.match(runtime, /往下写，需不需要替她做主？/, "没划清哪些才该停下来等她");
  assert.match(runtime, /她可以插嘴的地方不是停下的理由/, "又会在第一处能接话的地方就停（她 2026-09-23：一拍太短）");
  // ⚠️这条是关键：停下来等 ≠ 摆两个选项
...
test("大事归 TA 推，最后那一下归她", () => {
  ...
  assert.match(runtime, /需要了，才停/, "没把主动权还给她");
...
test("替我行动那两支各补一半，而且共用同一条尾巴", () => {
  ...
  assert.match(seg, /一直写到【再往下就得替她做主】的那一拍/, "没说清停在哪一拍");
  assert.match(seg, /不是停的理由/, "尾部又会叫它见缝就停");
  // 开着那一支要有刹车
