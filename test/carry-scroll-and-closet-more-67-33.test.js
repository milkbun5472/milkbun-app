...
test("prop 一路传到底（漏一处就是个点不动的按钮）", () => {
  ...
  // ⚠️别钉整串参数表：v70.81 中间插了 onDeleteItem，钉全的话这条会红，
  //   而它想守的是「onGenClosetMore / closetBusy 有没有往下传」。只钉那几个名字。
  assert.match(S, /onTogglePin, onDeleteItem, onPeek, onGen, onGenClosetMore, closetBusy, onGenGiftThought, onBack\n\s*\}\)\)\)\)\);/, "CarryAll 没往下传");
