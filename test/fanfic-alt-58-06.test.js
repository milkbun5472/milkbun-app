...
test("深浅和序号【由此刻排第几算出来】，不存在文章上", () => {
  ...
  // ⚠️v64.63 那根单独的「删掉这一篇」曾让这一页每篇外面多包一层 div（key 跟着挪上去）；
  //   v72.14 删除并回卡片上那一颗 ✕ 之后那层包装就没用了，key 又回到卡片本身。
  assert.match(fic, /h\(FicCard, \{ key: f\.id, fic: f, index: i, noLead: true,/, "key 得落在卡片上");
});
