...
test("旧档空性别沿用他，明确选择保持不变", () => {
  ...
  assert.equal(P.text({gender:"她"}, "他的手机，其他人和他们不改"), "她的手机，其他人和他们不改");
});

// 只在一处改：211 行逐个手改必然漏，而且下次加一段又要重来
test("共用规则只处理应用模板，拼好的引用与历史不做整段替换", () => {
  assert.match(ph, /const _full = spec\.instruction \+/, "没先把整段拼出来");
  assert.match(ph, /instruction: _full/, "拼装结果应原样保留引用");
  assert.doesNotMatch(ph, /instruction: phoneTa\(_full/);
  assert.match(ph, /characterText\(char,/);
  const chunk = ph.slice(ph.indexOf("const _full = spec.instruction"));
