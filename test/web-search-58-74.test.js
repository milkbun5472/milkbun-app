...
test("他去查了什么，一路要看得见", () => {
  ...
  // 那一行是否出现，两个条件各自独立
  // v73.23 起这个判断收成一处 hasReasonRow，列表两处和「定位」那一头都问它
  assert.equal((comp.match(/\(m\.reasoning \|\| \(m\.searched \|\| \[\]\)\.length \|\| \(m\.usedTools \|\| \[\]\)\.length\)/g) || []).length, 1,
    "决定要不要画那一行的判断又散回好几处了");
  assert.ok((comp.match(/hasReasonRow\(m\) \?/g) || []).length >= 2, "列表那两处没走 hasReasonRow");
});
