...
// ⚠️v73.29 反过来了：原来这里要求把动作/表情模板一条条点名，结果那张单子每轮都把霸总写法
//   念给模型听一遍（她 2026-09-23：「为什么线下还是会默认霸总啊」，同「质子」那次）。
//   现在只留判定，不留清单（施工规则/prompt-no-content-samples.md）。
test("霸总那一套给判定，不念清单", () => {
  assert.match(RUNTIME, /【别滑进网文腔·尤其是霸总那一套】/);
  ["攥住手腕", "扣住手腕", "把人往怀里带", "拦在身前", "拇指在对方腕骨上碾", "冷笑", "嗤笑", "挑眉", "嘴角勾起一个没什么温度的笑"].forEach(k =>
    assert.ok(!RUNTIME.includes(k), "又把模板念出来了：" + k));
  assert.match(RUNTIME, /原样换给任何一个同类型的角色也成立吗/);
...
test("禁的是模板，不是强势的人物", () => {
  ...
  // 给了额度而不是一刀切
  assert.match(RUNTIME, /重写成只有这个人、此刻、对她才会有的那一下/);
});
