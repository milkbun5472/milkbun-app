...
test("是一条不是两条——同判据的东西不许拆开说两遍", () => {
  ...
  // ⚠️v70.19 换了判据。原来写的是「换一个人，这句话照样能原样说出口吗？能，那它就不是你的」——
  //   那是把库里那句老判据【搬错了对象】：它管的是【人物塑造】不该通用，
  //   不是每句台词都得独特。真人聊天大量语料本来就是共享的，
  //   而同一摞里的 ONLINE_CHAT_RULE_V2 明写着「纯笑出来的『哈哈哈哈』、单独一个语气词、
  //   随口的附和……这些低力气消息是关系放松的正常呼吸」——两条直接打架。
  //   现在问的是【这个人此刻会不会这么说】，不是【别人能不能这么说】。
  assert.match(BAN, /这句话是不是【这个人此刻真会说的】？/);
  assert.match(BAN, /跟别人会不会这么说没关系/);
  assert.ok(!/换一个人，这句话照样能原样说出口吗/.test(BAN), "那个会误杀「哈哈哈哈」的判据又回来了");
});
