...
// 反应）——.claude/rules/prompt-no-content-samples.md：给了例子就会被逐字照抄，
// 每个角色都会去做同一件事。规则里只许有判据和维度。
test("规则里不许有内容示范", () => {
  assert.doesNotMatch(rule, /如「|比如「|例如「|、如 /, "又给例句了");
  // 第一版那几个词，一个都不许回来
  ["通译", "番邦", "留过洋", "皱眉", "识字的人", "闹个笑话", "胡闹"]
    .forEach(x => assert.equal(rule.indexOf(x), -1, "示范又回来了：" + x));
  // 换成判据：反应从这个人长出来，不是照着单子挑一个
  assert.match(rule, /怎么表现出来是各人各样的/);
  assert.match(rule, /别所有人都是同一种反应/);
});
