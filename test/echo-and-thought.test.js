...
test("合并型也要削：挤进同一条消息不算逃过", () => {
  ...
  // ⚠️v70.31 提示词那半句撤了（她 2026-09-18：「有一些我怀疑现在不适用了，可以适当放松点」）。
  //   撤的理由是 施工规则/bans-make-it-dumber.md 第①问：**这件事已经有人管了吗**——
  //   合并型由上面这个 strip 在展示前真的削掉，不靠模型自觉。留着那半句是同一件事说两遍，
  //   而它占的是缓存前缀里最贵的位置。所以这儿改成钉【代码那一刀还在】。
});
