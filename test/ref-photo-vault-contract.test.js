...
assert.match(engine, /if \(useRef && refBlobs\.length\) \{/, "有参考 Blob 才走编辑接口");
assert.match(engine, /if \(refMode === "first"\) fd\.append\("image", refBlobs\[0\], refFilename\(refBlobs\[0\]\)\)/, "单图字段必须使用解析后的 Blob");
assert.match(engine, /fd\.append\("image\[\]", blob, refFilename\(blob, i\)\)/, "数组字段也必须使用解析后的 Blob");
assert.doesNotMatch(engine, /refBlobs\.length === 1 \|\| refMode === "first"/, "单张参考照也必须允许 image[] 兼容模式");
