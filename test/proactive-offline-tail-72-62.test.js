...
// ⚠️她当轮追的那一句：「但是也不一定所有人都是线下啊」——上面那条只在真有一场刚散的
//   线下时才出手，纯线上的人一点变化都没有。而纯线上缺的根本不是材料（上次那几十条本来
//   就在上下文里），缺的是【许可】：那句一刀切的「不要默认续接聊天记录最后一句」。
//   所以把两件事拆开：不许续演的是【情绪】，不是话题。
test("隔久主动那一档：拦情绪，不拦话题", () => {
  const i = app.indexOf("【此刻·隔了一阵后主动开口】");
  assert.ok(i > 0, "抠不出隔久主动那一段");
  const seg = app.slice(i, i + 900);
  assert.ok(!/不要默认续接聊天记录最后一句/.test(seg), "那句一刀切还在——纯线上的人照样接不上");
  assert.match(seg, /不许续演的是【情绪】/, "没说清拦的是情绪");
  assert.match(seg, /上次聊到哪儿、一起干了什么、有没有说了一半的事，你是记得的/, "没把话题那一半还回去");
  assert.match(seg, /也可以完全换一件事说/, "只留了一条路——那又成了另一种一刀切");
  assert.match(seg, /别质问 Ta 为什么没回/, "原来那条真正要拦的东西丢了");
});
