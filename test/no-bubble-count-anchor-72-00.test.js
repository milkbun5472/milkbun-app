...
// ---- 4. 光拿掉数字不够，得给判据（她 2026-09-20：三处都清了还是三条）----
// 提示词里只剩一句许可（「话多的人连发几条不是毛病」）时，模型退回自己的先验：
// 要一个 JSON 数组就给三个。所以条数得有个【可执行的判据】——按「几件事」切。
assert.match(rule, /这一轮发几条，看你要说的是几件事/, "条数没有判据，只剩许可");
assert.match(rule, /一件事一条/, "判据得说得出怎么切");
assert.match(rule, /别把两件事压进一条/, "反向那半句丢了");
assert.match(rule, /别停在一个顺手的条数上/, "没挡住「退回默认值」这件事本身");
// 判据里一个具体数字都不许有——那正是前三版在删的东西
// ⚠️「一条」是【单位】不是条数（「一件事一条」「再来一条」），不算；
//   要挡的是给这一轮定量的说法：两条/三条/四条、1~3 条、2-4 条这一族。
const crit = rule.slice(rule.indexOf("【这一轮发几条"), rule.indexOf("标点按【手机打字】来"));
assert.ok(!/[两二三四五六七八九十0-9]\s*[~-]?\s*[0-9]*\s*条/.test(crit),
  "判据里又写进了一个可抄的条数：" + crit.slice(0, 120));

console.log("✓ 气泡条数：数字清干净，并且给了按【几件事】切的判据");
