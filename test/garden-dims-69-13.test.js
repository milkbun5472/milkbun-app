...
// ⚠️这一版最要紧的一条：形变规则【只写在 clay_doll.deform 一处】，导出成形态键，
...
test('形变只在 Blender 那一处，网页只送「值-1」', () => {
  assert.match(exp, /clay_doll.export\(OUT\)/);
  assert.match(doll, /deform\(v.co,o,key\)/, '导出没生成形态键');
  assert.match(doll, /export_morph=True/);
...
test('空的形态键要摘掉——留着白涨接近两兆', () => {
  assert.match(doll, /if all\(\(v.co-p\).length<1e-6 for v,p in zip\(basis.data,coords\)\):continue/);
});
