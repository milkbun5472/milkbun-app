...
test("逼它扣住真实发生过的事，并给一条可判定的自检", () => {
  ...
  assert.match(imp, /「温柔又坚强」「像月亮一样」这种谁都能套的词一律不许用/);
...
test("句式池整体挪到肖像式，池子里不许再有叙事形状", () => {
  ...
  assert.ok(!m.QUOTE_FORMS.some(x => /引用|原话|抱怨|第一次注意到|没告诉她/.test(x)),
    "这些都会写出轶事，不是印象");
  // 肖像形状：比喻整个人 / 感官 / 判断句 / 两面 / 用动词定义
  ["比喻", "感官", "她是……", "两面", "动词"].forEach(k =>
...
test("title 改成「类型名」，不是外号也不是事件概括", () => {
  ...
  assert.match(imp, /不是外号，不是事件概括，也不是抽象概念拼出来的机器词/);
});
...
// v54.45：她拿别家的「初形象生成」卡对照——那才是她要的效果：温润流动的鉴定词
// （总意象＋两面性格＋一丝反差余味），title 是「气质定语＋真实身份」，tags 像
// 「静谧慵懒」「知性松弛」那样顺口。旧池子里刻意求怪的面（名词相撞/只用否定）
// 正是「过载的嗡鸣」「融化的黄油」「逻辑闭环」这类冷硬技术腔的出处，整体换血。
test("文案的靶子是温润鉴定词，不是求怪", () => {
  const m = dice();
  assert.ok(!m.QUOTE_FORMS.some(x => /名词】撞在一起|只用否定|不许用「却」/.test(x)),
    "刻意求怪的面全部退场——怪腔就是它们喂出来的");
  assert.ok(m.QUOTE_FORMS.some(x => /两面/.test(x)), "两面性格是「初形象生成」那种卡的主心骨");
  assert.match(imp, /怪不是目的，准才是/);
  assert.match(imp, /温润、流动、一读就懂/);
  assert.match(imp, /机械或技术黑话/, "「逻辑闭环」「未捕获的异常」这类腔要点名拦");
  assert.match(imp, /气质定语＋一个【真实身份】/, "title 学「清冷理性的科研学者」的构词");
  assert.match(imp, /「静谧慵懒」「知性松弛」/, "tags 的词形给出正面样例");
  assert.match(imp, /40~80 字的【鉴定词】，二到三个短句/, "quote 从一句话扩成一小段鉴定词");
});

test("卡片版式照「初形象生成」那套：白点连线标签＋英文水印＋大引号", () => {
  assert.match(imp, /白点＋细线＋金边小签/);
  ...
  const row = imp.slice(imp.indexOf("相纸下半张那道白边"), imp.indexOf("印象变了哪儿") + 60);
