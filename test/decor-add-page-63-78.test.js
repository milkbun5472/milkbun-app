...
const PAGE = cut("showDecorLibrary && (function () {", "\n}\n// 主页名片");
...
test("整页，不是半窗（no-half-sheet）", () => {
  ...
  assert.match(PAGE, /h\(Head, \{ zh: "做一件装饰"/);
...
test("一次只摊开一段，收起来还看得见自己选了什么", () => {
  ...
  assert.match(PAGE, /meta\.name \+ \(decorDraftType === "photo" && frameName \? " · " \+ frameName : ""\)/);
...
test("顶上钉一张实时预览，画的就是等会儿真放上去的那一份", () => {
  ...
  //   而且不会有任何报错。
  assert.match(comp, /function decorDraftItem\(id\) \{/);
  assert.equal((comp.match(/decorDraftItem\(/g) || []).length, 3, "造草稿的口子不止一个，或者有一处没用它");
  assert.match(PAGE, /var PV = decorDraftItem\("__preview"\);/);
  assert.match(comp, /var item = decorDraftItem\(id\);/, "真放上去那一处没用同一份");
  // 预览要连壳带材质一起画，不然看到的和桌面上的还是两回事
  assert.match(PAGE, /homeWidgetPresetStyle\(decorDraftPreset, t, decorDraftType\)/);
  assert.match(PAGE, /homeDecorMaterialStyle\(PV, t, decorDraftPreset\)/);
  assert.match(PAGE, /h\(HomeDecorItem, \{ item: PV, preset: decorDraftPreset, now: now \}\)/);
  // 竖着的那几款按竖的比例摆，别塞进一个横台子里
  assert.match(PAGE, /HOME_PHOTO_FRAMES_TALL\[decorDraftFrame\] \|\| decorDraftType === "bookmark" \|\| decorDraftType === "scroll" \? 96 : 208/);
});
...
test("二十来种相框默认只露八种", () => {
  ...
  assert.match(PAGE, /h\(HomePhotoFrameGrid, \{ value: decorDraftFrame, list: frames/);
...
test("按钮钉在底下，不用把整页滚到尽头才够得着", () => {
  ...
  assert.match(foot, /onClick: addDecoration/);
  assert.match(foot, /"放到桌面上"/);
});

test("退出这一页要把草稿和摊开的那一段一起清干净", () => {
  assert.match(PAGE, /setShowDecorLibrary\(false\); resetDecorDraft\(\); setDecorStep\("what"\); setDecorFrameAll\(false\);/);
});
